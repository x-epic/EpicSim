...
static int t_version_only(void)
{
      ...
      fflush(0);
      snprintf(tmp, sizeof tmp, "%s%cepicsim-precompiler -V", ivlpp_dir, sep);
      rc = system(tmp);
      if (rc != 0) {
      ...
      fflush(0);
      snprintf(tmp, sizeof tmp, "%s%cepicsim-compiler -V -C\"%s\" -C\"%s\"", base, sep,
...
static void build_preprocess_command(int e_flag)
{
      if(use_compath) {
            char path[1024] = {0};
            snprintf(path,sizeof path,"%s%c/lib/epicsim",compile_base,sep);
            ivlpp_dir = path;
      }
      snprintf(tmp, sizeof tmp, "%s%cepicsim-precompiler %s%s%s -F\"%s\" -f\"%s\" -p\"%s\"%s",
	       ivlpp_dir, sep,
               verbose_flag ? " -v" : "",
	       e_flag ? "" : " -L",
...
static void find_ivl_root(void)
{
      ...
      const char *ivl_lib_suffix = "/epicsim/epicsim-compiler" IVL_SUFFIX;
      ssize_t len = 0;
      char *s;
	...
	/* First try the location specified in the build process. */
      /* First try the location specified in the build process. */
      if (access(GALAXSIM_LIB_ROOT, F_OK) != -1) {
        assert(strlen(GALAXSIM_LIB_ROOT) < sizeof ivl_root);
        strcpy(ivl_root, GALAXSIM_LIB_ROOT);
        return;
      } else if (access(IVL_ROOT, F_OK) != -1) {
...
static void find_compath_base(void)
{
    int cnt = readlink("/proc/self/exe",compile_base , 1024);
    if(cnt < 0|| cnt >= 1024)
    {
        return;
    }
...
int main(int argc, char **argv)
{
	...
	/* Write values to the iconfig file. */
      if (use_compath)
          fprintf(iconfig_file, "basedir:%s/lib/epicsim\n", compile_base);
      else 
	...
	/* Write the preprocessor command needed to preprocess a
	   ...
	   files. */
      fprintf(iconfig_file, "ivlpp:%s%cepicsim-precompiler %s -L -F\"%s\" -P\"%s\"\n",
              ivlpp_dir, sep,
              strchr(warning_flags, 'r') ? "-Wredef-all" :
              strchr(warning_flags, 'R') ? "-Wredef-chg" : "",
