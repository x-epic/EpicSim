...
static void add_vpi_file(const char* name) {
    ...
    if (strchr(name, sep)) {
        ...
        for (unsigned idx = 0; !found && (idx < vpi_path_list_size); idx += 1) {
            found = probe_for_vpi_module(vpi_path_list[idx], name, path,
                                         sizeof(path));
        }
        for (unsigned idx = 0; !found && (idx < env_vpi_path_list_size);
             idx += 1) {
...
int main(int argc, char** argv) {
    ...
    while ((opt = getopt(
                argc, argv,
                "c:D:d:Ef:g:hl:I:iL:M:m:N:o:O:P:p:qSs:T:t:uvVW:y:Y:")) != EOF) {
        switch (opt) {
            case 'c':
            case 'f':
                add_cmd_file(optarg);
            ...
            case 'P':
                defparm_size += 1;
                defparm_base = (const char**)realloc(
                    defparm_base, defparm_size * sizeof(char*));
