...
/*
 *  Verilog-A math library for EpicSim
...
static double va_fmin(const double x, const double y) {
  if (x != x) {
    return y;
  } /* x is NaN so return y. */
  if (y != y) {
    return x;
  } /* y is NaN so return x. */
...
static t_single_data va_single_data[] = {
    {"$abs", fabs}, {0, 0} /* Must be NULL terminated! */
};
...
static PLI_INT32 va_single_argument_compiletf(ICARUS_VPI_CONST PLI_BYTE8 *ud) {
  ...
  single_funcs = (va_single_t **)realloc(
      single_funcs, single_funcs_count * sizeof(va_single_t **));
...
static PLI_INT32 va_double_argument_compiletf(ICARUS_VPI_CONST PLI_BYTE8 *ud) {
  ...
  double_funcs = (va_double_t **)realloc(
      double_funcs, double_funcs_count * sizeof(va_double_t **));
...
/*
 * Hook to get EpicSim to find the registration function.
 */
void (*vlog_startup_routines[])(void) = {va_math_register, 0};
