...
#else
static void sys_lxt_register(void) {
...
static void sys_lxt2_register(void) {
  fputs("LXT2 support disabled since zlib not available\n", stderr);
  exit(1);
}
static void sys_fst_register(void) {
...
static void sys_lxt_or_vcd_register(void) {
  ...
  /* Get the dumper of choice from the EPICSIM_DUMPER
     environment variable. */
  dumper = getenv("EPICSIM_DUMPER");
  if (dumper) {
...
void (*vlog_startup_routines[])(void) = {sys_convert_register,
