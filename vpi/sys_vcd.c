...
__inline__ static int install_dumpvars_callback(void) {
  ...
  if (dumpvars_status == 2) {
    vpi_printf(
        "VCD warning: $dumpvars ignored, previously"
...
static PLI_INT32 sys_dumpfile_calltf(ICARUS_VPI_CONST PLI_BYTE8 *name) {
  ...
  if (dumpvars_status != 0) {
    ...
    snprintf(msg, sizeof(msg),
             "VCD warning: %s:%d:", vpi_get_str(vpiFile, callh),
             (int)vpi_get(vpiLineNo, callh));
    msg[sizeof(msg) - 1] = 0;
...
static void scan_item(unsigned depth, vpiHandle item, int skip) {
  ...
  switch (item_type) {
    ...
    default:
      vpi_printf(
          "VCD warning: $dumpvars: Unsupported argument "
          "type (%s)\n",
  ...
  if (item_type == vpiMemoryWord) {
    ...
    if (vpi_get(vpiType, item) == vpiMemoryWord &&
      ...
      vpi_printf(
          "VCD warning: array word %s will conflict "
  ...
  switch (item_type) {
    ...
    case vpiNet:
      ...
      fprintf(dump_file, "$var %s %u %s %s%s", type, size, ident, prefix, name);

      /* Add a range for vectored values. */
      if (size > 1 || vpi_get(vpiLeftRange, item) != 0) {
        fprintf(dump_file, " [%i:%i]", (int)vpi_get(vpiLeftRange, item),
    ...
    case vpiNamedFork:
      ...
      if (depth > 0) {
        ...
        if (nskip) {
          vpi_printf(
              "VCD warning: ignoring signals in "
...
static int draw_scope(vpiHandle item, vpiHandle callh) {
  ...
  switch (vpi_get(vpiType, scope)) {
    ...
    default:
      type = "invalid";
      vpi_printf(
          "VCD Error: %s:%d: $dumpvars: Unsupported scope "
          "type (%d)\n",
