...
extern "C" void nexus_ident_delete() { nexus_ident_map.clear(); }

static std::map<int, const char*> fsdbnexus_ident_map;

extern "C" const char* fsdbfind_nexus_ident(int nex) {
  std::map<int, const char*>::const_iterator cur =
      fsdbnexus_ident_map.find(nex);
...
extern "C" void fsdbnexus_ident_delete() { fsdbnexus_ident_map.clear(); }
...
extern "C" void vcd_scope_names_delete(void) { vcd_scope_names_set.clear(); }
...
extern "C" void vcd_work_thread_pop(void) {
  ...
  use_next += 1;
  if (use_next >= WORK_QUEUE_SIZE) use_next = 0;
...
static struct vcd_work_item_s* grab_item(void) {
  ...
  if (cur >= WORK_QUEUE_SIZE) cur -= WORK_QUEUE_SIZE;
...
static void end_batch(void) {
  ...
  if (was_empty_flag) pthread_cond_signal(&work_queue_notempty_sig);
...
static inline void unlock_item(bool flush_batch = false) {
  current_batch_cnt += 1;
  if (current_batch_cnt == current_batch_alloc || flush_batch) end_batch();
}

extern "C" void vcd_work_sync(void) {
  if (current_batch_alloc > 0) end_batch();
...
extern "C" void vcd_work_set_time(uint64_t val) { work_queue_next_time = val; }
...
extern "C" void vcd_work_emit_bits(struct lxt2_wr_symbol* sym,
                                   const char* val) {
  struct vcd_work_item_s* cell = grab_item();
