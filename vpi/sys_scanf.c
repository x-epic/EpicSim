...
static int scan_format_float(vpiHandle callh, vpiHandle argv,
                             ...
                             unsigned suppress_flag,
                             ICARUS_VPI_CONST PLI_BYTE8 *name, char code) {
  vpiHandle arg;
  int match;
  s_vpi_value val;
  double result;
  ...
  if (!arg) {
    ...
    vpi_printf("%s() ran out of variables for %%%c format code.", name, code);
...
static int scan_format_base(vpiHandle callh, vpiHandle argv,
                            struct byte_source *src, unsigned width,
                            unsigned suppress_flag,
                            ICARUS_VPI_CONST PLI_BYTE8 *name, const char *match,
                            char code, PLI_INT32 type) {
  vpiHandle arg;
  char *strval = malloc(1);
  unsigned len = 0;
  s_vpi_value val;
...
static int scan_format_hex(vpiHandle callh, vpiHandle argv,
                           ...
                           ICARUS_VPI_CONST PLI_BYTE8 *name) {
  return scan_format_base(callh, argv, src, width, suppress_flag, name,
                          "0123456789abcdefxzABCDEFXZ?_", 'h', vpiHexStrVal);
}
      ...
      for (byte = 0; byte < 4; byte += 1) {
        ...
        if (ch == EOF) {
          vpi_printf("WARNING: %s:%d: ", vpi_get_str(vpiFile, callh),
                     (int)vpi_get(vpiLineNo, callh));
          vpi_printf(
              "%s() only found %u of %u bytes "
              "needed by %%u format code.\n",
              name, word * 4U + byte, words * 4U);
          return 0;
        }
      }
    }
    ...
    for (byte = 0; byte <= 3; byte += 1) {
      ...
      if (ch == EOF) {
        vpi_printf("WARNING: %s:%d: ", vpi_get_str(vpiFile, callh),
                   (int)vpi_get(vpiLineNo, callh));
        vpi_printf(
            "%s() only found %d of %u bytes needed by "
            "%%u format code.\n",
            name,
            word * 4 +
#ifdef WORDS_BIGENDIAN
  ...
  while (fmtp && *fmtp != 0 && match) {
    if (isspace((int)*fmtp)) {
      ...
      switch (code) {
        ...
        case 'b':
          match = scan_format_binary(callh, argv, src, max_width, suppress_flag,
                                     name);
          if (match == 1) rc += 1;
          break;

        case 'c':
          match = scan_format_char(callh, argv, src, max_width, suppress_flag,
                                   name);
          if (match == 1) rc += 1;
          break;
        ...
        case 'f':
        case 'g':
          match = scan_format_float(callh, argv, src, max_width, suppress_flag,
                                    name, code);
          if (match == 1) rc += 1;
          break;
        ...
        case 'x':
          match =
              scan_format_hex(callh, argv, src, max_width, suppress_flag, name);
          if (match == 1) rc += 1;
          break;
        ...
        case 'm':
          ...
          match = scan_format_module_path(callh, argv, suppress_flag, name);
          if (match == 1) rc += 1;
          break;

        case 'o':
          match = scan_format_octal(callh, argv, src, max_width, suppress_flag,
                                    name);
          if (match == 1) rc += 1;
          break;

        case 's':
          match = scan_format_string(callh, argv, src, max_width, suppress_flag,
                                     name);
          if (match == 1) rc += 1;
          break;

        case 't':
          match = scan_format_float_time(callh, argv, src, max_width,
                                         suppress_flag, name);
          if (match == 1) rc += 1;
          break;

        case 'u':
          match = scan_format_two_state(callh, argv, src, max_width,
                                        suppress_flag, name);
        ...
        case 'z':
          match = scan_format_four_state(callh, argv, src, max_width,
                                         suppress_flag, name);
          /* If a binary match fails and it is the first item
           * matched then treat that as an EOF. */
          if ((match == 0) && (rc == 0)) rc = EOF;
        ...
        default:
          vpi_printf("ERROR: %s:%d: ", vpi_get_str(vpiFile, callh),
                     (int)vpi_get(vpiLineNo, callh));
          vpi_printf(
              "%s() was given an invalid format code: "
              "%%%c\n",
              name, code);
          vpi_control(vpiFinish, 1);
          break;
