...
void parm_to_defparam_list(const string& param) {
    ...
    *ptr++ = '\0';
    ...
    if (bit_l != 0) {
      *bit_l++ = '\0';
      ...
      if (bit_r == 0) {
        cerr << "<command line>: error: missing ']' for defparam: " << nkey
             << endl;
        free(key);
        free(value);
        return;
      ...
      *bit_r = '\0';
      ...
      while (*(bit_l + i) != '\0')
        if (!isdigit(*(bit_l + i++))) {
          cerr << "<command line>: error: scope index expression is not "
                  "constant: "
               << nkey << endl;
          free(key);
          free(value);
  ...
  if (*value == '"') {
    ...
    while (*buf_ptr != '"' && *buf_ptr != '\0') {
      ...
      // end of string.
      if (*buf_ptr == '\\' && *(buf_ptr + 1) != '\0') buf_ptr += 2;
    }
    if (*buf_ptr == '\0')  // String end without '"'
      cerr << "<command line>: error: missing close quote of string for "
              "defparam: "
           << name << endl;
    else if (*(buf_ptr + 1) != 0) {  // '"' appears within string with no escape
      cerr << buf_ptr << endl;
      cerr << "<command line>: error: \'\"\' appears within string value for "
              "defparam: "
           << name << ". Ignore characters after \'\"\'" << endl;
    }
    ...
    switch (*base) {
      ...
      case 'H':
        if (is_hex_digit_str(base + 1)) {
          ...
          cerr << "<command line>: error: invalid digit in hex value specified "
                  "for defparam: "
               << name << endl;
          free(value);
          return;
      ...
      case 'D':
        if (is_dec_digit_str(base + 1)) {
          ...
          cerr << "<command line>: error: invalid digit in decimal value "
                  "specified for defparam: "
               << name << endl;
          free(value);
          return;
        }
      ...
      case 'O':
        if (is_oct_digit_str(base + 1)) {
          ...
          cerr << "<command line>: error: invalid digit in octal value "
                  "specified for defparam: "
               << name << endl;
          free(value);
          return;
      ...
      case 'B':
        if (is_bin_digit_str(base + 1)) {
          ...
          cerr << "<command line>: error: invalid digit in binary value "
                  "specified for defparam: "
               << name << endl;
          free(value);
          return;
        ...
        break;
      default:
        cerr << "<command line>: error: invalid numeric base specified for "
                "defparam: "
             << name << endl;
        free(value);
        return;
    ...
    if (num != value) {  // based number with size
      *num = 0;
      if (is_dec_digit_str(value)) {
        ...
        cerr << "<command line>: error: invalid size for value specified for "
                "defparam: "
             << name << endl;
        free(value);
        return;
  ...
  // None of the above.
  cerr << "<command line>: error: invalid value specified for defparam: "
       << name << endl;
  free(value);
}
...
void pform_pop_scope() {
  ...
  for (cur = scope->possible_imports.begin();
       cur != scope->possible_imports.end(); ++cur) {
...
static LexicalScope::lifetime_t find_lifetime(
    LexicalScope::lifetime_t lifetime) {
  if (lifetime != LexicalScope::INHERITED) return lifetime;
...
static PPackage* find_potential_import(const struct vlltype& loc,
                                       ...
                                       bool tf_call, bool make_explicit) {
  assert(scope);

  PPackage* found_pkg = 0;
  for (set<PPackage*>::const_iterator cur_pkg =
    ...
    if (cur_sym != search_pkg->local_symbols.end()) {
      if (found_pkg && make_explicit) {
        cerr << loc.get_fileline()
             ...
             << found_pkg->pscope_name() << "' and by '"
             << search_pkg->pscope_name() << "'." << endl;
        error_count += 1;
      } else {
...
static void check_potential_imports(const struct vlltype& loc, perm_string name,
  ...
  LexicalScope* scope = lexical_scope;
  while (scope) {
    if (scope->local_symbols.find(name) != scope->local_symbols.end()) return;
    if (scope->explicit_imports.find(name) != scope->explicit_imports.end())
      return;
    if (find_potential_import(loc, scope, name, tf_call, true)) return;
...
PTask* pform_push_task_scope(const struct vlltype& loc, char* name,
  ...
  if (is_compilation_unit(scopex) && !gn_system_verilog()) {
    cerr << task->get_fileline()
         << ": error: task declarations "
...
PFunction* pform_push_function_scope(const struct vlltype& loc,
                                     const char* name,
                                     LexicalScope::lifetime_t lifetime) {
...
PBlock* pform_push_block_scope(const struct vlltype& loc, char* name,
  ...
  if (name)
    block_name = lex_strings.make(name);
  else {
    // Create a unique name for this unnamed block.
    char tmp[32];
    snprintf(tmp, sizeof tmp, "$unm_blk_%u", scope_unnamed_block_with_decl);
...
PGenerate* pform_parent_generate(void) { return pform_cur_generate; }
...
void pform_bind_attributes(map<perm_string, PExpr*>& attributes,
  ...
  if (!keep_attrs) delete attr;
}

bool pform_in_program_block() {
  if (pform_cur_module.empty()) return false;
  if (pform_cur_module.front()->program_block) return true;
  return false;
}

bool pform_in_interface() {
  if (pform_cur_module.empty()) return false;
  if (pform_cur_module.front()->is_interface) return true;
  return false;
}
...
static void pform_put_enum_type_in_scope(enum_type_t* enum_set) {
  if (lexical_scope->enum_sets.count(enum_set)) return;
...
PWire* pform_get_make_wire_in_scope(const struct vlltype&, perm_string name,
  ...
  if (cur && cur->get_data_type() != IVL_VT_NO_TYPE) cur = 0;
...
void pform_set_typedef(perm_string name, data_type_t* data_type,
  ...
  if (unp_ranges) data_type = new uarray_type_t(data_type, unp_ranges);
...
data_type_t* pform_test_type_identifier(const struct vlltype& loc,
                                        const char* txt) {
  ...
  do {
    ...
    if (cur != cur_scope->typedefs.end()) return cur->second;
...
bool pform_test_type_identifier_local(perm_string name) {
  ...
  cur = cur_scope->typedefs.find(name);
  if (cur != cur_scope->typedefs.end()) return true;
...
PForeach* pform_make_foreach(const struct vlltype& loc, char* name,
                             list<perm_string>* loop_vars, Statement* stmt) {
...
void pform_set_default_nettype(NetNet::Type type, const char* file,
                               unsigned lineno) {
  ...
  if (!pform_cur_module.empty()) {
    ...
    cerr << file << ":" << lineno << ":      : "
         ...
         << " starts on line " << pform_cur_module.back()->get_fileline() << "."
         << endl;
    error_count += 1;
  }
}
...
static void pform_declare_implicit_nets(PExpr* expr) {
  ...
  if (pform_default_nettype == NetNet::NONE) return;

  if (expr) expr->declare_implicit_nets(lexical_scope, pform_default_nettype);
}
 ...
 */
void pform_set_timescale(int unit, int prec, const char* file,
                         unsigned lineno) {
  assert(unit >= prec);
...
bool get_time_unit(const char* cp, int& unit) {
  ...
  if (strchr(cp, '_')) {
    VLerror(yylloc,
            "Invalid timeunit constant ('_' is not "
            "supported).");
    return false;
  }
...
static bool get_time_unit_prec(const char* cp, int& res, bool is_unit) {
  ...
  if (strchr(cp, '_')) {
    if (is_unit) {
      ...
      VLerror(yylloc,
              "Invalid timeprecision constant ('_' is not "
              "supported).");
    }
    return true;
  }
  ...
  if (res > 2) {
    if (is_unit) {
      VLerror(yylloc,
              "Invalid timeunit constant (number of "
              "zeros).");
    } else {
      VLerror(yylloc,
              "Invalid timeprecision constant (number of "
              "zeros).");
    }
    return true;
  }
...
void pform_set_timeunit(const char* txt, bool initial_decl) {
  ...
  if (initial_decl) {
    ...
    VLerror(yylloc,
            "error: repeat timeunit found and the initial "
            "timeunit for this scope is missing.");
  } else if (scope->time_unit != val) {
    VLerror(yylloc,
            "error: repeat timeunit does not match the "
...
void pform_set_timeprec(const char* txt, bool initial_decl) {
  ...
  if (initial_decl) {
    ...
    VLerror(yylloc,
            "error: repeat timeprecision found and the initial "
            "timeprecision for this scope is missing.");
  } else if (scope->time_precision != val) {
    VLerror(yylloc,
            "error: repeat timeprecision does not match the "
            "initial timeprecision for this scope.");
  }
}

verinum* pform_verinum_with_size(verinum* siz, verinum* val, const char* file,
                                 unsigned lineno) {
...
void pform_startmodule(const struct vlltype& loc, const char* name,
  ...
  if (!pform_cur_module.empty() && !gn_system_verilog()) {
    cerr << loc << ": error: Module definition " << name
         << " cannot nest into module " << pform_cur_module.front()->mod_name()
         << "." << endl;
    error_count += 1;
  }
  ...
  if (lifetime != LexicalScope::INHERITED && !gn_system_verilog()) {
    cerr << loc
         << ": error: Default subroutine lifetimes "
  ...
  if (gn_system_verilog() && !pform_cur_module.empty()) {
    if (pform_cur_module.front()->program_block) {
      cerr << loc
           << ": error: module, program, or interface "
    ...
    if (pform_cur_module.front()->is_interface &&
      ...
      cerr << loc
           << ": error: module declarations are not "
 ...
 */
Module::port_t* pform_module_port_reference(perm_string name, const char* file,
                                            unsigned lineno) {
...
void pform_endmodule(const char* name, bool inside_celldefine,
  ...
  if (strcmp(name, mod_name) != 0) {
    while (pform_cur_module.size() > 0) {
      ...
      pform_cur_module.pop_front();
      ostringstream msg;
      msg << "Module " << mod_name << " was nested within " << tmp_name
          << " but broken.";
      VLerror(msg.str().c_str());
      ...
      mod_name = tmp_name;
      if (strcmp(name, mod_name) == 0) break;
    }
  }
  ...
  // if needed.
  map<perm_string, Module*>& use_module_map =
      (pform_cur_module.empty()) ? pform_modules
...
void pform_start_generate_for(const struct vlltype& li, bool local_index,
                              char* ident1, PExpr* init, PExpr* test,
...
void pform_start_generate_nblock(const struct vlltype& li, char* name) {
  ...
  add_local_symbol(pform_cur_generate->parent_scope(),
                   pform_cur_generate->scope_name, pform_cur_generate);
}
 ...
 */
void pform_generate_case_item(const struct vlltype& li,
                              list<PExpr*>* expr_list) {
  assert(pform_cur_generate);
...
void pform_generate_block_name(char* name) {
  ...
  if (pform_cur_generate->scheme_type == PGenerate::GS_CONDIT ||
      pform_cur_generate->scheme_type == PGenerate::GS_ELSE ||
      pform_cur_generate->scheme_type == PGenerate::GS_CASE_ITEM) {
    if (conditional_block_names.front().count(scope_name)) return;
...
void pform_endgenerate(bool end_conditional) {
  ...
  if (end_conditional) conditional_block_names.pop_front();
...
template <>
inline svector<perm_string>::svector(unsigned size)
    : nitems_(size), items_(new perm_string[size]) {}

static void process_udp_table(PUdp* udp, list<string>* table, const char* file,
                              unsigned lineno) {
  ...
  {
    unsigned idx = 0;
    for (list<string>::iterator cur = table->begin(); cur != table->end();
         ++cur, idx += 1) {
      string tmp = *cur;
...
void pform_make_udp(perm_string name, list<perm_string>* parms,
                    ...
                    Statement* init_expr, const char* file, unsigned lineno) {
  ...
  svector<perm_string> pin_names(parms->size());
  {
    ...
    unsigned idx;
    for (cur = parms->begin(), idx = 0; cur != parms->end(); ++idx, ++cur) {
      pins[idx] = defs[*cur];
      pin_names[idx] = *cur;
    }
  }
  ...
  */
  ...
  for (unsigned idx = 1; idx < pins.count(); idx += 1) {
    if (pins[idx] == 0) {
      cerr << file << ":" << lineno << ": error: "
           << "Port " << (idx + 1) << " of primitive " << name << " missing"
           << " input declaration." << endl;
      cerr << file << ":" << lineno << ":      : "
           << "Try: input " << pin_names[idx] << ";" << endl;
      error_count += 1;
      local_errors += 1;
      continue;
    }
    if (pins[idx]->get_port_type() != NetNet::PINPUT) {
      cerr << file << ":" << lineno << ": error: "
           << "Input port " << (idx + 1) << " of primitive " << name
           << " has an output (or missing) declaration." << endl;
      cerr << file << ":" << lineno << ":      : "
           << "Note that only the first port can be an output." << endl;
      cerr << file << ":" << lineno << ":      : "
           << "Try \"input " << name << ";\"" << endl;
      error_count += 1;
      local_errors += 1;
    ...
    if (pins[idx]->get_wire_type() == NetNet::REG) {
      cerr << file << ":" << lineno << ": error: "
           << "Port " << (idx + 1) << " of primitive " << name
           << " is an input port"
           << " with a reg declaration." << endl;
      cerr << file << ":" << lineno << ":      : "
           << "primitive inputs cannot be reg." << endl;
      error_count += 1;
      local_errors += 1;
      continue;
    }
  ...
  if (pform_primitives[name]) {
    ...
    if (pins[0]->get_wire_type() == NetNet::REG) udp->sequential = true;
...
void pform_make_udp(perm_string name, bool synchronous_flag,
  ...
  pins[0] = new PWire(out_name, synchronous_flag ? NetNet::REG : NetNet::WIRE,
  ...
  /* Make the PWire objects for the input ports. */
  {
    list<perm_string>::iterator cur;
    unsigned idx;
    for (cur = parms->begin(), idx = 1; cur != parms->end(); idx += 1, ++cur) {
      assert(idx < pins.count());
      pins[idx] = new PWire(*cur, NetNet::WIRE, NetNet::PINPUT, IVL_VT_LOGIC);
 ...
 */
static void pform_set_net_range(perm_string name, NetNet::Type net_type,
                                const list<pform_range_t>* range,
                                bool signed_flag, ivl_variable_type_t dt,
                                PWSRType rt, std::list<named_pexpr_t>* attr) {
  PWire* cur = pform_get_wire_in_scope(name);
  if (cur == 0) {
  ...
  if (net_type != NetNet::IMPLICIT && net_type != NetNet::NONE) {
    ...
    if (rc == false) {
      ostringstream msg;
      msg << name << " " << net_type << " definition conflicts with "
          << cur->get_wire_type() << " definition at " << cur->get_fileline()
          << ".";
      VLerror(msg.str().c_str());
    }
  }
  ...
  if (dt != IVL_VT_NO_TYPE) cur->set_data_type(dt);

  pform_bind_attributes(cur->attributes, attr, true);
}

static void pform_set_net_range(list<perm_string>* names,
                                list<pform_range_t>* range, bool signed_flag,
                                ivl_variable_type_t dt, NetNet::Type net_type,
                                std::list<named_pexpr_t>* attr) {
  for (list<perm_string>::iterator cur = names->begin(); cur != names->end();
       ++cur) {
    perm_string txt = *cur;
 ...
 */
static void pform_makegate(PGBuiltin::Type type, struct str_pair_t str,
                           list<PExpr*>* delay, const lgate& info,
                           list<named_pexpr_t>* attr) {
  if (info.parms_by_name) {
    cerr << info.file << ":" << info.lineno
         << ": Gates do not "
            "have port names."
         << endl;
    error_count += 1;
  ...
  if (info.range.first) cur->set_range(info.range.first, info.range.second);
  ...
  if (pform_cur_generate) {
    ...
    if (dev_name != "")
      add_local_symbol(pform_cur_module.front(), dev_name, cur);
    pform_cur_module.front()->add_gate(cur);
  }
}

void pform_makegates(const struct vlltype& loc, PGBuiltin::Type type,
                     struct str_pair_t str, list<PExpr*>* delay,
                     svector<lgate>* gates, list<named_pexpr_t>* attr) {
  assert(!pform_cur_module.empty());
  if (pform_cur_module.front()->program_block) {
 ...
 */
static void pform_make_modgate(perm_string type, perm_string name,
                               struct parmvalue_t* overrides,
                               list<PExpr*>* wires, PExpr* msb, PExpr* lsb,
                               const char* fn, unsigned ln,
                               std::list<named_pexpr_t>* attr) {
  for (list<PExpr*>::iterator idx = wires->begin(); idx != wires->end();
       ++idx) {
...
static void pform_make_modgate(perm_string type, perm_string name,
                               struct parmvalue_t* overrides,
                               list<named_pexpr_t>* bind, PExpr* msb,
                               PExpr* lsb, const char* fn, unsigned ln,
                               std::list<named_pexpr_t>* attr) {
...
void pform_make_modgates(const struct vlltype& loc, perm_string type,
                         struct parmvalue_t* overrides, svector<lgate>* gates,
                         std::list<named_pexpr_t>* attr) {
  ...
  for (unsigned idx = 0; idx < gates->count(); idx += 1) {
    ...
    if (cur.parms_by_name) {
      pform_make_modgate(type, cur_name, overrides, cur.parms_by_name,
                         cur.range.first, cur.range.second, cur.file,
                         cur.lineno, attr);
      ...
      pform_make_modgate(type, cur_name, overrides, cur.parms, cur.range.first,
                         cur.range.second, cur.file, cur.lineno, attr);

    } else {
      list<PExpr*>* wires = new list<PExpr*>;
      pform_make_modgate(type, cur_name, overrides, wires, cur.range.first,
                         cur.range.second, cur.file, cur.lineno, attr);
    }
  }
...
static PGAssign* pform_make_pgassign(PExpr* lval, PExpr* rval,
                                     list<PExpr*>* del, struct str_pair_t str) {
  /* Implicit declaration of nets on the LHS of a continuous
     assignment was introduced in IEEE1364-2001. */
  if (generation_flag != GN_VER1995) pform_declare_implicit_nets(lval);
...
void pform_make_pgassign_list(list<PExpr*>* alist, list<PExpr*>* del,
                              struct str_pair_t str, const char* fn,
                              unsigned lineno) {
  assert(alist->size() % 2 == 0);
  while (!alist->empty()) {
    PExpr* lval = alist->front();
    alist->pop_front();
    PExpr* rval = alist->front();
    alist->pop_front();
 ...
 */
void pform_make_var_init(const struct vlltype& li, perm_string name,
                         PExpr* expr) {
  if (!pform_at_module_level() && !gn_system_verilog()) {
    VLerror(li,
            "error: variable declaration assignments are only "
 ...
 */
void pform_module_define_port(const struct vlltype& li, perm_string name,
                              NetNet::PortType port_kind, NetNet::Type type,
                              data_type_t* vtype, list<named_pexpr_t>* attr,
                              bool keep_attr) {
  ...
  if (vector_type_t* vec_type = dynamic_cast<vector_type_t*>(vtype)) {
    ...
    if (vec_type->reg_flag) type = NetNet::REG;
  ...
  if (data_type == IVL_VT_NO_TYPE) data_type = IVL_VT_LOGIC;
...
void pform_module_define_port(const struct vlltype& li,
                              ...
                              NetNet::PortType port_kind, NetNet::Type type,
                              data_type_t* vtype, list<named_pexpr_t>* attr) {
  for (list<pform_port_t>::iterator cur = ports->begin(); cur != ports->end();
       ++cur) {
    data_type_t* use_type = vtype;
    if (cur->udims) use_type = new uarray_type_t(vtype, cur->udims);

    pform_module_define_port(li, cur->name, port_kind, type, use_type, attr,
                             true);
    if (cur->udims) delete use_type;

    if (cur->expr) pform_make_var_init(li, cur->name, cur->expr);
  }
...
static PWire* pform_get_or_make_wire(const vlltype& li, perm_string name,
  ...
  if (cur && (cur->get_data_type() == IVL_VT_NO_TYPE ||
    ...
    if (type != NetNet::IMPLICIT) {
      ...
      if (rc == false) {
        ostringstream msg;
        msg << name << " " << type << " definition conflicts with "
            << cur->get_wire_type() << " definition at " << cur->get_fileline()
            << ".";
        VLerror(msg.str().c_str());
      }
...
void pform_add_net_delay(list<perm_string>* names, list<PExpr*>* delay) {
  std::list<perm_string>::iterator iter = names->begin();
  if (iter->str()) {
    PWire* cur = pform_get_wire_in_scope(*iter);
    if (cur) {
      cur->set_delays(delay);
    }
  }
}

void pform_maketrireg(const struct vlltype& li, list<perm_string>* names,
                      NetNet::Type type, list<PExpr*>* delay,
                      int charge_strength, NetNet::PortType pt,
                      ivl_variable_type_t dt, list<pform_range_t>* range,
                      list<named_pexpr_t>* attr) {
  for (list<perm_string>::iterator cur = names->begin(); cur != names->end();
       ++cur) {
    ...
    if (pWire && delay) pWire->set_delays(delay);
 ...
 */
void pform_makewire(const vlltype& li, list<pform_range_t>* range,
                    bool signed_flag, list<perm_string>* names,
                    NetNet::Type type, NetNet::PortType pt,
                    ivl_variable_type_t dt, list<named_pexpr_t>* attr,
                    PWSRType rt) {
  for (list<perm_string>::iterator cur = names->begin(); cur != names->end();
       ++cur) {
    perm_string txt = *cur;
 ...
 */
void pform_makewire(const vlltype& li, list<PExpr*>* delay, str_pair_t str,
                    net_decl_assign_t* decls, NetNet::Type type,
  ...
  do {
    pform_makewire(li, first->name, type, NetNet::NOT_A_PORT, IVL_VT_NO_TYPE,
                   0);
  ...
  while (first) {
    ...
    if (cur != 0) {
      ...
      PGAssign* ass = pform_make_pgassign(lval, first->expr, delay, str);
 ...
 */
void pform_makewire(const struct vlltype& li, std::list<PExpr*>* delay,
                    str_pair_t str, std::list<decl_assignment_t*>* assign_list,
                    NetNet::Type type, data_type_t* data_type) {
  if (is_compilation_unit(lexical_scope) && !gn_system_verilog()) {
    VLerror(li,
            "error: variable declarations must be contained within a module.");
    return;
  }
 ...
 */
vector<pform_tf_port_t>* pform_make_task_ports(
    const struct vlltype& loc, NetNet::PortType pt, ivl_variable_type_t vtype,
    bool signed_flag, list<pform_range_t>* range, list<perm_string>* names,
    bool isint) {
...
vector<pform_tf_port_t>* pform_make_task_ports(const struct vlltype& loc,
  ...
  if (atom2_type_t* atype = dynamic_cast<atom2_type_t*>(vtype)) {
    ...
    ret = pform_make_task_ports(loc, pt, IVL_VT_BOOL, atype->signed_flag,
                                range_tmp, names);
  }

  if (vector_type_t* vec_type = dynamic_cast<vector_type_t*>(vtype)) {
    ret = pform_make_task_ports(
        loc, pt, vec_type->base_type, vec_type->signed_flag,
        copy_range(vec_type->pdims.get()), names, vec_type->integer_flag);
  }

  if (/*real_type_t*real_type = */ dynamic_cast<real_type_t*>(vtype)) {
    ret = pform_make_task_ports(loc, pt, IVL_VT_REAL, true, 0, names);
  }

  if (dynamic_cast<string_type_t*>(vtype)) {
    ret = pform_make_task_ports(loc, pt, IVL_VT_STRING, false, 0, names);
  }
 ...
 */
PAssign* pform_compressed_assign_from_inc_dec(const struct vlltype& loc,
                                              PExpr* exp) {
...
PExpr* pform_genvar_inc_dec(const struct vlltype& loc, const char* name,
                            bool inc_flag) {
  if (!gn_system_verilog()) {
    cerr << loc
         << ": error: Increment/decrement operators "
            "require SystemVerilog."
         << endl;
    error_count += 1;
  }
 ...
 */
void pform_set_type_attrib(perm_string name, const string& key, char* value) {
...
void pform_set_reg_idx(perm_string name, list<pform_range_t>* indices) {
  ...
  if (indices && !indices->empty()) cur->set_unpacked_idx(*indices);
}
...
void pform_set_parameter(const struct vlltype& loc, perm_string name,
                         ivl_variable_type_t type, bool signed_flag,
                         list<pform_range_t>* range, PExpr* expr,
                         LexicalScope::range_t* value_range) {
  LexicalScope* scope = lexical_scope;
  if (is_compilation_unit(scope) && !gn_system_verilog()) {
    VLerror(loc,
            "error: parameter declarations must be contained within a module.");
    return;
  }
...
void pform_set_localparam(const struct vlltype& loc, perm_string name,
                          ivl_variable_type_t type, bool signed_flag,
                          list<pform_range_t>* range, PExpr* expr) {
  LexicalScope* scope = lexical_scope;
  if (is_compilation_unit(scope) && !gn_system_verilog()) {
    VLerror(
        loc,
        "error: localparam declarations must be contained within a module.");
    return;
  }
...
void pform_set_param_from_type(const struct vlltype& loc,
                               const data_type_t* data_type, const char* name,
                               list<pform_range_t>*& param_range,
                               bool& param_signed,
                               ivl_variable_type_t& param_type) {
  if (const vector_type_t* vec =
          dynamic_cast<const vector_type_t*>(data_type)) {
  ...
  cerr << loc.get_fileline()
       << ": sorry: cannot currently create a "
          "parameter of type '"
       << name << "' which was defined at: " << data_type->get_fileline() << "."
       << endl;
  error_count += 1;
}
/*
...
static void pform_set_port_type(perm_string name, NetNet::PortType pt,
  ...
  switch (cur->get_port_type()) {
    case NetNet::PIMPLICIT:
      if (!cur->set_port_type(pt)) VLerror("error setting port direction.");
      break;

    case NetNet::NOT_A_PORT:
      cerr << file << ":" << lineno << ": error: "
           << "port " << name << " is not in the port list." << endl;
      error_count += 1;
      break;

    default:
      cerr << file << ":" << lineno << ": error: "
           << "port " << name << " already has a port declaration." << endl;
      error_count += 1;
      break;
  }
}

void pform_set_port_type(const struct vlltype& li, list<pform_port_t>* ports,
                         NetNet::PortType pt, data_type_t* dt,
                         list<named_pexpr_t>* attr) {
  assert(pt != NetNet::PIMPLICIT && pt != NetNet::NOT_A_PORT);
  ...
  if (have_init_expr) {
    cerr << li.text << ":" << li.first_line << ": error: "
         << "Incomplete port declarations cannot be initialized." << endl;
    error_count += 1;
  }
...
static void pform_set_integer_2atom(const struct vlltype& li, uint64_t width,
                                    bool signed_flag, perm_string name,
                                    NetNet::Type net_type,
                                    list<named_pexpr_t>* attr) {
  PWire* cur = pform_get_make_wire_in_scope(li, name, net_type,
                                            NetNet::NOT_A_PORT, IVL_VT_BOOL);
  assert(cur);
...
template <class T>
static void pform_set2_data_type(const struct vlltype& li, T* data_type,
  ...
  PWire* net = pform_get_make_wire_in_scope(li, name, net_type,
                                            NetNet::NOT_A_PORT, base_type);
  assert(net);
...
static void pform_set_enum(const struct vlltype& li, enum_type_t* enum_type,
                           perm_string name, NetNet::Type net_type,
                           std::list<named_pexpr_t>* attr) {
  PWire* cur = pform_get_make_wire_in_scope(
      li, name, net_type, NetNet::NOT_A_PORT, enum_type->base_type);
  assert(cur);
...
static void pform_set_enum(const struct vlltype& li, enum_type_t* enum_type,
  ...
  // IVL_VT_BOOL.
  assert(enum_type->base_type == IVL_VT_LOGIC ||
  ...
  if (enum_type->name.nil()) pform_put_enum_type_in_scope(enum_type);
...
void pform_set_data_type(const struct vlltype& li, data_type_t* data_type,
  ...
  if (uarray_type) data_type = uarray_type->base_type;

  if (atom2_type_t* atom2_type = dynamic_cast<atom2_type_t*>(data_type)) {
    pform_set_integer_2atom(li, atom2_type->type_code, atom2_type->signed_flag,
                            names, net_type, attr);
  }

  else if (struct_type_t* struct_type =
               dynamic_cast<struct_type_t*>(data_type)) {
    ...
    pform_set_net_range(names, vec_type->pdims.get(), vec_type->signed_flag,
  ...
  else if (parray_type_t* array_type =
               dynamic_cast<parray_type_t*>(data_type)) {
    pform_set2_data_type(li, array_type, names, net_type, attr);
  }

  else if (string_type_t* string_type =
               dynamic_cast<string_type_t*>(data_type)) {
...
vector<PWire*>* pform_make_udp_input_ports(list<perm_string>* names) {
  ...
  for (list<perm_string>::iterator cur = names->begin(); cur != names->end();
       ++cur) {
    perm_string txt = *cur;
    PWire* pp = new PWire(txt, NetNet::IMPLICIT, NetNet::PINPUT, IVL_VT_LOGIC);
    (*out)[idx] = pp;
    idx += 1;
  }
...
void pform_add_modport_port(const struct vlltype& loc,
                            NetNet::PortType port_type, perm_string name,
                            PExpr* expr) {
  ivl_assert(loc, pform_cur_modport);

  if (pform_cur_modport->simple_ports.find(name) !=
      pform_cur_modport->simple_ports.end()) {
    cerr << loc << ": error: duplicate declaration of port '" << name
         << "' in modport list '" << pform_cur_modport->name() << "'." << endl;
    error_count += 1;
  }
...
int pform_parse(const char* path) {
  ...
  if (strcmp(path, "-") == 0) {
    ...
    char* cmdline = (char*)malloc(strlen(ivlpp_string) + strlen(path) + 4);
    strcpy(cmdline, ivlpp_string);
    strcat(cmdline, " \"");
    strcat(cmdline, path);
    strcat(cmdline, "\"");

    if (verbose_flag) cerr << "Executing: " << cmdline << endl << flush;
