...
bool load_module(const char *type, int &parser_errors) {
  ...
  for (char *tmp = ltype; *tmp; tmp += 1) *tmp = tolower(*tmp);

  for (struct module_library *lcur = library_list; lcur != 0;
       lcur = lcur->next) {
    ...
    if (verbose_flag) cerr << "Loading library file " << path << "." << endl;

    parser_errors = pform_parse(path);

    if (verbose_flag) cerr << "... Load module complete." << endl << flush;
...
int build_library_index(const char *path, bool key_case_sensitive) {
  ...
  while (struct dirent *de = readdir(dir)) {
    ...
    for (list<const char *>::iterator suf = library_suff.begin();
      ...
      if (sufsiz >= namsiz) continue;

      /* If the directory is case insensitive, then so
         is the suffix. */
      if (key_case_sensitive) {
        if (strcmp(de->d_name + (namsiz - sufsiz), sufptr) != 0) continue;
      } else {
        if (strcasecmp(de->d_name + (namsiz - sufsiz), sufptr) != 0) continue;
      }
    ...
    /* If the key is not to be case sensitive, then change
       it to lowercase. */
    if (!key_case_sensitive)
      for (char *tmp = key; *tmp; tmp += 1) *tmp = tolower(*tmp);
