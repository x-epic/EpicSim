...
extern "C" ivl_discipline_t ivl_design_discipline(ivl_design_t des,
                                                  unsigned idx) {
  assert(des);
...
extern "C" ivl_expr_t ivl_const_delay(ivl_net_const_t net,
                                      unsigned transition) {
  assert(net);
  assert(transition < 3);
  ...
  switch (net->type_) {
    ...
    case IVL_EX_NUMBER: {
      ...
      for (unsigned long idx = 0; idx < net->width_; idx += 1) {
        if (net->u_.number_.bits_[idx] == '1') val |= 1UL << idx;
      }
...
extern "C" const char* ivl_logic_attr(ivl_net_logic_t net, const char* key) {
  ...
  for (idx = 0; idx < net->nattr; idx += 1) {
    if (strcmp(net->attr[idx].key, key) == 0)
      return net->attr[idx].type == IVL_ATT_STR ? net->attr[idx].val.str : 0;
  }
...
extern "C" ivl_drive_t ivl_logic_drive0(ivl_net_logic_t net) {
  ...
  for (unsigned idx = 0; idx < ivl_nexus_ptrs(nex); idx += 1) {
    ...
    if (ivl_nexus_ptr_log(cur) != net) continue;
    if (ivl_nexus_ptr_pin(cur) != 0) continue;
...
extern "C" ivl_expr_t ivl_logic_delay(ivl_net_logic_t net,
                                      unsigned transition) {
  assert(net);
  assert(transition < 3);
  return net->delay[transition];
...
extern "C" ivl_drive_t ivl_lpm_drive0(ivl_lpm_t net) {
  ...
  for (unsigned idx = 0; idx < ivl_nexus_ptrs(nex); idx += 1) {
    ivl_nexus_ptr_t cur = ivl_nexus_ptr(nex, idx);
    if (ivl_nexus_ptr_lpm(cur) != net) continue;
    if (ivl_nexus_ptr_pin(cur) != 0) continue;
    return ivl_nexus_ptr_drive0(cur);
  }
  ...
  switch (net->type) {
    ...
    case IVL_LPM_CONCATZ:
      cerr << "error: ivl_lpm_selects() is no longer supported for "
              "IVL_LPM_CONCAT, use ivl_lpm_size() instead."
           << endl;
      // fallthrough
    default:
      assert(0);
 ...
 * Deprecated
 */
extern "C" ivl_expr_t ivl_lval_mux(ivl_lval_t) { return 0; }

extern "C" ivl_expr_t ivl_lval_idx(ivl_lval_t net) {
  assert(net);

  if (net->type_ == IVL_LVAL_ARR) return net->idx;
  return 0x0;
}
...
extern "C" ivl_lval_t ivl_lval_nest(ivl_lval_t net) {
  assert(net);
  if (net->type_ == IVL_LVAL_LVAL) return net->n.nest;
...
extern "C" unsigned ivl_parameter_width(ivl_parameter_t net) {
  ...
  assert(net);
  if (net->msb >= net->lsb)
    result += net->msb - net->lsb;
  else
    result += net->lsb - net->msb;
  return result;
}
...
extern "C" ivl_attribute_t ivl_scope_attr_val(ivl_scope_t net, unsigned idx) {
  assert(net);
  assert(idx < net->nattr);
  return net->attr + idx;
}
...
extern "C" int ivl_scope_children(ivl_scope_t net, ivl_scope_f func, void* cd) {
...
extern "C" const char* ivl_scope_name(ivl_scope_t net) {
  ...
  if (net->parent == 0) return net->name_;
...
extern "C" const char* ivl_scope_mod_module_port_name(ivl_scope_t net,
                                                      unsigned idx) {
  assert(net);
  assert(net->type_ == IVL_SCT_MODULE);
...
extern "C" ivl_signal_port_t ivl_scope_mod_module_port_type(ivl_scope_t net,
                                                            unsigned idx) {
  assert(net);
...
extern "C" unsigned ivl_scope_mod_module_port_width(ivl_scope_t net,
                                                    unsigned idx) {
  assert(net);
...
extern "C" unsigned ivl_scope_ports(ivl_scope_t net) {
  assert(net);
  if (net->type_ == IVL_SCT_MODULE || net->type_ == IVL_SCT_FUNCTION ||
...
extern "C" ivl_signal_t ivl_scope_port(ivl_scope_t net, unsigned idx) {
  assert(net);
  assert(net->type_ == IVL_SCT_FUNCTION || net->type_ == IVL_SCT_TASK);
...
extern "C" int ivl_signal_msb(ivl_signal_t net) {
  assert(net);
  if (net->packed_dims.empty()) return 0;
...
extern "C" ivl_statement_t ivl_stmt_case_stmt(ivl_statement_t net,
                                              unsigned idx) {
  assert(net);
  switch (net->type_) {
    case IVL_ST_CASE:
...
extern "C" unsigned ivl_stmt_lwidth(ivl_statement_t net) {
  assert(net);
  assert((net->type_ == IVL_ST_ASSIGN) || (net->type_ == IVL_ST_ASSIGN_NB) ||
         (net->type_ == IVL_ST_CASSIGN) || (net->type_ == IVL_ST_DEASSIGN) ||
         (net->type_ == IVL_ST_FORCE) || (net->type_ == IVL_ST_RELEASE));
