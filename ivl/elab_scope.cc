...
typedef map<perm_string, LexicalScope::param_expr_t *>::const_iterator
    mparm_it_t;
...
static void collect_parm_item_(Design *des, NetScope *scope, perm_string name,
  ...
  for (LexicalScope::range_t *range = cur.range; range; range = range->next) {
    ...
    if (range->high_expr && range->high_expr == range->low_expr) {
      ...
      // in this case
      ivl_assert(*range->high_expr,
                 tmp->low_open_flag == false && tmp->high_open_flag == false);
...
static void collect_scope_parameters_(
    Design *des, NetScope *scope,
    const map<perm_string, LexicalScope::param_expr_t *> &parameters) {
  for (mparm_it_t cur = parameters.begin(); cur != parameters.end(); ++cur) {
    collect_parm_item_(des, scope, cur->first, *(cur->second), false, false);
  }
}

static void collect_scope_localparams_(
    Design *des, NetScope *scope,
    const map<perm_string, LexicalScope::param_expr_t *> &localparams) {
  for (mparm_it_t cur = localparams.begin(); cur != localparams.end(); ++cur) {
...
static void collect_scope_specparams_(
  ...
  for (mparm_it_t cur = specparams.begin(); cur != specparams.end(); ++cur) {
...
static void elaborate_scope_enumeration(Design *des, NetScope *scope,
  ...
  netenum_t *use_enum = new netenum_t(
      enum_type->base_type, enum_type->signed_flag, enum_type->integer_flag,
      msb, lsb, enum_type->names->size(), enum_type);
  ...
  for (list<named_pexpr_t>::const_iterator cur = enum_type->names->begin();
    ...
    if (cur->parm) {
      ...
      if (val_const == 0) {
        cerr << use_enum->get_fileline()
             << ": error: Enumeration expression for " << cur->name
             << " is not an integer constant." << endl;
        des->errors += 1;
        continue;
      }
      ...
      if (enum_type->base_type == IVL_VT_BOOL && !cur_value.is_defined()) {
        cerr << use_enum->get_fileline() << ": error: Enumeration name "
             << cur->name << " can not have an undefined value." << endl;
        des->errors += 1;
      }
      ...
      if (PENumber *tmp = dynamic_cast<PENumber *>(cur->parm)) {
        if (tmp->value().has_len() && (tmp->value().len() != enum_width)) {
          cerr << use_enum->get_fileline() << ": error: Enumeration name "
               << cur->name << " has an incorrectly sized constant." << endl;
          des->errors += 1;
        }
      }
      ...
      if (((cur_value.len() != enum_width) || !cur_value.has_len()) &&
          !enum_type->signed_flag && cur_value.is_negative()) {
        cerr << use_enum->get_fileline() << ": error: Enumeration name "
             << cur->name << " has a negative value." << endl;
        des->errors += 1;
      }
      ...
      if (cur_value.len() > enum_width) {
        ...
        for (unsigned idx = enum_width; idx < cur_value.len(); idx += 1) {
          if (cur_value[idx] != cur_value[check_width]) {
            ...
            if (cur_value.is_defined()) {
              cerr << use_enum->get_fileline() << ": error: Enumeration name "
                   << cur->name << " has a value that is too "
                   << ((cur_value > max_value) ? "large" : "small") << " "
                   << cur_value << "." << endl;
            } else {
              cerr << use_enum->get_fileline() << ": error: Enumeration name "
                   << cur->name << " has trimmed bits that do "
        ...
        if (!cur_value.has_sign() && (cur_value[enum_width] == verinum::V1)) {
          cerr << use_enum->get_fileline() << ": error: Enumeration name "
               << cur->name << " has a value that is too large: " << cur_value
               << "." << endl;
          des->errors += 1;
          break;
        }
      ...
      cerr << use_enum->get_fileline() << ": error: Enumeration name "
           << cur->name << " has an undefined inferred value." << endl;
      des->errors += 1;
      continue;
    }
    ...
    if (implicit_wrapped) {
      cerr << use_enum->get_fileline() << ": error: Enumeration name "
           << cur->name << " has an inferred value that overflowed." << endl;
      des->errors += 1;
    }
...
static void blend_class_constructors(PClass *pclass) {
  ...
  if (use_new == 0 && use_new2 == 0) return;
  ...
  PChainConstructor *chain_new =
      use_new ? use_new->extract_chain_constructor() : 0;
...
static void elaborate_scope_class(Design *des, NetScope *scope,
  ...
  if (debug_scopes) {
    cerr << pclass->get_fileline() << ": elaborate_scope_class: "
         ...
         << " within scope " << scope_path(scope) << endl;
  }
  ...
  if (use_type->base_type && !base_class) {
    cerr << pclass->get_fileline() << ": error: "
         << "Base type of " << use_type->name << " is not a class." << endl;
    des->errors += 1;
  }
  ...
  if (base_class) {
    ...
    if (use_base_class == 0) {
      cerr << pclass->get_fileline() << ": error: "
           << "Base class " << base_class->name << " not found." << endl;
      des->errors += 1;
    }
  }
  ...
  for (map<perm_string, class_type_t::prop_info_t>::iterator cur =
           use_type->properties.begin();
       cur != use_type->properties.end(); ++cur) {
    ivl_type_s *tmp = cur->second.type->elaborate_type(des, class_scope);
    ivl_assert(*pclass, tmp);
    if (debug_scopes) {
      cerr << pclass->get_fileline() << ": elaborate_scope_class: "
           << "  Property " << cur->first << " type=" << *tmp << endl;
    }
  ...
  for (map<perm_string, PTask *>::iterator cur = pclass->tasks.begin();
    ...
    NetScope *method_scope =
        new NetScope(class_scope, use_name, NetScope::TASK);
    ...
    if (debug_scopes) {
      cerr << cur->second->get_fileline() << ": elaborate_scope_class: "
           << "Elaborate method (task) scope " << scope_path(method_scope)
           << endl;
    }
  ...
  for (map<perm_string, PFunction *>::iterator cur = pclass->funcs.begin();
    ...
    hname_t use_name(cur->first);
    NetScope *method_scope =
        new NetScope(class_scope, use_name, NetScope::FUNC);
    ...
    if (debug_scopes) {
      cerr << cur->second->get_fileline() << ": elaborate_scope_class: "
           << "Elaborate method (function) scope " << scope_path(method_scope)
           << endl;
    }
  ...
  for (Module::replace_t::const_iterator cur = replacements.begin();
    ...
    if (val == 0) {
      cerr << loc.get_fileline() << ": internal error: "
           << "Missing expression in parameter replacement for " << (*cur).first
           << endl;
      ;
    }
    assert(val);
    if (debug_scopes) {
      cerr << loc.get_fileline() << ": debug: "
           << "Replace " << (*cur).first << " with expression " << *val
           << " from " << val->get_fileline() << "." << endl;
      cerr << loc.get_fileline() << ":      : "
           << "Type=" << val->expr_type() << endl;
    }
    bool flag = scope->replace_parameter((*cur).first, val, scope->parent());
    if (!flag) {
      cerr << val->get_fileline() << ": warning: parameter " << (*cur).first
           << " not found in " << scope_path(scope) << "." << endl;
    }
  }
}
...
static void elaborate_scope_tasks(Design *des, NetScope *scope,
  ...
  for (tasks_it_t cur = tasks.begin(); cur != tasks.end(); ++cur) {
    elaborate_scope_task(des, scope, cur->second);
  }
}

static void elaborate_scope_func(Design *des, NetScope *scope,
                                 PFunction *task) {
  hname_t use_name(task->pscope_name());
...
static void elaborate_scope_funcs(Design *des, NetScope *scope,
  ...
  for (funcs_it_t cur = funcs.begin(); cur != funcs.end(); ++cur) {
  ...
  void elaborate_runrun() {
    if (debug_scopes)
      cerr << mod_->get_fileline() << ": debug: "
           << "Processing generate schemes for " << scope_path(scope_) << endl;
...
bool PPackage::elaborate_scope(Design *des, NetScope *scope) {
  ...
  if (debug_scopes) {
    cerr << get_fileline() << ": PPackage::elaborate_scope: "
         ...
         << " in package scope " << scope_path(scope) << "." << endl;
  }
...
bool Module::elaborate_scope(Design *des, NetScope *scope,
  ...
  for (defparms_iter_t cur = defparms.begin(); cur != defparms.end(); ++cur) {
  ...
  des->elaboration_work_list.push_back(
      new generate_schemes_work_item_t(des, scope, this));
  ...
  for (map<perm_string, Module *>::iterator cur = nested_modules.begin();
    ...
    if (cur->second->port_count() > 0) continue;
  ...
  for (gates_it_t cur = gates_.begin(); cur != gates_.end(); ++cur) {
...
bool PGenerate::generate_scope_loop_(Design *des, NetScope *container) {
  ...
  while (test->value().as_long()) {
    ...
    // genvar as a constant.
    {
      ...
      NetEConstParam *gp =
          new NetEConstParam(scope, loop_index, genvar_verinum);
      ...
      if (debug_scopes)
        cerr << get_fileline() << ": debug: "
             << "Create implicit localparam " << loop_index << " = "
             << genvar_verinum << endl;
    }
    ...
    if (debug_scopes)
      cerr << get_fileline() << ": debug: genvar step from " << genvar << " to "
           << step->value().as_long() << endl;
...
bool PGenerate::generate_scope_condit_(Design *des, NetScope *container,
                                       bool else_flag) {
  NetExpr *test_ex = elab_and_eval(des, container, loop_test, -1, true);
  NetEConst *test = dynamic_cast<NetEConst *>(test_ex);
  if (test == 0) {
  ...
  if ((test->value().as_long() == 0 && !else_flag) ||
    ...
    if (debug_scopes)
      cerr << get_fileline() << ": debug: Generate condition "
           << (else_flag ? "(else)" : "(if)") << " value=" << test->value()
           << ": skip generation" << endl;
    delete test_ex;
    return true;
  }
  ...
  if (direct_nested_) {
    if (debug_scopes)
      cerr << get_fileline() << ": debug: Generate condition "
           << (else_flag ? "(else)" : "(if)") << " detected direct nesting."
           << endl;
...
bool PGenerate::generate_scope_case_(Design *des, NetScope *container) {
  ...
  while (cur != generate_schemes.end()) {
    ...
    for (unsigned idx = 0; idx < item->item_test.size() && !match_flag;
         idx += 1) {
      NetExpr *item_value_ex =
          elab_and_eval(des, container, item->item_test[idx], -1, true);
      NetEConst *item_value_co = dynamic_cast<NetEConst *>(item_value_ex);
      if (item_value_co == 0) {
        cerr << get_fileline() << ": error: Cannot evaluate "
             << " genvar case item expression: " << *item->item_test[idx]
             << endl;
        des->errors += 1;
        return false;
      }
      ...
      if (case_value_co->value() == item_value_co->value()) match_flag = true;
  ...
  if (debug_scopes)
    cerr << get_fileline() << ": debug: "
         << "Generate case matches item at " << item->get_fileline() << endl;
  ...
  if (debug_scopes) {
    cerr << get_fileline() << ": PGenerate::generate_scope_case_: "
         << "Generate subscope " << use_name << " and elaborate." << endl;
  }
...
void PGenerate::elaborate_subscope_(Design *des, NetScope *scope) {
  ...
  for (pgate_list_it_t cur = gates.begin(); cur != gates.end(); ++cur) {
    (*cur)->elaborate_scope(des, scope);
  }
...
class delayed_elaborate_scope_mod_instances : public elaborator_work_item_t {
 public:
  delayed_elaborate_scope_mod_instances(Design *des__, const PGModule *obj,
                                        Module *mod, NetScope *sc)
...
void delayed_elaborate_scope_mod_instances::elaborate_runrun() {
  if (debug_scopes)
    cerr << obj_->get_fileline() << ": debug: "
         << "Resume scope elaboration of instances of " << mod_->mod_name()
         << "." << endl;
 ...
 */
void PGModule::elaborate_scope_mod_(Design *des, Module *mod,
                                    NetScope *sc) const {
  if (get_name() == "") {
  ...
  for (NetScope *scn = sc; scn; scn = scn->parent()) {
    ...
    if (in_genblk) {
      ...
      if (rl_count > recursive_mod_limit) {
        cerr << get_fileline() << ": error: instance " << scope_path(sc) << "."
             << get_name() << " of module " << mod->mod_name()
             << " is nested too deep." << endl;
        cerr << get_fileline()
             << ":      : check for "
                "proper recursion termination or increase the "
                "limit ("
             << recursive_mod_limit << ") with the -pRECURSIVE_MOD_LIMIT flag."
             << endl;
        des->errors += 1;
        return;
      }
  ...
  if (msb_ || lsb_) {
    ...
    if (debug_scopes)
      cerr << get_fileline() << ": debug: delay elaborate_scope"
           << " of array of " << get_name() << " in scope " << scope_path(sc)
           << "." << endl;
 ...
 */
void PGModule::elaborate_scope_mod_instances_(Design *des, Module *mod,
                                              NetScope *sc) const {
  ...
  for (int idx = 0; idx < instance_count; idx += 1) {
    ...
    if (debug_scopes) {
      cerr << get_fileline() << ": debug: Module instance " << use_name
           << " becomes child of " << scope_path(sc) << "." << endl;
    }
    ...
    NetScope *my_scope = new NetScope(sc, use_name, NetScope::MODULE, 0,
                                      ...
                                      mod->program_block, mod->is_interface);
    my_scope->set_line(get_file(), mod->get_file(), get_lineno(),
                       mod->get_lineno());
    ...
    if (overrides_) {
      ...
      for (;;) {
        if (jdx == overrides_->end()) break;
        if (cur == mod->param_names.end()) break;

        // No expression means that the parameter is not
        // replaced at all.
        if (*jdx) replace[*cur] = *jdx;
    ...
    if (parms_) {
      ...
      for (unsigned jdx = 0; jdx < nparms_; jdx += 1) {
        // No expression means that the parameter is not
        // replaced.
        if (parms_[jdx].parm) replace[parms_[jdx].name] = parms_[jdx].parm;
      }
    }
...
void PFunction::elaborate_scope(Design *des, NetScope *scope) const {
  ...
  if (statement_) statement_->elaborate_scope(des, scope);
}
...
void PBlock::elaborate_scope(Design *des, NetScope *scope) const {
  ...
  if (pscope_name() != 0) {
    ...
    if (debug_scopes)
      cerr << get_fileline() << ": debug: "
           << "Elaborate block scope " << use_name << " within "
           << scope_path(scope) << endl;

    // The scope type is begin-end or fork-join. The
    // sub-types of fork-join are not interesting to the scope.
    my_scope = new NetScope(
        scope, use_name,
        bl_type_ != BL_SEQ ? NetScope::FORK_JOIN : NetScope::BEGIN_END);
...
void PCase::elaborate_scope(Design *des, NetScope *scope) const {
  ...
  for (unsigned idx = 0; idx < (*items_).count(); idx += 1) {
    ...
    if (Statement *sp = (*items_)[idx]->stat) sp->elaborate_scope(des, scope);
  }
}
...
void PCondit::elaborate_scope(Design *des, NetScope *scope) const {
  if (if_) if_->elaborate_scope(des, scope);

  if (else_) else_->elaborate_scope(des, scope);
}
