...
static NetNet* convert_to_real_const(Design* des, NetScope* scope,
                                     NetEConst* expr) {
...
static bool process_binary_args(Design* des, NetScope* scope, NetExpr* root,
                                NetExpr* left, NetExpr* right, NetNet*& lsig,
                                NetNet*& rsig, bool& real_args) {
  if (left->expr_type() == IVL_VT_REAL || right->expr_type() == IVL_VT_REAL) {
    real_args = true;
...
NetNet* NetExpr::synthesize(Design* des, NetScope*, NetExpr*) {
  cerr << get_fileline()
       << ": internal error: cannot synthesize expression: " << *this << endl;
  des->errors += 1;
  return 0;
}
...
NetNet* NetEBAdd::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  if (expr_type() == IVL_VT_REAL) {
    width = 1;
    if (lsig->data_type() != IVL_VT_REAL) lsig = cast_to_real(des, scope, lsig);
    if (rsig->data_type() != IVL_VT_REAL) rsig = cast_to_real(des, scope, rsig);
...
NetNet* NetEBBits::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  netvector_t* osig_vec = new netvector_t(expr_type(), width - 1, 0);
  NetNet* osig =
      new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, osig_vec);
  osig->set_line(*this);
  osig->local_flag(true);
...
NetNet* NetEBComp::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": debug: Comparison (" << op_ << ")"
         << " is " << (signed_compare ? "signed" : "unsigned") << endl;
    cerr << get_fileline() << ":      : lsig is "
         << (lsig->get_signed() ? "signed" : "unsigned") << " rsig is "
         << (rsig->get_signed() ? "signed" : "unsigned") << endl;
  }

  if (op_ == 'E' || op_ == 'N') {
    NetCaseCmp* gate =
        new NetCaseCmp(scope, scope->local_symbol(), width,
                       op_ == 'E' ? NetCaseCmp::EEQ : NetCaseCmp::NEQ);
    gate->set_line(*this);
  ...
  if (op_ == 'w' || op_ == 'W') {
    NetCaseCmp* gate =
        new NetCaseCmp(scope, scope->local_symbol(), width,
                       op_ == 'w' ? NetCaseCmp::WEQ : NetCaseCmp::WNE);
    gate->set_line(*this);
    connect(gate->pin(0), osig->pin(0));
    connect(gate->pin(1), lsig->pin(0));
    connect(gate->pin(2), rsig->pin(0));
  ...
  if ((width == 1) && (op_ == 'e') && !real_args) {
    NetLogic* gate =
        new NetLogic(scope, scope->local_symbol(), 3, NetLogic::XNOR, 1, true);
    gate->set_line(*this);
    connect(gate->pin(0), osig->pin(0));
    connect(gate->pin(1), lsig->pin(0));
    connect(gate->pin(2), rsig->pin(0));
  ...
  if ((width == 1) && (op_ == 'n') && !real_args) {
    NetLogic* gate =
        new NetLogic(scope, scope->local_symbol(), 3, NetLogic::XOR, 1, true);
    gate->set_line(*this);
    connect(gate->pin(0), osig->pin(0));
    connect(gate->pin(1), lsig->pin(0));
    connect(gate->pin(2), rsig->pin(0));
  ...
  switch (op_) {
    ...
    case 'E':  // === ?
      if (real_args) {
        cerr << get_fileline()
             << ": error: Case equality may "
                "not have real operands."
             << endl;
        des->errors += 1;
        return 0;
      }
    ...
    default:
      cerr << get_fileline()
           << ": internal error: cannot synthesize "
              "comparison: "
           << *this << endl;
      des->errors += 1;
      return 0;
...
NetNet* NetEBPow::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  NetPow* powr = new NetPow(scope, scope->local_symbol(), width,
                            lsig->vector_width(), rsig->vector_width());
  powr->set_line(*this);
  des->add_node(powr);
...
NetNet* NetEBMult::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  NetMult* mult = new NetMult(scope, scope->local_symbol(), width,
                              lsig->vector_width(), rsig->vector_width());
  mult->set_line(*this);
  des->add_node(mult);
...
NetNet* NetEBDiv::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  switch (op()) {
    case '/': {
      NetDivide* div =
          new NetDivide(scope, scope->local_symbol(), width,
                        lsig->vector_width(), rsig->vector_width());
      div->set_line(*this);
    ...
    case '%': {
      ...
      NetModulo* div =
          new NetModulo(scope, scope->local_symbol(), width,
                        lsig->vector_width(), rsig->vector_width());
      div->set_line(*this);
      div->set_signed(has_sign());
      des->add_node(div);
    ...
    default: {
      cerr << get_fileline() << ": internal error: "
           << "NetEBDiv has unexpected op() code: " << op() << endl;
      des->errors += 1;
...
NetNet* NetEBLogic::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  if (lsig->data_type() == IVL_VT_REAL || rsig->data_type() == IVL_VT_REAL) {
    cerr << get_fileline() << ": internal error: " << human_readable_op(op_)
  ...
  NetNet* osig =
      new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, osig_tmp);
  osig->set_line(*this);
  osig->local_flag(true);
...
NetNet* NetEBShift::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  if (lsig->data_type() == IVL_VT_REAL) {
    cerr << get_fileline() << ": error: shift operator ("
         << human_readable_op(op_) << ") cannot shift a real values." << endl;
    des->errors += 1;
    return 0;
  }
  ...
  if (NetEConst* rcon = dynamic_cast<NetEConst*>(right_)) {
    ...
    if (right_flag) shift = 0 - shift;

    if (shift == 0) return lsig;
    ...
    NetPartSelect* psel =
        new NetPartSelect(lsig, shift < 0 ? ushift : 0, part_width,
                          NetPartSelect::VP, signed_flag && right_flag);
    psel->set_line(*this);
    des->add_node(psel);

    netvector_t* psig_vec = new netvector_t(expr_type(), part_width - 1, 0);
    NetNet* psig =
        new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, psig_vec);
    psig->set_line(*this);
    ...
    if (signed_flag && right_flag) {
      NetSignExtend* pad =
          new NetSignExtend(scope, scope->local_symbol(), osig->vector_width());
      pad->set_line(*this);
      des->add_node(pad);
    ...
    NetConst* zcon = new NetConst(scope, scope->local_symbol(), znum);
    des->add_node(zcon);

    netvector_t* zsig_vec =
        new netvector_t(osig->data_type(), znum.len() - 1, 0);
    NetNet* zsig =
        new NetNet(scope, scope->local_symbol(), NetNet::WIRE, zsig_vec);
    zsig->set_line(*this);
    zsig->local_flag(true);
    connect(zcon->pin(0), zsig->pin(0));

    NetConcat* ccat =
        new NetConcat(scope, scope->local_symbol(), osig->vector_width(), 2);
    ccat->set_line(*this);
    des->add_node(ccat);
  ...
  NetCLShift* dev =
      new NetCLShift(scope, scope->local_symbol(), osig->vector_width(),
                     rsig->vector_width(), right_flag, signed_flag);
  dev->set_line(*this);
  des->add_node(dev);
...
NetNet* NetEConcat::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    if (parms_[idx]->expr_width() == 0) {
      ...
      switch (data_type) {
        case IVL_VT_NO_TYPE:
          ...
          break;
        case IVL_VT_BOOL:
          if (tmp[idx]->data_type() == IVL_VT_LOGIC) data_type = IVL_VT_LOGIC;
          break;
        default:
          break;
      }
  ...
  NetConcat* concat = new NetConcat(scope, scope->local_symbol(),
                                    osig->vector_width(), num_parms * repeat());
  ...
  if (count_input_width != osig->vector_width()) {
    cerr << get_fileline() << ": internal error: "
         << "NetEConcat input width = " << count_input_width << ", expecting "
         << osig->vector_width() << " (repeat=" << repeat() << ")" << endl;
    des->errors += 1;
  }
...
NetNet* NetEUBits::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  if (isig->data_type() == IVL_VT_REAL) {
    cerr << get_fileline() << ": error: bit-wise negation ("
         << human_readable_op(op_) << ") may not have a REAL operand." << endl;
    des->errors += 1;
    return 0;
  }
...
NetNet* NetEUnary::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  if (op_ == '+') return expr_->synthesize(des, scope, root);
  ...
  if (op_ == 'm') {
    ...
    if (expr_->has_sign() == false) return sub;

    netvector_t* sig_vec =
        new netvector_t(sub->data_type(), sub->vector_width() - 1, 0);
    NetNet* sig =
        new NetNet(scope, scope->local_symbol(), NetNet::WIRE, sig_vec);
    sig->set_line(*this);
    sig->local_flag(true);
...
NetNet* NetEUReduce::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  NetUReduce* gate =
      new NetUReduce(scope, scope->local_symbol(), rtype, isig->vector_width());
  gate->set_line(*this);
  des->add_node(gate);
...
NetNet* NetESelect::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  if (NetEConst* base_const = dynamic_cast<NetEConst*>(base_)) {
    ...
    NetPartSelect* sel =
        new NetPartSelect(sub, base_val, select_width, NetPartSelect::VP);
    des->add_node(sel);

    ivl_assert(*this, select_width > 0);
    netvector_t* tmp_vec =
        new netvector_t(sub->data_type(), select_width - 1, 0);
    NetNet* tmp =
    ...
    if (above) concat_count += 1;
    if (below) concat_count += 1;
    if (concat_count > 1) {
      NetConcat* cat = new NetConcat(scope, scope->local_symbol(), expr_width(),
                                     concat_count);
      cat->set_line(*this);
      des->add_node(cat);
      if (below) {
      ...
      tmp = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp_vec);
      tmp->set_line(*this);
      tmp->local_flag(true);
  ...
  if (base_ != 0) {
    ...
    NetPartSelect* sel =
        new NetPartSelect(sub, off, expr_width(), base_->has_sign());
    sel->set_line(*this);
    des->add_node(sel);

    netvector_t* tmp_vec =
        new netvector_t(sub->data_type(), expr_width() - 1, 0);
    NetNet* tmp =
        new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, tmp_vec);
    tmp->local_flag(true);
    tmp->set_line(*this);
    sub = tmp;
  ...
  // are done.
  if (sub->vector_width() == expr_width()) return sub;

  netvector_t* net_vec = new netvector_t(expr_type(), expr_width() - 1, 0);
  net_vec->set_signed(has_sign());
  NetNet* net =
      new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, net_vec);
  net->set_line(*this);
  net->local_flag(true);
  ...
  if (expr_width() < sub->vector_width()) {
    NetPartSelect* sel =
        new NetPartSelect(sub, 0, expr_width(), NetPartSelect::VP, has_sign());
    sel->set_line(*this);
    des->add_node(sel);
    ...
    NetSignExtend* pad =
        new NetSignExtend(scope, scope->local_symbol(), expr_width());
    pad->set_line(*this);
    des->add_node(pad);
    ...
    NetConcat* cat =
        new NetConcat(scope, scope->local_symbol(), expr_width(), 2);
    cat->set_line(*this);
    des->add_node(cat);
...
NetNet* NetETernary::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  if (!NetETernary::test_operand_compat(tsig->data_type(), fsig->data_type())) {
    ...
    cerr << get_fileline() << ":      : True  clause is: " << tsig->data_type()
         << " (" << true_val_->expr_type() << "): " << *true_val_ << endl;
    cerr << get_fileline() << ":      : False clause is: " << fsig->data_type()
         << " (" << false_val_->expr_type() << "): " << *false_val_ << endl;
    des->errors += 1;
    return 0;
  } else if (tsig->data_type() == IVL_VT_NO_TYPE) {
    cerr << get_fileline()
         << ": internal error: True and False "
            "clauses of ternary both have NO TYPE."
         << endl;
    des->errors += 1;
    return 0;
  }
  ...
  NetMux* mux =
      new NetMux(csig->scope(), oname, width, 2, csig->vector_width());
  mux->set_line(*this);
...
NetNet* NetESignal::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  if (net_->scope() == scope && net_->name() == scope->genvar_tmp) {
    netvector_t* tmp_vec =
        new netvector_t(net_->data_type(), net_->vector_width() - 1, 0);
    NetNet* tmp =
        new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, tmp_vec);
    tmp->set_line(*this);
    tmp->local_flag(true);
  ...
  if (NetEConst* index_co = dynamic_cast<NetEConst*>(word_)) {
    ...
    NetArrayDq* mux =
        new NetArrayDq(scope, scope->local_symbol(), net_, selwid);
    mux->set_line(*this);
    des->add_node(mux);
...
static NetEvWait* make_func_trigger(Design* des, NetScope* scope,
                                    NetExpr* root) {
  NetEvWait* trigger = 0;
...
NetNet* NetESFunc::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  if (def == 0 || def->name == 0) {
    cerr << get_fileline() << ": error: System function " << name_
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": debug: Net system function " << name_
         << " returns " << def->type << endl;
  }
  ...
  NetSysFunc* net = new NetSysFunc(scope, scope->local_symbol(), def,
                                   1 + parms_.size(), trigger);
  net->set_line(*this);
  des->add_node(net);
  ...
  NetNet* osig =
      new NetNet(scope, scope->local_symbol(), NetNet::WIRE, osig_vec);
  osig->set_line(*this);
  osig->local_flag(true);
  ...
  for (unsigned idx = 0; idx < parms_.size(); idx += 1) {
    ...
    if (tmp == 0) {
      cerr << get_fileline() << ": error: Unable to elaborate "
           << "argument " << idx << " of call to " << name_ << "." << endl;
      errors += 1;
      des->errors += 1;
      continue;
    }
...
NetNet* NetEUFunc::synthesize(Design* des, NetScope* scope, NetExpr* root) {
  ...
  for (unsigned idx = 0; idx < eparms.size(); idx += 1) {
    ...
    if (tmp == 0) {
      cerr << get_fileline()
           << ": error: Unable to synthesize "
              "port "
           << idx << " of call to " << func_->basename() << "." << endl;
      errors = true;
      des->errors += 1;
      continue;
    }
  ...
  NetUserFunc* net =
      new NetUserFunc(scope_, scope_->local_symbol(), func_, trigger);
  net->set_line(*this);
  des->add_node(net);
  ...
  NetNet* osig =
      new NetNet(scope_, scope_->local_symbol(), NetNet::WIRE, osig_vec);
  osig->set_line(*this);
  osig->local_flag(true);
  connect(net->pin(0), osig->pin(0));
