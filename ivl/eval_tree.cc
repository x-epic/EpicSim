...
NetExpr* NetExpr::eval_tree() { return 0; }
...
NetExpr* NetEBAdd::eval_tree() {
  ...
  if (expr_type() == IVL_VT_REAL) return 0;
...
NetEConst* NetEBBits::eval_arguments_(const NetExpr* l,
                                      const NetExpr* r) const {
  const NetEConst* lc = dynamic_cast<const NetEConst*>(l);
  const NetEConst* rc = dynamic_cast<const NetEConst*>(r);
...
NetEConst* NetEBComp::must_be_leeq_(const NetExpr* le, const verinum& rv,
                                    bool eq_flag) const {
...
NetEConst* NetEBComp::eval_leeq_real_(const NetExpr* le, const NetExpr* re,
                                      bool eq_flag) const {
  double lval;
  double rval;
 ...
 */
NetEConst* NetEBComp::eval_eqeq_real_(bool ne_flag, const NetExpr* le,
  ...
  verinum result(((lval == rval) != ne_flag) ? verinum::V1 : verinum::V0, 1);
  NetEConst* res = new NetEConst(result);
  ivl_assert(*this, res);
...
NetEConst* NetEBComp::eval_eqeq_(bool ne_flag, const NetExpr* le,
                                 const NetExpr* re) const {
  if (le->expr_type() == IVL_VT_REAL || re->expr_type() == IVL_VT_REAL)
...
NetEConst* NetEBComp::eval_eqeqeq_(bool ne_flag, const NetExpr* le,
                                   const NetExpr* re) const {
  const NetEConst* lc = dynamic_cast<const NetEConst*>(le);
  const NetEConst* rc = dynamic_cast<const NetEConst*>(re);
  if (lc == 0 || rc == 0) return 0;
...
NetEConst* NetEBComp::eval_weqeq_(bool ne_flag, const NetExpr* le,
                                  const NetExpr* re) const {
  const NetEConst* lc = dynamic_cast<const NetEConst*>(le);
  const NetEConst* rc = dynamic_cast<const NetEConst*>(re);
  if (lc == 0 || rc == 0) return 0;
  ...
  for (unsigned idx = 0; idx < lv.len(); idx += 1) {
    ...
    // An X or Z in the L-value that is not matches by an R-value X/Z returns
    // undefined.
    switch (lv.get(idx)) {
      case verinum::Vx:
      case verinum::Vz:
...
NetEConst* NetEBComp::eval_arguments_(const NetExpr* l,
                                      const NetExpr* r) const {
  NetEConst* res = 0;
  ...
  eval_debug(this, res,
             l->expr_type() == IVL_VT_REAL || r->expr_type() == IVL_VT_REAL);
  return res;
}
...
NetEConst* NetEBLogic::eval_tree_real_(const NetExpr* l,
                                       const NetExpr* r) const {
  double lval;
  double rval;
...
NetEConst* NetEBLogic::eval_arguments_(const NetExpr* l,
                                       const NetExpr* r) const {
...
NetExpr* NetEBMinMax::eval_tree_real_(const NetExpr* l,
                                      const NetExpr* r) const {
  double lval;
  double rval;
...
NetExpr* NetEBMinMax::eval_arguments_(const NetExpr* l,
                                      const NetExpr* r) const {
  if (expr_type() == IVL_VT_REAL) return eval_tree_real_(l, r);
  assert(expr_type() == IVL_VT_LOGIC);
...
NetEConst* NetEBShift::eval_arguments_(const NetExpr* l,
                                       const NetExpr* r) const {
    ...
    if (expr) {
      ...
      if (!expr->has_width()) {
        cerr << get_fileline() << ": error: concatenation "
             << "operand has indefinite width: " << *parms_[idx] << endl;
        local_errors += 1;
      } else if (expr->expr_width() == 0) {
        cerr << expr->get_fileline() << ": internal error: "
             << "Operand of concatenation has no width: " << *expr << endl;
        local_errors += 1;
      }
  ...
  switch (const_logical(cond_)) {
    case C_0:
      ...
      if (debug_eval_tree) {
        ...
        cerr << get_fileline()
             << ":      : Selecting false case: " << *false_val_ << endl;
      }
    ...
    case C_1:
      ...
      if (debug_eval_tree) {
        cerr << get_fileline() << ": debug: Evaluate ternary with "
             << "constant condition value: ";
        print_ternary_cond(cond_);
        cerr << get_fileline() << ":      : Selecting true case: " << *true_val_
             << endl;
      }

      if (expr_type() == IVL_VT_REAL && true_val_->expr_type() != IVL_VT_REAL) {
        verireal t;
...
NetExpr* NetETernary::blended_arguments_(const NetExpr* te,
                                         const NetExpr* fe) const {
  ...
  if (t == 0 || f == 0) {
    ...
    if (debug_eval_tree) {
      ...
      cerr << get_fileline() << ":      : Blending real cases "
           << "true=" << tv.as_double() << ", false=" << fv.as_double()
           << ", to get " << val << endl;
    }
  ...
  for (unsigned idx = 0; idx < rsize; idx += 1) {
    ...
    if (tv == fv && (tv == verinum::V0 || tv == verinum::V1))
      val.set(idx, tv);
  }
  val.has_sign(has_sign());
  ...
  if (debug_eval_tree) {
    cerr << get_fileline() << ": debug: Evaluate ternary with "
         << "constant condition value: ";
    print_ternary_cond(cond_);
    cerr << get_fileline() << ":      : Blending cases to get " << val << endl;
  }
...
NetExpr* NetEUnary::eval_arguments_(const NetExpr* ex) const {
  ...
  switch (op_) {
    ...
    case '~':
      ...
      for (unsigned idx = 0; idx < val.len(); idx += 1) switch (val.get(idx)) {
          case verinum::V0:
...
NetEConst* NetEUReduce::eval_tree_real_(const NetExpr* ex) const {
  ...
  verinum::V res = val->value().as_double() == 0.0 ? verinum::V1 : verinum::V0;
  ...
  switch (op_) {
    ...
    case '2':
      if (const NetEConst* val = dynamic_cast<const NetEConst*>(ex)) {
        ...
        if (expr_width() > 0) res_val = cast_to_width(res_val, expr_width());
...
NetEConst* NetESFunc::evaluate_clog2_(const NetExpr* arg_) const {
  ...
  if (!arg.is_defined()) {
    ...
    if (arg.is_negative()) {
      ...
      if (!arg.has_len()) arg = trim_vnum(arg);
    }
    ...
    if (is_neg && res < integer_width) res = integer_width;
...
static void no_string_arg(const NetESFunc* info, unsigned arg_num) {
  cerr << info->get_fileline() << ": error: constant function " << info->name()
       << "() does not support a string argument (" << arg_num + 1 << ")."
       << endl;
}
...
static bool get_array_info(const NetExpr* arg, long dim, long& left,
                           long& right, bool& defer) {
...
NetEConst* NetESFunc::evaluate_array_funcs_(ID id, const NetExpr* arg0,
...
NetESFunc::ID NetESFunc::built_in_id_() const {
   ...
   * flag was given. */
  if (funcs_need_init &&
      ((generation_flag >= GN_VER2005) || gn_icarus_misc_flag)) {
  ...
  switch (parms_.size()) {
    case 1:
      if (!takes_nargs_(id, 1)) {
        cerr << get_fileline() << ": error: constant function " << name_
    ...
    default:
      ...
      if (!takes_nargs_(id, parms_.size())) {
        cerr << get_fileline() << ": error: constant function " << name_
             << "() does not support " << parms_.size() << " arguments."
             << endl;
        return 0;
      }
      if (id == CTBITS) {
        ...
        cerr << get_fileline() << ": sorry: constant functions with "
             << parms_.size() << " arguments are not supported: " << name_
             << "()." << endl;
      }
      return 0;
  }
...
NetExpr* NetEUFunc::eval_tree() {
  ...
  if (!opt_const_func && !need_const_) return 0;
  ...
  // us this is safe.
  if (!func()->is_auto() && !need_const_ && (opt_const_func < 2)) return 0;

  // Run through the input parameters to check they are constants.
  for (unsigned idx = 0; idx < parm_count(); idx += 1) {
    if (dynamic_cast<const NetEConst*>(parm(idx))) continue;
    if (dynamic_cast<const NetECReal*>(parm(idx))) continue;
    return 0;
  }
