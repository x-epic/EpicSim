...
NetNet* PEConcat::elaborate_lnet_common_(Design* des, NetScope* scope,
  ...
  if (repeat_) {
    cerr << get_fileline()
         << ": sorry: I do not know how to"
            " elaborate repeat concatenation nets."
         << endl;
    return 0;
  }
  ...
  for (unsigned idx = 0; idx < nets.count(); idx += 1) {
    if (debug_elaborate) {
      cerr << get_fileline() << ": debug: Elaborate subexpression " << idx
           << " of " << nets.count() << " l-values: " << *parms_[idx] << endl;
    }
    ...
    if (nets[idx] == 0) {
      ...
      cerr << parms_[idx]->get_fileline() << ": error: "
           << "concatenation operand can no be real: " << *parms_[idx] << endl;
      errors += 1;
      continue;
    } else {
  ...
  NetNet* osig =
      new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, tmp2_vec);
  ...
  if (bidirectional_flag) {
    if (debug_elaborate) {
      cerr << get_fileline() << ": debug: Generating tran(VP) "
           << "to connect input l-value to subexpressions." << endl;
    }
...
bool PEConcat::is_collapsible_net(Design* des, NetScope* scope) const {
  ...
  for (unsigned idx = 0; idx < parms_.size(); idx += 1) {
    ...
    if (parms_[idx] == 0) return false;

    if (!parms_[idx]->is_collapsible_net(des, scope)) return false;
  }
    ...
    case index_component_t::SEL_IDX_UP: {
      ...
      if (!tmp) {
        cerr << get_fileline() << ": error: indexed part select of "
             << sig->name() << " must be a constant in this context." << endl;
        des->errors += 1;
        return 0;
      }
      ...
      if (!tmp->value().is_defined()) {
        if (warn_ob_select) {
          ...
          cerr << wid << "] is always outside the vector." << endl;
        }
        return false;
      }
    ...
    case index_component_t::SEL_PART: {
      ...
      if (!part_defined_flag) {
        if (warn_ob_select) {
          cerr << get_fileline() << ": warning: " << sig->name();
          if (sig->unpacked_dimensions() > 0) cerr << "[]";
          cerr << "['bx] is always outside the vector." << endl;
        }
        return false;
      }
      ...
      if (prefix_indices.size() + 1 < sig->packed_dims().size()) {
        ...
        if (lidx_tmp > midx_tmp) {
          cerr << get_fileline() << ": error: Part select " << sig->name()
               << "[" << msb << ":" << lsb << "] indices reversed." << endl;
          cerr << get_fileline() << ":      : Did you mean " << sig->name()
               << "[" << lsb << ":" << msb << "]?" << endl;
          long tmp = midx_tmp;
          midx_tmp = lidx_tmp;
          lidx_tmp = tmp;
        ...
        if (midx_tmp >= (long)sig->vector_width() || lidx_tmp < 0) {
          cerr << get_fileline() << ": warning: Part select " << sig->name();
          if (sig->unpacked_dimensions() > 0) {
            cerr << "[]";
          }
          cerr << "[" << msb << ":" << lsb << "] is out of range." << endl;
        }
        /* This is completely out side the signal so just skip it. */
    ...
    case index_component_t::SEL_BIT:
      if (name_tail.index.size() > sig->unpacked_dimensions()) {
        ...
        if (prefix_indices.size() + 2 <= sig->packed_dims().size()) {
          ...
          bool rcl = sig->sb_to_slice(prefix_indices, msb, tmp_loff, tmp_lwid);
          ivl_assert(*this, rcl);
...
NetNet* PEIdent::elaborate_lnet_common_(Design* des, NetScope* scope,
  ...
  if (sig == 0 && path_.size() >= 2) {
    if (debug_elaborate) {
      cerr << get_fileline()
           << ": PEIdent::elaborate_lnet_common_: "
              "Symbol not found, try again with path_prefix="
           << path_prefix << " and method_name=" << path_tail.name << endl;
    }
  ...
  /* If this is SystemVerilog and the variable is not yet
     ...
     wire. */
  if (gn_var_can_be_uwire() && (sig->type() == NetNet::REG) &&
      (sig->peek_lref() == 0)) {
  ...
  if ((struct_type = sig->struct_type()) && !method_name.nil()) {
    ...
    const struct netstruct_t::member_t* member =
        struct_type->packed_member(method_name, member_off);
    ivl_assert(*this, member);

    if (debug_elaborate) {
      cerr << get_fileline() << ": PEIdent::elaborate_lnet_common_: "
           << "Member " << method_name << " has type " << *member->net_type
           << "." << endl;
      cerr << get_fileline() << ":                                : "
    ...
    ivl_assert(*this,
               path_prefix.back().index.size() + 1 == sig->packed_dimensions());
    ...
    if (sig->packed_dimensions() > 1) {
      ...
      if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_lnet_common_: "
             << "packed_base=" << *packed_base << ", member_off=" << member_off
             << endl;
      }
    }
    ...
    if (!path_tail.index.empty()) {
      ...
      ivl_assert(*this, tail_sel.sel == index_component_t::SEL_PART ||
                            tail_sel.sel == index_component_t::SEL_BIT);
    ...
    if (debug_elaborate) {
      cerr << get_fileline() << ": PEIdent::elaborate_lnet_common_: "
           << "Net assign to unpacked array \"" << sig->name() << "\" with "
           << sig->pin_count() << " elements." << endl;
    }
    ...
    if (path_tail.index.size() < sig->unpacked_dimensions()) {
      cerr << get_fileline() << ": error: Array " << path() << " needs "
           << sig->unpacked_dimensions() << " indices,"
    ...
    indices_to_expressions(des, scope, this, path_tail.index,
                           sig->unpacked_dimensions(), true, flags,
    ...
    if (flags.invalid) {
      ...
      cerr << get_fileline() << ": warning: "
           << "ignoring undefined l-value array access " << sig->name()
      ...
      ivl_assert(*this,
                 unpacked_indices_const.size() == sig->unpacked_dimensions());
      canon_index = normalize_variable_unpacked(sig, unpacked_indices_const);

      if (canon_index == 0) {
        cerr << get_fileline() << ": warning: "
             << "ignoring out of bounds l-value array access " << sig->name()
    ...
    if (path_tail.index.size() > sig->unpacked_dimensions()) {
      ...
      if (!eval_part_select_(des, scope, sig, midx_tmp, lidx_tmp)) return 0;

      if (lidx_tmp < 0) {
        cerr << get_fileline()
             << ": sorry: part selects "
                "straddling the start of signal ("
             << path_ << ") are not currently supported." << endl;
        des->errors += 1;
        return 0;
      }
      midx = midx_tmp;
    ...
    if (debug_elaborate) {
      cerr << get_fileline() << ": PEIdent::elaborate_lnet_common_: "
           << "path_tail.index.size()=" << path_tail.index.size() << endl;
    }
  ...
  if (sig->type() == NetNet::UNRESOLVED_WIRE &&
      sig->test_and_set_part_driver(midx, lidx, widx_flag ? widx : 0)) {
    cerr << get_fileline() << ": error: Unresolved net/uwire " << sig->name()
         << " cannot have multiple drivers." << endl;
    if (debug_elaborate) {
      cerr << get_fileline() << ":      : Overlap in "
           << "[" << midx << ":" << lidx << "] (canonical)"
           << ", widx=" << (widx_flag ? widx : 0)
           << ", vector width=" << sig->vector_width() << endl;
    }
    des->errors += 1;
    return 0;
  }
  ...
  if (sig->pin_count() > 1 && widx_flag) {
    ...
    NetNet* tmp =
        new NetNet(scope, scope->local_symbol(), sig->type(), tmp2_vec);
    tmp->set_line(*this);
    tmp->local_flag(true);
  ...
  if (subnet_wid != sig->vector_width()) {
    ...
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: "
           << "Elaborate lnet part select " << sig->name() << "[base=" << lidx
           << " wid=" << subnet_wid << "]" << endl;

    netvector_t* tmp2_vec =
        new netvector_t(sig->data_type(), subnet_wid - 1, 0);
    NetNet* subsig = new NetNet(sig->scope(), sig->scope()->local_symbol(),
    ...
    if (bidirectional_flag) {
      ...
      NetTran* sub = new NetTran(scope, scope->local_symbol(),
                                 sig->vector_width(), subnet_wid, lidx);
      sub->set_line(*this);
      des->add_node(sub);
      ...
      NetPartSelect* sub =
          new NetPartSelect(sig, lidx, subnet_wid, NetPartSelect::PV);
      des->add_node(sub);
      sub->set_line(*this);
...
NetNet* PEIdent::elaborate_subport(Design* des, NetScope* scope) const {
  ...
  if (sig == 0) {
    cerr << get_fileline() << ": error: no wire/reg " << path_ << " in module "
  ...
  if (sig->unpacked_dimensions() && !gn_system_verilog()) {
    cerr << get_fileline() << ": error: "
         << "Ports cannot be unpacked arrays. Try enabling SystemVerilog "
            "support."
         << endl;
    des->errors += 1;
    return 0;
  ...
  if (sig->unpacked_dimensions()) {
    if (debug_elaborate) {
      cerr << get_fileline() << ": PEIdent::elaborate_subport: "
           << "path_=\"" << path_ << "\" is an unpacked array with "
           << sig->pin_count() << " elements." << endl;
    }
  ...
  if (!eval_part_select_(des, scope, sig, midx, lidx)) return 0;
  ...
  switch (sig->port_type()) {
    ...
    case NetNet::PINOUT:
      ps = new NetTran(scope, scope->local_symbol(), sig->vector_width(), swid,
                       lidx);
...
bool PEIdent::is_collapsible_net(Design* des, NetScope* scope) const {
  ...
  if (sig->type() == NetNet::REG) return false;
