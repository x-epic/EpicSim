...
static NetBranch* find_existing_implicit_branch(NetNet* sig, NetNet* gnd) {
  ...
  for (Link* cur = nex->first_nlink(); cur; cur = cur->next_nlink()) {
    if (cur->is_equal(sig->pin(0))) continue;
    ...
    if (tmp->pin(1).is_linked(gnd->pin(0))) return tmp;
  }
...
NetExpr* elaborate_rval_expr(Design* des, NetScope* scope,
  ...
  if (debug_elaborate) {
    ...
    cerr << ", lv_type=" << lv_type << ", lv_width=" << lv_width << endl;
  }
  ...
  return elab_and_eval(des, scope, expr, context_wid, need_const, false,
                       lv_type, force_unsigned);
}
...
NetExpr* PExpr::elaborate_expr(Design* des, NetScope* scope, ivl_type_t,
...
NetExpr* PExpr::elaborate_expr(Design* des, NetScope*, unsigned,
                               unsigned) const {
  cerr << get_fileline() << ": internal error: I do not know how to"
  ...
  cerr << get_fileline() << ":               : Expression is: " << *this
       << endl;
  cerr << get_fileline()
       << ":               : Expression type: " << typeid(*this).name() << endl;
  des->errors += 1;
  return 0;
}
...
NetExpr* PEAssignPattern::elaborate_expr(Design* des, NetScope* scope,
  ...
  // handle. Internally, EpicSim uses this to represent
  // nil dynamic arrays.
  ...
  cerr << get_fileline() << ":      : Expression is: " << *this << endl;
  des->errors += 1;
  return 0;
}
...
NetExpr* PEAssignPattern::elaborate_expr(Design* des, NetScope*, unsigned,
                                         unsigned) const {
...
unsigned PEBinary::test_width(Design* des, NetScope* scope,
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PEBinary::test_width: "
         << "op_=" << op_ << ", l_width=" << l_width << ", r_width=" << r_width
  ...
  if (expr_type_ == IVL_VT_REAL) {
    ...
    switch (op_) {
      case '+':
      case '-':
        if (mode >= EXPAND) expr_width_ += 1;
        break;

      case '*':
        if (mode >= EXPAND) expr_width_ = l_width + r_width;
        break;
 ...
 */
NetExpr* PEBinary::elaborate_expr_base_(Design* des, NetExpr* lp, NetExpr* rp,
                                        unsigned expr_wid) const {
  if (debug_elaborate) {
    cerr << get_fileline() << ": debug: elaborate expression " << *this
         << " expr_width=" << expr_wid << endl;
  }
  ...
  switch (op_) {
    ...
    case 'o':
      cerr << get_fileline() << ": internal error: "
           ...
           << " Should have been handled in NetEBLogic::elaborate." << endl;
      des->errors += 1;
      return 0;
    ...
    case 'p':
      cerr << get_fileline() << ": internal error: "
           << "Elaboration of " << human_readable_op(op_)
           << " Should have been handled in NetEBPower::elaborate." << endl;
      des->errors += 1;
      return 0;
    ...
    case 'R':
      cerr << get_fileline() << ": internal error: "
           << "Elaboration of " << human_readable_op(op_)
           << " Should have been handled in NetEBShift::elaborate." << endl;
      des->errors += 1;
      return 0;
    ...
    case '>':
      cerr << get_fileline() << ": internal error: "
           << "Elaboration of " << human_readable_op(op_)
           << " Should have been handled in NetEBComp::elaborate." << endl;
      des->errors += 1;
      return 0;
...
NetExpr* PEBinary::elaborate_expr_base_bits_(Design* des, NetExpr* lp,
                                             NetExpr* rp,
                                             unsigned expr_wid) const {
...
NetExpr* PEBinary::elaborate_expr_base_div_(Design* des, NetExpr* lp,
                                            NetExpr* rp,
                                            unsigned expr_wid) const {
  ...
  if (op_ == '%' && !gn_icarus_misc_flag) {
    if (lp->expr_type() == IVL_VT_REAL || rp->expr_type() == IVL_VT_REAL) {
      cerr << get_fileline()
           << ": error: Modulus operator "
              "may not have REAL operands."
           << endl;
      des->errors += 1;
    }
  }
...
NetExpr* PEBinary::elaborate_expr_base_mult_(Design*, NetExpr* lp, NetExpr* rp,
                                             unsigned expr_wid) const {
...
unsigned PEBComp::test_width(Design* des, NetScope* scope, width_mode_t&) {
  ...
  l_width_ = l_width;
  if (type_is_vectorable(l_type) && (r_width > l_width)) l_width_ = r_width;

  r_width_ = r_width;
  if (type_is_vectorable(r_type) && (l_width > r_width)) r_width_ = l_width;
  ...
  if (mode >= EXPAND) {
    if (type_is_vectorable(l_type) && (l_width_ < integer_width)) l_width_ += 1;
    if (type_is_vectorable(r_type) && (r_width_ < integer_width)) r_width_ += 1;
  }

  if (debug_elaborate) {
    cerr << get_fileline() << ": debug: "
         << "Comparison expression operands are " << l_type << " " << l_width
         << " bits and " << r_type << " " << r_width << " bits. Resorting to "
         << l_width_ << " bits and " << r_width_ << " bits." << endl;
  }
...
NetExpr* PEBComp::elaborate_expr(Design* des, NetScope* scope,
  ...
  switch (op_) {
    case 'E': /* === */
    case 'N': /* !== */
      if (lp->expr_type() == IVL_VT_REAL || lp->expr_type() == IVL_VT_STRING ||
          rp->expr_type() == IVL_VT_REAL || rp->expr_type() == IVL_VT_STRING) {
        cerr << get_fileline() << ": error: " << human_readable_op(op_)
             << " operator may not have REAL or STRING operands." << endl;
        des->errors += 1;
        return 0;
      }
      break;
...
unsigned PEBLeftWidth::test_width(Design* des, NetScope* scope,
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PEBLeftWidth::test_width: "
         << "op_=" << op_ << ", left_=" << *left_ << ", right_=" << *right_
  ...
  if ((mode >= EXPAND) && type_is_vectorable(expr_type_)) {
    ...
    if (r_width < sizeof(long) * 8) {
      r_val = (1L << r_width) - 1L;
      if ((op_ == 'p') && right_->has_sign()) r_val >>= 1;
    }
    ...
    if (r_val < 0) r_val = 0;
    if ((unsigned long)r_val > width_cap) r_val = width_cap;
    ...
    switch (op_) {
      case 'l':  // <<
        if (l_mode != SIZED) use_width += (unsigned)r_val;
        break;
      ...
      case 'R':  // >>>
                 ...
                 // expression type.
        if ((mode == LOSSLESS) && signed_flag_) mode = UPSIZE;
        break;
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PEBLeftWidth::test_width: "
         ...
         << ", min_width_=" << min_width_ << ", mode=" << width_mode_name(mode)
         << endl;
  }
...
NetExpr* PEBShift::elaborate_expr_leaf(Design* des, NetExpr* lp, NetExpr* rp,
  ...
  if (NetEConst* lpc = dynamic_cast<NetEConst*>(lp)) {
    ...
    if ((op_ == 'R' && signed_flag_) && (shift >= expr_wid)) {
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "Value signed-right-shifted " << shift << " beyond width of "
             << expr_wid << ". Elaborate as replicated top bit." << endl;
    ...
    if (shift >= expr_wid) {
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "Value shifted " << shift << " beyond width of " << expr_wid
             << ". Elaborate as constant zero." << endl;
...
unsigned PECallFunction::test_width_sfunc_(Design* des, NetScope* scope,
  ...
  if (name == "$ivlh_to_unsigned") {
    ...
    // The EpicSim specific $ivlh_to_unsigned() system
    ...
    if (nexpr == 0) {
      cerr << get_fileline() << ": error: "
           << "Unable to evaluate " << name << " width argument: " << *pexpr
           << endl;
      return 0;
    }
  ...
  if (name == "$signed" || name == "$unsigned") {
    ...
    if ((arg_mode >= EXPAND) && type_is_vectorable(expr_type_)) {
      ...
      if (expr_width_ < integer_width) expr_width_ = integer_width;
    }
  ...
  if (debug_elaborate)
    cerr << get_fileline() << ": debug: test_width "
         << "of system function " << name << " returns wid=" << expr_width_
...
unsigned PECallFunction::test_width(Design* des, NetScope* scope,
  ...
  if (def == 0) {
    ...
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: test_width "
           << "cannot find definition of " << path_ << " in "
...
unsigned PECallFunction::test_width_method_(Design* des, NetScope* scope,
  ...
  if (net != 0) use_darray = net->darray_type();
  ...
  if (net == 0 && use_path.size() >= 2) {
    ...
    net = 0;
    symbol_search(this, des, scope, tmp_path, net, par, eve, ex1, ex2);
    if (net && net->class_type()) {
      if (debug_elaborate) {
        cerr << get_fileline() << ": PECallFunction::test_width_method_: "
             << "Found net=" << tmp_path << ", member_name=" << member_name
  ...
  if (const netclass_t* class_type = net->class_type()) {
    if (debug_elaborate) {
      cerr << get_fileline() << ": PECallFunction::test_width_method_: "
           << "Try to find method " << method_name << " of class "
...
NetExpr* PECallFunction::elaborate_sfunc_(Design* des, NetScope* scope,
  ...
  if (name == "$signed" || name == "$unsigned") {
    ...
    if (!type_is_vectorable(expr_type_)) {
      cerr << get_fileline() << ": error: The argument to " << name
    ...
    if (debug_elaborate) {
      cerr << get_fileline() << ": PECallFunction::elaborate_sfunc_: " << name
           << " expression is the argument cast to expr_wid=" << expr_wid
           << endl;
    }
    PExpr* expr = parms_[0];
  ...
  NetESFunc* fun =
      new NetESFunc(name, expr_type_, expr_width_, nparms, is_overridden_);
  fun->set_line(*this);
  ...
  if (is_overridden_ && (need_const || scope->need_const_func())) {
    cerr << get_fileline()
         << ": sorry: Cannot evaluate "
            "overridden system function."
         << endl;
    des->errors += 1;
  }
  ...
  for (unsigned idx = 0; idx < nparms; idx += 1) {
    ...
    if (expr) {
      NetExpr* tmp = elab_sys_task_arg(des, scope, name, idx, expr, need_const);
      if (tmp) {
        fun->parm(idx, tmp);
      } else {
        parm_errors += 1;
  ...
  if (missing_parms || parm_errors) return 0;
...
NetExpr* PECallFunction::elaborate_access_func_(Design* des, NetScope* scope,
  ...
  if (parms_.size() == 1) {
    ...
    if ((branch = find_existing_implicit_branch(sig, gnd))) {
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "Re-use implicit branch from " << branch->get_fileline()
             << endl;
    } else {
...
static NetExpr* check_for_enum_methods(
    const LineInfo* li, Design* des, NetScope* scope, const netenum_t* netenum,
    const pform_name_t& use_path, perm_string method_name, NetExpr* expr,
    unsigned rtn_wid, PExpr* parg, unsigned args) {
  ...
  if (method_name == "first") {
    ...
    NetEConstEnum* tmp =
        new NetEConstEnum(scope, item->first, netenum, item->second);
    tmp->set_line(*li);
    delete expr;  // The elaborated enum variable is not needed.
    return tmp;
  }
  ...
  NetExpr* count = 0;
  if (args != 0 && parg) {
    count = elaborate_rval_expr(des, scope, &netvector_t::atom2u32, IVL_VT_BOOL,
                                32, parg);
    if (count == 0) {
      cerr << li->get_fileline()
           << ": error: unable to elaborate "
              "enumeration method argument "
           << use_path << "." << method_name << "(" << parg << ")." << endl;
      args = 0;
      des->errors += 1;
  ...
  if (method_name == "name") {
    ...
    sys_expr =
        new NetESFunc("$ivl_enum_method$name", IVL_VT_STRING, rtn_wid, 2);
    ...
    sys_expr = new NetESFunc("$ivl_enum_method$next", netenum, 2 + (args != 0));
    sys_expr->parm(0, new NetENetenum(netenum));
    sys_expr->parm(1, expr);
    ...
    sys_expr = new NetESFunc("$ivl_enum_method$prev", netenum, 2 + (args != 0));
    sys_expr->parm(0, new NetENetenum(netenum));
    sys_expr->parm(1, expr);
    if (args != 0) sys_expr->parm(2, count);
    ...
    cerr << li->get_fileline()
         << ": error: Unknown enumeration "
            "method "
         << use_path << "." << method_name << "()." << endl;
    des->errors += 1;
    return expr;
  }
  ...
  if (debug_elaborate) {
    cerr << li->get_fileline() << ": debug: Generate " << sys_expr->name()
         << "(" << use_path << ")" << endl;
  }
...
bool calculate_part(const LineInfo* li, Design* des, NetScope* scope,
  ...
  if (texpr == 0 || !eval_as_long(msb, texpr)) {
    cerr << li->get_fileline()
         << ": error: "
            "Array/part index expressions must be constant here."
         << endl;
    des->errors += 1;
    return false;
  }
 ...
 * of the variable.
 */
static NetExpr* check_for_struct_members(
    const LineInfo* li, Design* des, NetScope* scope, NetNet* net,
    const list<index_component_t>& base_index, pform_name_t member_path) {
  const netstruct_t* struct_type = net->struct_type();
  ivl_assert(*li, struct_type);

  if (!struct_type->packed()) {
    cerr << li->get_fileline() << ": sorry: "
         << "Unpacked structures not supported here." << endl;
    des->errors += 1;
    return 0;
  }
  ...
  do {
    ...
    if (debug_elaborate) {
      cerr << li->get_fileline() << ": check_for_struct_members: "
           ...
           << " (completed_path=" << completed_path << ")" << endl;
    }
    ...
    const netstruct_t::member_t* member =
        struct_type->packed_member(member_name, tmp_off);

    if (member == 0) {
      cerr << li->get_fileline() << ": error: Member " << member_name
           << " is not a member of struct type of " << net->name() << "."
           << completed_path << endl;
      des->errors += 1;
      return 0;
    }
    ...
    off += tmp_off;
    ivl_assert(*li,
               use_width >= (unsigned long)member->net_type->packed_width());
    ...
    if (const netstruct_t* tmp_struct =
            dynamic_cast<const netstruct_t*>(member->net_type)) {
      // If the member is itself a struct, then get
      // ready to go on to the next iteration.
      struct_type = tmp_struct;

    } else if (const netenum_t* tmp_enum =
                   dynamic_cast<const netenum_t*>(member->net_type)) {
      // If the element is an enum, then we don't have
      // anything special to do.
      if (debug_elaborate) {
        cerr << li->get_fileline() << ": check_for_struct_members: "
             << "Tail element is an enum" << *tmp_enum << endl;
      }
      struct_type = 0;

    } else if (const netvector_t* mem_vec =
                   dynamic_cast<const netvector_t*>(member->net_type)) {
      if (debug_elaborate) {
        cerr << li->get_fileline() << ": check_for_struct_members: "
             << "member_comp=" << member_comp << " has "
             << member_comp.index.size() << " indices." << endl;
      }
      ...
      if (member_comp.index.size() > 0) {
        ...
        rc = calculate_part(li, des, scope, member_comp.index.back(), tail_off,
                            tail_wid);
        if (!rc) return 0;
        ...
        if (debug_elaborate) {
          cerr << li->get_fileline() << ": check_for_struct_members: "
               ...
               << " off=" << off << " use_width=" << use_width << endl;
        }
    ...
    } else if (const netparray_t* array =
      ...
      list<long> prefix_indices;
      bool rc =
          evaluate_index_prefix(des, scope, prefix_indices, member_comp.index);
      ivl_assert(*li, rc);

      // Evaluate the last index expression into a constant long.
      NetExpr* texpr =
          elab_and_eval(des, scope, member_comp.index.back().msb, -1, true);
      long tmp;
      ...
      if (debug_elaborate) {
        cerr << li->get_fileline()
             << ": PEIdent::elaborate_lval_net_packed_member_: "
             << "parray subselection loff=" << loff << ", lwid=" << lwid
             << ", element_width=" << element_width << endl;
      }
      ...
      cerr << li->get_fileline() << ": internal error: "
           << "Unexpected member type? " << *(member->net_type) << endl;
      des->errors += 1;
      struct_type = 0;
    }
  ...
  if (debug_elaborate) {
    cerr << li->get_fileline() << ": check_for_struct_member: "
         << "Finally, completed_path=" << completed_path << ", off=" << off
         << ", use_width=" << use_width << ", base=" << *base << endl;
  }
...
static NetExpr* check_for_class_property(const LineInfo* li, Design* des,
                                         NetScope* scope, NetNet* net,
                                         const name_component_t& comp) {
  const netclass_t* class_type = net->class_type();
  int pidx = class_type->property_idx_from_name(comp.name);
  if (pidx < 0) {
    cerr << li->get_fileline() << ": error: "
         << "Class " << class_type->get_name() << " has no property "
         << comp.name << "." << endl;
    des->errors += 1;
    return 0;
  }
  ...
  if (debug_elaborate) {
    cerr << li->get_fileline() << ": check_for_class_property: "
         << "Property " << comp.name << " of net " << net->name()
         << ", context scope=" << scope_path(scope) << endl;
  }
  ...
  if (qual.test_static()) {
    ...
    return class_static_property_expression(li, class_type, prop_name);
  }
...
NetExpr* PECallFunction::elaborate_expr_pkg_(Design* des, NetScope* scope,
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PECallFunction::elaborate_expr_pkg_: "
         << "Elaborate " << path_ << " as function in package "
         << package_->pscope_name() << "." << endl;
  }
  ...
  if (!check_call_matches_definition_(des, dscope)) return 0;
...
NetExpr* PECallFunction::elaborate_expr(Design* des, NetScope* scope,
                                        unsigned expr_wid,
                                        unsigned flags) const {
  if (package_) return elaborate_expr_pkg_(des, scope, expr_wid, flags);
  ...
  if (def == 0) {
    ...
    if (access_nature)
      return elaborate_access_func_(des, scope, access_nature, expr_wid);
    ...
    cerr << get_fileline() << ": error: No function named `" << path_
         << "' found in this context (" << scope_path(scope) << ")." << endl;
    des->errors += 1;
    return 0;
  }
  ...
  if (gn_system_verilog() && (path_.size() == 1)) {
    ...
    if (c_scope && (c_scope == dscope->get_class_scope())) {
      NetExpr* tmp = elaborate_expr_method_(des, scope, expr_wid, true);
      assert(tmp);
      return tmp;
    }
  }
...
NetExpr* PECallFunction::elaborate_expr(Design* des, NetScope* scope,
  ...
  assert(darray);
  return elaborate_expr(des, scope, darray->element_type()->packed_width(),
                        flags);
}

NetExpr* PECallFunction::elaborate_base_(Design* des, NetScope* scope,
                                         NetScope* dscope, unsigned expr_wid,
                                         unsigned flags) const {
  if (!check_call_matches_definition_(des, dscope)) return 0;
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PECallFunction::elaborate_base_: "
         << "Expecting " << parms_count << " argument for function "
         << scope_path(dscope) << "." << endl;
  }
  ...
  unsigned parm_errors =
      elaborate_arguments_(des, scope, def, need_const, parms, 0);
  ...
  if (NetNet* res = dscope->find_signal(dscope->basename())) {
    ...
    if (res->darray_type()) return func;
...
unsigned PECallFunction::elaborate_arguments_(Design* des, NetScope* scope,
  ...
  /* The parser can't distinguish between a function call with
     ...
     argument case here. */
  if ((parm_count == 0) && (actual_count == 1) && (parms_[0] == 0)) return 0;

  if (actual_count > parm_count) {
    cerr << get_fileline() << ": error: "
         << "Too many arguments (" << actual_count << ", expecting "
         << parm_count << ")"
  ...
  for (unsigned idx = 0; idx < parm_count; idx += 1) {
    ...
    if (tmp) {
      parms[pidx] = elaborate_rval_expr(
          des, scope, def->port(pidx)->net_type(), def->port(pidx)->data_type(),
          (unsigned)def->port(pidx)->vector_width(), tmp, need_const);
      if (parms[pidx] == 0) {
        parm_errors += 1;
        continue;
      }
      ...
      if (debug_elaborate)
        cerr << get_fileline() << ": debug:"
             << " function " << path_ << " arg " << (idx + 1)
             << " argwid=" << parms[pidx]->expr_width() << ": " << *parms[idx]
             << endl;
...
NetExpr* PECallFunction::elaborate_expr_method_(Design* des, NetScope* scope,
  ...
  if (net->data_type() == IVL_VT_STRING) {
    if (method_name == "len") {
      NetESFunc* sys_expr =
          new NetESFunc("$ivl_string_method$len", IVL_VT_BOOL, 32, 1);
    ...
    if (method_name == "substr") {
      NetESFunc* sys_expr =
          new NetESFunc("$ivl_string_method$substr", IVL_VT_STRING, 1, 3);
      ...
      tmp = elaborate_rval_expr(des, scope, &netvector_t::atom2u32, IVL_VT_BOOL,
                                32, parms_[0], false);
      sys_expr->parm(1, tmp);
  ...
  if (const netenum_t* netenum = net->enumeration()) {
    ...
    return check_for_enum_methods(this, des, scope, netenum, use_path,
                                  method_name, expr, expr_wid, tmp,
                                  parms_.size());
  }
...
unsigned PECastSize::test_width(Design* des, NetScope* scope, width_mode_t&) {
  ...
  if (expr_width_ == 0) {
    cerr << get_fileline()
         << ": error: Cast size expression "
            "must be constant and greater than zero."
         << endl;
    des->errors += 1;
    return 0;
  }
...
NetExpr* PECastSize::elaborate_expr(Design* des, NetScope* scope,
  ...
  if (cast_width < expr_width_) cast_width = expr_width_;
...
unsigned PECastType::test_width(Design* des, NetScope* scope, width_mode_t&) {
  ...
  if (const netdarray_t* use_darray = dynamic_cast<const netdarray_t*>(t)) {
    ...
    expr_width_ = use_darray->element_width();
                 ...
                 dynamic_cast<const netstring_t*>(t)) {
...
NetExpr* PECastType::elaborate_expr(Design* des, NetScope* scope,
  ...
  if (dynamic_cast<const string_type_t*>(target_)) {
    if (base_->expr_type() == IVL_VT_STRING) return sub;  // no conversion

    if (base_->expr_type() == IVL_VT_LOGIC || base_->expr_type() == IVL_VT_BOOL)
      return sub;  // handled by the target as special cases
  }

  cerr << get_fileline() << ": sorry: This cast operation is not yet supported."
       << endl;
  des->errors += 1;
  return 0;
}
...
unsigned PEConcat::test_width(Design* des, NetScope* scope, width_mode_t&) {
  ...
  for (unsigned idx = 0; idx < parms_.size(); idx += 1) {
    ...
    if (dynamic_cast<PEString*>(parms_[idx])) continue;
  ...
  if (repeat_ && (scope != tested_scope_)) {
    ...
    if (rep == 0) {
      cerr << get_fileline()
           ...
           << endl;
      cerr << get_fileline() << ":      : The expression is: " << *tmp << endl;
      des->errors += 1;
      return 0;
    }
    ...
    if (!rep->value().is_defined()) {
      cerr << get_fileline() << ": error: Concatenation repeat "
           << "may not be undefined (" << rep->value() << ")." << endl;
      des->errors += 1;
      return 0;
    }
    ...
    if (rep->value().is_negative()) {
      cerr << get_fileline() << ": error: Concatenation repeat "
           << "may not be negative (" << rep->value().as_long() << ")." << endl;
      des->errors += 1;
      return 0;
    }
...
NetExpr* PEConcat::elaborate_expr(Design*, NetScope*, ivl_type_t type,
                                  unsigned /*flags*/) const {
...
NetExpr* PEConcat::elaborate_expr(Design* des, NetScope* scope,
  ...
  for (unsigned idx = 0; idx < parms_.size(); idx += 1) {
    if (parms_[idx] == 0) {
      cerr << get_fileline() << ": error: Missing expression " << (idx + 1)
           << " of concatenation list." << endl;
      des->errors += 1;
      continue;
    }
    ...
    if (ex->expr_type() == IVL_VT_REAL) {
      cerr << ex->get_fileline() << ": error: "
           << "Concatenation operand can not be real: " << *parms_[idx] << endl;
      des->errors += 1;
      parm_errors += 1;
      continue;
    }
...
NetExpr* PEFNumber::elaborate_expr(Design*, NetScope*, ivl_type_t,
                                   unsigned) const {
...
NetExpr* PEFNumber::elaborate_expr(Design*, NetScope*, unsigned,
                                   unsigned) const {
  NetECReal* tmp = new NetECReal(*value_);
  tmp->set_line(*this);
  return tmp;
...
bool PEIdent::calculate_packed_indices_(Design* des, NetScope* scope,
                                        NetNet* net,
  ...
  if (path_.back().index.size() > dimensions) {
    cerr << get_fileline() << ": error: the number of indices ("
         ...
         << ") is greater than the number of dimensions (" << dimensions << ")."
         << endl;
    des->errors += 1;
    return false;
  }
...
bool PEIdent::calculate_bits_(Design* des, NetScope* scope, long& msb,
                              bool& defined) const {
  defined = true;
  ...
  if (msb_c == 0) {
    ...
    if (!msb_c->value().is_defined()) defined = false;
 ...
 */
bool PEIdent::calculate_parts_(Design* des, NetScope* scope, long& msb,
                               long& lsb, bool& defined) const {
  defined = true;
  const name_component_t& name_tail = path_.back();
  ivl_assert(*this, !name_tail.index.empty());
  ...
  if (lsb_c == 0) {
    ...
    if (!lsb_c->value().is_defined()) defined = false;
...
bool PEIdent::calculate_param_range_(Design*, NetScope*, const NetExpr* par_msb,
                                     long& par_msv, const NetExpr* par_lsb,
                                     long& par_lsv, long length) const {
  if (par_msb == 0) {
...
unsigned PEIdent::test_width_method_(Design* des, NetScope* scope,
                                     width_mode_t&) {
  if (!gn_system_verilog()) return 0;
  if (path_.size() < 2) return 0;
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PEIdent::test_width_method_: "
         << "Try to find method=" << member_name << " of signal " << use_path
         << endl;
  }
...
unsigned PEIdent::test_width(Design* des, NetScope* scope, width_mode_t& mode) {
  ...
  NetScope* found_in =
      symbol_search(this, des, use_scope, path_, net, par, eve, ex1, ex2);
  ...
  switch (use_sel) {
    ...
    case index_component_t::SEL_BIT: {
      ...
      if ((net == 0) || (net->packed_dimensions() <= 1)) use_width = 1;
      break;
  ...
  if (net != 0) {
    ...
    signed_flag_ = net->get_signed();
    if (debug_elaborate) {
      cerr << get_fileline() << ": PEIdent::test_width: " << net->name()
           << " is a net, "
           << "type=" << expr_type_ << ", width=" << expr_width_
           << ", signed_=" << (signed_flag_ ? "true" : "false")
           << ", use_depth=" << use_depth
           << ", packed_dimensions=" << net->packed_dimensions()
           << ", unpacked_dimensions=" << net->unpacked_dimensions() << endl;
    }
    return expr_width_;
  }
  ...
  if (par != 0) {
    ...
    if (!par->has_width() && (mode < LOSSLESS)) mode = LOSSLESS;

    return expr_width_;
  }

  if (path_.size() == 1 && scope->genvar_tmp.str() &&
  ...
  if (gn_system_verilog() && found_in == 0 && path_.size() >= 2) {
    ...
    if (net != 0) {
      ...
      if (net->struct_type() != 0) {
        ...
        mem = get_struct_member(this, des, scope, net, method_name, unused);
        if (mem) {
...
NetExpr* PEIdent::elaborate_expr(Design* des, NetScope* scope, ivl_type_t ntype,
                                 unsigned flags) const {
  bool need_const = NEED_CONST & flags;
  ...
  /* NetScope*found_in = */ symbol_search(this, des, use_scope, path_, net, par,
                                          eve, ex1, ex2);
  ...
  if (net == 0 && gn_system_verilog() && path_.size() >= 2) {
    ...
    if (net == 0) {
      ...
      return check_for_struct_members(this, des, use_scope, net,
                                      use_path.back().index, member_path);

    } else if (net->class_type() != 0) {
      if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_expr: "
             << "Ident " << use_path << " look for property " << member_comp
             << endl;
      }

      return check_for_class_property(this, des, scope, net, member_comp);
    }
  }

  if (net == 0) {
    cerr << get_fileline() << ": error: Unable to bind variable `" << path_
         << "' in `" << scope_path(use_scope) << "'" << endl;
    des->errors += 1;
    return 0;
  }
  ...
  if (!ntype->type_compatible(net->net_type())) {
    cerr << get_fileline() << ": error: the type of the variable '" << path_
         << "' doesn't match the context type." << endl;
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PEIdent::elaborate_expr: "
         << "Typed ident " << net->name() << " with " << use_comp.index.size()
         << " indices"
         << " and " << net->unpacked_dimensions() << " expected." << endl;
  }

  if (net->unpacked_dimensions() != use_comp.index.size()) {
    cerr << get_fileline() << ": sorry: "
         << "Net " << net->name() << " expects " << net->unpacked_dimensions()
         << ", but got " << use_comp.index.size() << "." << endl;
    des->errors += 1;
  ...
  indices_to_expressions(des, scope, this, use_comp.index,
                         net->unpacked_dimensions(), need_const, idx_flags,
  ...
  if (idx_flags.invalid) {
    ...
    cerr << get_fileline() << ": warning: "
         << "returning 'bx for undefined array access " << net->name()
    ...
    ivl_assert(*this,
               unpacked_indices_const.size() == net->unpacked_dimensions());
...
NetExpr* PEIdent::elaborate_expr_class_member_(Design* des, NetScope* scope,
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PEIdent::elaborate_expr_class_member: "
         << "Found member " << member_name << " is a member of class "
         << class_type->get_name() << ", context scope=" << scope_path(scope)
  ...
  if (const netuarray_t* tmp_ua = dynamic_cast<const netuarray_t*>(tmp_type)) {
    ...
    if (debug_elaborate) {
      cerr << get_fileline() << ": PEIdent::elaborate_expr_class_member_: "
           << "Property " << class_type->get_prop_name(pidx) << " has "
    ...
    if (dims.size() != name_comp.index.size()) {
      cerr << get_fileline() << ": error: "
           ...
           << "expecting " << dims.size() << " to index the property "
           << class_type->get_prop_name(pidx) << "." << endl;
      des->errors += 1;

    } else {
      canon_index = make_canonical_index(des, scope, this, name_comp.index,
                                         tmp_ua, false);
    }
  }
...
NetExpr* PEIdent::elaborate_expr(Design* des, NetScope* scope,
  ...
  if (NetExpr* tmp =
          elaborate_expr_class_member_(des, scope, expr_wid, flags)) {
    return tmp;
  }
  ...
  if (path_.size() > 1) {
    if (NEED_CONST & flags) {
      cerr << get_fileline()
           << ": error: A hierarchical reference"
              " (`"
           << path_
  ...
  while (net == 0 && par == 0 && eve == 0 && base_path.size() > 0) {
    found_in =
        symbol_search(this, des, use_scope, base_path, net, par, eve, ex1, ex2);
    if (net) break;
    if (par) break;
    if (eve) break;
...
research_symbol:
  if (needRescan) {
    cerr << get_fileline()
         << ": PEIdent::elaborate_expr: Try rescan symbol with scope first now."
         << endl;
    net = 0;
    par = 0;
    eve = 0;
    ...
    while (net == 0 && par == 0 && eve == 0 && base_path.size() > 0) {
      found_in = symbol_search_scopefirst(this, des, use_scope, base_path, net,
                                          par, eve, ex1, ex2);
      if (net) break;
      if (par) break;
      if (eve) break;
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PEIdent::elaborate_expr: "
         ...
         << ", member_path=" << member_path << ", par=" << par
         << ", net=" << net << ", eve=" << eve << endl;
  }
  ...
  if (par != 0) {
    ...
    NetExpr* tmp = elaborate_expr_param_(des, scope, par, found_in, ex1, ex2,
                                         expr_wid, flags);
  ...
  if (net != 0) {
    ...
    if (net->scope()->type() == NetScope::MODULE) {
      if (scope->need_const_func()) {
        cerr << get_fileline()
             << ": error: A reference to a "
                "non-local wire or reg (`"
             << path_
             << "') is "
                "not allowed in a constant function."
             << endl;
        des->errors += 1;
        return 0;
      }
    ...
    if (net->struct_type() && member_path.size() > 0) {
      if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_expr: "
             << "Ident " << base_path << " look for struct member "
             << member_path << endl;
      }

      NetExpr* tmp = check_for_struct_members(
          this, des, use_scope, net, base_path.back().index, member_path);
      if (!tmp)
        return 0;
      else
        return pad_to_width(tmp, expr_wid, signed_flag_, *this);
    ...
    if (net->darray_type() && member_path.size() > 0) {
      if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_expr: "
             << "Ident " << base_path << " look for array property "
             << member_path << endl;
      }
    ...
    if (net->class_type() && member_path.size() > 0) {
      if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_expr: "
             << "Ident " << base_path << " look for class property "
             << member_path << endl;
      }

      ivl_assert(*this, member_path.size() == 1);
      const name_component_t member_comp = member_path.front();
      return check_for_class_property(this, des, use_scope, net, member_comp);
    }
    ...
    if (net->enumeration() && member_path.size() > 0) {
      ...
      if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_expr: "
             << "Ident " << base_path << " look for enumeration method "
             << member_path << endl;
      }
      ...
      return check_for_enum_methods(this, des, use_scope, netenum, base_path,
                                    member_comp.name, expr, expr_wid, NULL, 0);
    }
    ...
    if (member_path.size() > 0) {
      ...
      cerr << get_fileline() << ": PEIdent::elaborate_expr: "
           << "Ident " << base_path << " look for member " << member_path
           << " failed, Please check input." << endl;
      needRescan = true;
      goto research_symbol;
    }
    NetExpr* tmp =
        elaborate_expr_net(des, scope, net, found_in, expr_wid, flags);
    ...
    if (debug_elaborate) {
      cerr << get_fileline() << ": PEIdent::elaborate_expr: "
           ...
           << ", tmp->expr_width()=" << tmp->expr_width() << ", tmp=" << *tmp
           << endl;
    }
  ...
  if (path_.size() == 1 && scope->genvar_tmp.str() &&
      strcmp(peek_tail_name(path_), scope->genvar_tmp) == 0) {
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: " << path_ << " is genvar with value "
           << scope->genvar_tmp_val << "." << endl;
    ...
    if (scope->need_const_func()) {
      cerr << get_fileline() << ":      : `" << scope->basename()
           << "' is being used as a constant function, so may "
              "only reference local variables."
           << endl;
    }
    des->errors += 1;
    return 0;
  ...
  if ((path_.size() == 1)) {
    ...
    if (NetScope* nsc = scope->child(use_name)) {
      ...
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: Found scope " << use_name
             << " in scope " << scope->basename() << endl;
  ...
  if (NetScope* nsc = des->find_scope(spath)) {
    ...
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: Found scope " << nsc->basename()
           << " path=" << path_ << endl;

    if (!(SYS_TASK_ARG & flags)) {
      cerr << get_fileline() << ": error: Scope name " << nsc->basename()
           << " not allowed here." << endl;
      des->errors += 1;
    }
...
NetExpr* PEIdent::elaborate_expr_param_bit_(
    Design* des, NetScope* scope, const NetExpr* par, NetScope* found_in,
    const NetExpr* par_msb, const NetExpr* par_lsb, bool need_const) const {
  const NetEConst* par_ex = dynamic_cast<const NetEConst*>(par);
  ivl_assert(*this, par_ex);

  long par_msv, par_lsv;
  if (!calculate_param_range_(des, scope, par_msb, par_msv, par_lsb, par_lsv,
                              par_ex->value().len()))
    return 0;
    ...
    if (!sel_c->value().is_defined()) {
      if (warn_ob_select) {
        cerr << get_fileline()
             ...
             << sel_c->value() << "] for parameter '" << name << "'." << endl;
        cerr << get_fileline()
             << ":        : "
...
NetExpr* PEIdent::elaborate_expr_param_part_(Design* des, NetScope* scope,
  ...
  if (!parts_defined_flag) {
    if (warn_ob_select) {
      ...
      cerr << get_fileline()
           << ": warning: "
              "Undefined part select ["
           << *(psel.msb) << ":" << *(psel.lsb) << "] for parameter '" << name
           << "'." << endl;
      cerr << get_fileline()
           << ":        : "
              "Replacing select with a constant 'bx."
           << endl;
    }
  ...
  if ((msv > lsv && par_msv < par_lsv) || (msv < lsv && par_msv >= par_lsv)) {
    perm_string name = peek_tail_name(path_);
    cerr << get_fileline() << ": error: Part select " << name << "[" << msv
         << ":" << lsv << "] is out of order." << endl;
    des->errors += 1;
    return 0;
  }
  ...
  if (warn_ob_select) {
    if (base < 0) {
      ...
      cerr << get_fileline()
           << ":        : Replacing "
              "the out of bound bits with 'bx."
           << endl;
    }
    if (par_ex->value().has_len() && (base + wid > (long)par->expr_width())) {
      perm_string name = peek_tail_name(path_);
      cerr << get_fileline() << ": warning: Part select " << name << "[" << msv
           << ":" << lsv
           << "] is selecting "
              "after the parameter "
           << name << "[" << par_msv << ":" << par_lsv << "]." << endl;
      cerr << get_fileline()
           << ":        : Replacing "
              "the out of bound bits with 'bx."
           << endl;
...
static void warn_param_ob(long par_msv, long par_lsv, bool defined,
  ...
  if (defined) {
    if (par_msv < par_lsv)
      par_max = par_lsv - par_msv;
    else
      par_max = par_msv - par_lsv;
  } else {
    if (pwid < 0)
      par_max = integer_width;
    else
      par_max = pwid;
  }
  ...
  if (par_base < 0) {
    cerr << info->get_fileline() << ": warning: " << name << "[" << par_base;
    if (up)
      cerr << "+:";
    else
      cerr << "-:";
  ...
  if (par_base + (long)wid - 1 > par_max) {
    cerr << info->get_fileline() << ": warning: " << name << "[" << par_base
         << "+:" << wid << "] is selecting after vector." << endl;
  }
}

NetExpr* PEIdent::elaborate_expr_param_idx_up_(
    Design* des, NetScope* scope, const NetExpr* par, NetScope* found_in,
    const NetExpr* par_msb, const NetExpr* par_lsb, bool need_const) const {
  const NetEConst* par_ex = dynamic_cast<const NetEConst*>(par);
  ivl_assert(*this, par_ex);
  ...
  if (NetEConst* base_c = dynamic_cast<NetEConst*>(base)) {
    if (!base_c->value().is_defined()) {
      ...
      ex->set_line(*this);
      if (warn_ob_select) {
        cerr << get_fileline() << ": warning: " << name << "['bx+:" << wid
    ...
    if (warn_ob_select) {
      ...
      warn_param_ob(par_msv, par_lsv, defined, lsv - par_base, wid, pwid, this,
                    name, true);
    }
    verinum result =
        param_part_select_bits(par_ex->value(), wid, lsv - par_base);
    NetEConst* result_ex = new NetEConst(result);
    result_ex->set_line(*this);
    return result_ex;
  }
...
NetExpr* PEIdent::elaborate_expr_param_idx_do_(
    Design* des, NetScope* scope, const NetExpr* par, NetScope* found_in,
    const NetExpr* par_msb, const NetExpr* par_lsb, bool need_const) const {
  const NetEConst* par_ex = dynamic_cast<const NetEConst*>(par);
  ivl_assert(*this, par_ex);
 ...
 */
NetExpr* PEIdent::elaborate_expr_param_(Design* des, NetScope* scope,
  ...
  const name_component_t& name_tail = path_.back();
  index_component_t::ctype_t use_sel = index_component_t::SEL_NONE;
  if (!name_tail.index.empty()) use_sel = name_tail.index.back().sel;
  ...
  if (use_sel == index_component_t::SEL_BIT)
    return elaborate_expr_param_bit_(des, scope, par, found_in, par_msb,
                                     par_lsb, need_const);

  if (use_sel == index_component_t::SEL_PART)
    return elaborate_expr_param_part_(des, scope, par, found_in, par_msb,
                                      par_lsb, expr_wid);

  if (use_sel == index_component_t::SEL_IDX_UP)
    return elaborate_expr_param_idx_up_(des, scope, par, found_in, par_msb,
                                        par_lsb, need_const);

  if (use_sel == index_component_t::SEL_IDX_DO)
    return elaborate_expr_param_idx_do_(des, scope, par, found_in, par_msb,
                                        par_lsb, need_const);
  ...
  if (etmp) {
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: "
           << "Elaborate parameter <" << path_ << "> as enumeration constant."
           << *etmp << endl;
    tmp = etmp->dup_expr();
    ...
    if (ctmp) {
      ...
      if (cvalue.has_len()) cvalue.has_sign(signed_flag_);
      ...
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "Elaborate parameter <" << name << "> as constant " << *tmp
             << endl;
    }
...
NetExpr* PEIdent::elaborate_expr_net_word_(Design* des, NetScope* scope,
  ...
  if (name_tail.index.size() < net->unpacked_dimensions()) {
    cerr << get_fileline() << ": error: Array " << path() << " needs "
         << net->unpacked_dimensions() << " indices,"
  ...
  if (idx_flags.invalid) {
    ...
    if (canon_index == 0) {
      cerr << get_fileline() << ": warning: "
           << "returning 'bx for out of bounds array access " << net->name()
  ...
  if (net->get_scalar() && word_sel != index_component_t::SEL_NONE) {
    cerr << get_fileline() << ": error: can not select part of ";
    if (res->expr_type() == IVL_VT_REAL)
      cerr << "real";
    else
      cerr << "scalar";
    cerr << " array word: " << net->name() << as_indices(unpacked_indices)
         << endl;
    des->errors += 1;
    delete res;
    return 0;
  ...
  if (word_sel == index_component_t::SEL_IDX_UP)
    return elaborate_expr_net_idx_up_(des, scope, res, found_in, need_const);

  if (word_sel == index_component_t::SEL_IDX_DO)
    return elaborate_expr_net_idx_do_(des, scope, res, found_in, need_const);

  if (word_sel == index_component_t::SEL_BIT)
    return elaborate_expr_net_bit_(des, scope, res, found_in, need_const);
  ...
  if (warn_ob_select) {
    if ((sb_lsb >= (signed)net->vector_width()) ||
      ...
      cerr << get_fileline()
           << ": warning: "
              "Part select "
           << "[" << msv << ":" << lsv << "] is selecting after the ";
      if (net->word_index())
        cerr << "array word ";
      else
        cerr << "vector ";
    ...
    if ((sb_msb < 0) || (sb_lsb < 0)) {
      cerr << get_fileline()
           << ": warning: "
              "Part select "
           << "[" << msv << ":" << lsv << "] is selecting before the ";
      if (net->word_index())
        cerr << "array word ";
      else
        cerr << "vector ";
      ...
      cerr << get_fileline()
           << ":        : "
              "Replacing the out of bound bits with 'bx."
           << endl;
    }
  }
...
NetExpr* PEIdent::elaborate_expr_net_idx_up_(Design* des, NetScope* scope,
    ...
    if (base_c->value().is_defined()) {
      ...
      if (warn_ob_select) {
        if (rel_base < 0) {
          ...
          if (net->word_index()) cerr << "[]";
          cerr << "[" << lsv << "+:" << wid << "] is selecting before vector."
               << endl;
        }
        if (rel_base + wid > net->vector_width()) {
          cerr << get_fileline() << ": warning: " << net->name();
          if (net->word_index()) cerr << "[]";
          cerr << "[" << lsv << "+:" << wid << "] is selecting after vector."
               << endl;
        }
      }
    } else {
      ...
      if (warn_ob_select) {
        cerr << get_fileline() << ": warning: " << net->name();
        if (net->word_index()) cerr << "[]";
        cerr << "['bx+:" << wid << "] is always outside vector." << endl;
      }
      return ex;
    }
    NetESelect* ss = new NetESelect(net, ex, wid);
  ...
  base =
      normalize_variable_part_base(prefix_indices, base, net->sig(), wid, true);
...
NetExpr* PEIdent::elaborate_expr_net_idx_do_(Design* des, NetScope* scope,
  ...
  if (NetEConst* base_c = dynamic_cast<NetEConst*>(base)) {
    ...
    if (base_c->value().is_defined()) {
      ...
      // range.
      ex = new NetEConst(
          verinum(net->sig()->sb_to_idx(prefix_indices, lsv) + offset));
      if (warn_ob_select) {
...
NetExpr* PEIdent::elaborate_expr_net_bit_(Design* des, NetScope* scope,
  ...
  if (NetEConst* msc = dynamic_cast<NetEConst*>(mux)) {
    if (debug_elaborate) {
      cerr << get_fileline() << ": PEIdent::elaborate_expr_net_bit_: "
           ...
           << ", prefix_indices.size()=" << prefix_indices.size() << endl;
    }
    ...
    if (net->vector_width() == 1) return net;

    if (debug_elaborate) {
      cerr << get_fileline() << ": PEIdent::elaborate_expr_net_bit_: "
           << "Make bit select idx=" << idx << endl;
    }
  ...
  if (prefix_indices.size() + 2 <= sig_packed.size()) {
    ...
    // here is convert to a "slice" of the vector.
    unsigned long lwid;
    mux = normalize_variable_slice_base(prefix_indices, mux, net->sig(), lwid);
    mux->set_line(*net);
...
NetExpr* PEIdent::elaborate_expr_net(Design* des, NetScope* scope, NetNet* net,
                                     NetScope* found_in, unsigned expr_wid,
                                     unsigned flags) const {
  if (debug_elaborate) {
  ...
  if (net->unpacked_dimensions() > 0)
    return elaborate_expr_net_word_(des, scope, net, found_in, expr_wid, flags);
  ...
  index_component_t::ctype_t use_sel = index_component_t::SEL_NONE;
  if (!path_.back().index.empty()) use_sel = path_.back().index.back().sel;
  ...
  list<long> prefix_indices;
  bool rc =
      evaluate_index_prefix(des, scope, prefix_indices, path_.back().index);
  if (!rc) return 0;
  ...
  if (use_sel == index_component_t::SEL_PART)
    return elaborate_expr_net_part_(des, scope, node, found_in, expr_wid);

  if (use_sel == index_component_t::SEL_IDX_UP)
    return elaborate_expr_net_idx_up_(des, scope, node, found_in, need_const);

  if (use_sel == index_component_t::SEL_IDX_DO)
    return elaborate_expr_net_idx_do_(des, scope, node, found_in, need_const);

  if (use_sel == index_component_t::SEL_BIT)
    return elaborate_expr_net_bit_(des, scope, node, found_in, need_const);

  if (use_sel == index_component_t::SEL_BIT_LAST)
    return elaborate_expr_net_bit_last_(des, scope, node, found_in, need_const);
...
NetExpr* PENewArray::elaborate_expr(Design* des, NetScope*, unsigned,
                                    unsigned) const {
  cerr << get_fileline()
       << ": error: The new array constructor may "
          "only be used in an assignment to a dynamic array."
       << endl;
  des->errors += 1;
  return 0;
}
...
NetExpr* PENewClass::elaborate_expr_constructor_(Design* des, NetScope* scope,
  ...
  if (NetScope* new1_scope =
          ctype->method_from_name(perm_string::literal("new@"))) {
  ...
  for (size_t idx = 1; idx < parms.size(); idx += 1) {
    ...
    if (idx <= parms_.size() && parms_[idx - 1]) {
      ...
      parms[idx] = elaborate_rval_expr(
          des, scope, def->port(idx)->net_type(), def->port(idx)->data_type(),
          def->port(idx)->vector_width(), tmp, false);
      if (parms[idx] == 0) parm_errors += 1;
...
NetExpr* PENewCopy::elaborate_expr(Design* des, NetScope* scope,
                                   ivl_type_t obj_type, unsigned) const {
  NetExpr* copy_arg = src_->elaborate_expr(des, scope, obj_type, 0);
  if (copy_arg == 0) return 0;
...
NetExpr* PENull::elaborate_expr(Design*, NetScope*, ivl_type_t,
                                unsigned) const {
  NetENull* tmp = new NetENull;
  tmp->set_line(*this);
  return tmp;
...
unsigned PENumber::test_width(Design*, NetScope*, width_mode_t& mode) {
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PENumber::test_width: "
         << "Value=" << *value_ << ", width=" << expr_width_
...
NetExpr* PENumber::elaborate_expr(Design* des, NetScope*, ivl_type_t ntype,
                                  unsigned) const {
  ...
  if (use_type == 0) {
    cerr << get_fileline() << ": internal error: "
         << "I don't know how cast numbers to this type." << endl;
    des->errors += 1;
    return 0;
  }
...
NetEConst* PENumber::elaborate_expr(Design*, NetScope*, unsigned expr_wid,
                                    unsigned) const {
  assert(value_);
  verinum val = *value_;
  if (val.has_len()) val.has_sign(signed_flag_);
...
NetEConst* PEString::elaborate_expr(Design*, NetScope*, ivl_type_t,
                                    unsigned) const {
  verinum val(value());
  NetEConst* tmp = new NetEConst(val);
  tmp->cast_signed(signed_flag_);
...
NetEConst* PEString::elaborate_expr(Design*, NetScope*, unsigned expr_wid,
                                    unsigned) const {
  verinum val(value());
...
unsigned PETernary::test_width(Design* des, NetScope* scope,
  ...
  if (debug_elaborate)
    cerr << get_fileline() << ": debug: "
         ...
         << ", width=" << expr_width_ << " (tru_type=" << tru_type
...
bool NetETernary::test_operand_compat(ivl_variable_type_t l,
  ...
  if (l == IVL_VT_LOGIC && r == IVL_VT_BOOL) return true;
  if (l == IVL_VT_BOOL && r == IVL_VT_LOGIC) return true;

  if (l == IVL_VT_REAL && (r == IVL_VT_LOGIC || r == IVL_VT_BOOL)) return true;
  if (r == IVL_VT_REAL && (l == IVL_VT_LOGIC || l == IVL_VT_BOOL)) return true;
...
NetExpr* PETernary::elaborate_expr(Design* des, NetScope* scope,
  ...
  if (NetEConst* tmp = dynamic_cast<NetEConst*>(con)) {
    ...
    if (cval.get(0) == verinum::V1) {
      ...
      return elab_and_eval_alternative_(des, scope, tru_, expr_wid, flags,
                                        true);
    }
    ...
    if (cval.get(0) == verinum::V0) {
      ...
      return elab_and_eval_alternative_(des, scope, fal_, expr_wid, flags,
                                        true);
    }
  ...
  NetExpr* tru =
      elab_and_eval_alternative_(des, scope, tru_, expr_wid, flags, false);
  if (tru == 0) {
    delete con;
    return 0;
  }
  ...
  NetExpr* fal =
      elab_and_eval_alternative_(des, scope, fal_, expr_wid, flags, false);
  if (fal == 0) {
    delete con;
    delete tru;
    return 0;
  ...
  if (!NetETernary::test_operand_compat(tru->expr_type(), fal->expr_type())) {
    cerr << get_fileline() << ": error: Data types " << tru->expr_type()
         << " and " << fal->expr_type() << " of ternary"
         << " do not match." << endl;
    des->errors += 1;
    return 0;
  }
...
NetExpr* PETernary::elab_and_eval_alternative_(Design* des, NetScope* scope,
  ...
  if (type_is_vectorable(expr->expr_type()) &&
      !type_is_vectorable(expr_type_)) {
...
NetExpr* PETypename::elaborate_expr(Design* des, NetScope*, ivl_type_t,
                                    unsigned) const {
  cerr << get_fileline() << ": error: Type name not a valid expression here."
       << endl;
  des->errors += 1;
  return 0;
}
...
unsigned PEUnary::test_width(Design* des, NetScope* scope, width_mode_t& mode) {
  switch (op_) {
    ...
    case '!': {
      ...
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "Test width of sub-expression of " << op_ << " returns "
             << sub_width << "." << endl;
    }
      return expr_width_;
  }
  ...
  switch (op_) {
    ...
    case 'd':
      ...
      if (expr_wid != expr_->expr_width()) {
         ...
         */
        cerr << get_fileline() << ": sorry: " << human_readable_op(op_, true)
             << " operation is not yet supported on "
             << "vector slice." << endl;
        des->errors += 1;
        return 0;
      } else if (t == IVL_VT_LOGIC || t == IVL_VT_BOOL || t == IVL_VT_REAL) {
        if (dynamic_cast<NetEConst*>(ip) || dynamic_cast<NetECReal*>(ip)) {
          /*
           * invalid operand: operand is a constant
           * or real number
           */
          cerr << get_fileline() << ": error: "
               << "inappropriate use of " << human_readable_op(op_, true)
               << " operator." << endl;
          des->errors += 1;
          return 0;
        }
    ...
    case '!':  // Logical NOT
      ...
      if (NetEConst* ipc = dynamic_cast<NetEConst*>(ip)) {
        ...
        unsigned v1 = 0;
        unsigned vx = 0;
        for (unsigned idx = 0; idx < val.len(); idx += 1) switch (val[idx]) {
            case verinum::V0:
              break;
            case verinum::V1:
              v1 += 1;
    ...
    case 'X':  // Reduction NXOR (~^)
      if (ip->expr_type() == IVL_VT_REAL) {
        cerr << get_fileline() << ": error: " << human_readable_op(op_, true)
...
NetExpr* PEUnary::elaborate_expr_bits_(NetExpr* operand,
                                       unsigned expr_wid) const {
...
NetNet* Design::find_discipline_reference(ivl_discipline_t dis,
                                          NetScope* scope) {
