...
class IslandBranch {
 public:
  explicit IslandBranch(ivl_discipline_t dis = 0)
      : island_(0), discipline_(dis) {}
...
class NexusSet {
 public:
  struct elem_t {
    inline elem_t(Nexus* n, unsigned b, unsigned w) : base(b), wid(w) {
...
class NetDelaySrc : public NetObj {
 public:
  ...
  void set_delays(uint64_t t01, uint64_t t10, uint64_t t0z, uint64_t tz1,
                  uint64_t t1z, uint64_t tz0);
  void set_delays(uint64_t t01, uint64_t t10, uint64_t t0z, uint64_t tz1,
                  uint64_t t1z, uint64_t tz0, uint64_t t0x, uint64_t tx1,
                  uint64_t t1x, uint64_t tx0, uint64_t txz, uint64_t tzx);
...
class NetNet : public NetObj, public PortType {
 ...
 public:
  ...
  explicit NetNet(NetScope* s, perm_string n, Type t,
                  const std::list<netrange_t>& unpacked, ivl_type_t type);
  ...
  const std::vector<netrange_t>& unpacked_dims() const {
    return unpacked_dims_;
  }
  ...
  /* This method converts a partial packed indices list and a
     ...
     width. */
  bool sb_to_slice(const std::list<long>& prefix, long sb, long& off,
...
class NetScope : public Definitions, public Attrib {
 public:
  ...
  const netenum_t* find_enumeration_for_name(const Design* des,
                                             perm_string name);
  ...
  struct range_t;
  void set_parameter(perm_string name, bool is_annotatable, PExpr* val,
                     ivl_variable_type_t type, PExpr* msb, PExpr* lsb,
                     bool signed_flag, bool local_flag,
                     NetScope::range_t* range_list, const LineInfo& file_line);
  void set_parameter(perm_string name, NetExpr* val, const LineInfo& file_line);

  const NetExpr* get_parameter(Design* des, const char* name,
                               const NetExpr*& msb, const NetExpr*& lsb);
  ...
  void set_line(perm_string file, perm_string def_file, unsigned lineno,
                unsigned def_lineno);
  ...
  */
  ...
  void add_module_port_info(unsigned idx,
                            ...
                            PortType::Enum type, unsigned long width);
  ...
  virtual bool check_synth(ivl_process_type_t pr_type,
                           const NetScope* scope) const;
...
class NetCLShift : public NetNode {
 public:
  NetCLShift(NetScope* s, perm_string n, unsigned width, unsigned width_dist,
...
class NetDivide : public NetNode {
 public:
  NetDivide(NetScope* scope, perm_string n, unsigned width, unsigned wa,
            unsigned wb);
  ~NetDivide();
...
class NetModulo : public NetNode {
 public:
  NetModulo(NetScope* s, perm_string n, unsigned width, unsigned wa,
            unsigned wb);
  ~NetModulo();
...
class NetMult : public NetNode {
 public:
  NetMult(NetScope* sc, perm_string n, unsigned width, unsigned wa,
          unsigned wb);
  ~NetMult();
...
class NetMux : public NetNode {
 public:
  NetMux(NetScope* scope, perm_string n, unsigned width, unsigned size,
         unsigned selw);
  ~NetMux();
...
class NetPow : public NetNode {
 public:
  NetPow(NetScope* sc, perm_string n, unsigned width, unsigned wa, unsigned wb);
  ~NetPow();
...
class NetSysFunc : public NetNode {
 public:
  NetSysFunc(NetScope* s, perm_string n, const struct sfunc_return_type* def,
...
class NetTran : public NetNode, public IslandBranch {
 public:
  ...
  NetTran(NetScope* scope, perm_string n, ivl_switch_type_t type, unsigned wid);
  // Create a TRAN_VP
  NetTran(NetScope* scope, perm_string n, unsigned wid, unsigned part,
          unsigned off);
  ~NetTran();
...
class NetExpr : public LineInfo {
 public:
  ...
  virtual bool is_constant_expr() const { return false; }
  virtual bool is_signal_expr() const { return false; }
  virtual bool is_binary_expr() const { return false; }
  virtual bool is_unary_expr() const { return false; }
  virtual bool is_ternary_expr() const { return false; }
...
class NetEConst : public NetExpr {
 public:
  ...
  bool is_constant_expr() const override { return true; }
...
class NetPartSelect : public NetNode {
 public:
  ...
  explicit NetPartSelect(NetNet* sig, unsigned off, unsigned wid, dir_t dir,
                         bool signed_flag__ = false);
  explicit NetPartSelect(NetNet* sig, NetNet* sel, unsigned wid,
                         bool signed_flag__ = false);
  ~NetPartSelect();
...
class NetLogic : public NetNode {
 public:
  ...
  explicit NetLogic(NetScope* s, perm_string n, unsigned pins, TYPE t,
                    unsigned wid, bool is_cassign__ = false);
...
class NetProc : public virtual LineInfo {
 public:
  ...
  virtual bool synth_async(Design* des, NetScope* scope, NexusSet& nex_map,
  ...
  virtual bool synth_sync(Design* des, NetScope* scope, bool& ff_negedge,
                          NetNet* ff_clock, NetBus& ff_ce, NetBus& ff_aclr,
  ...
  virtual bool is_net_delay() const { return false; }
...
class NetAssign_ {
 public:
  ...
  void set_part(NetExpr* loff, unsigned wid, ivl_select_type_t = IVL_SEL_OTHER);
...
class NetAssignBase : public NetProc {
 public:
  ...
  bool synth_async(Design* des, NetScope* scope, NexusSet& nex_map,
                   NetBus& nex_out, NetBus& enables, vector<mask_t>& bitmasks);
...
class NetAssign : public NetAssignBase {
 ...
 private:
  void eval_func_lval_op_real_(const LineInfo& loc, verireal& lv,
                               verireal& rv) const;
...
class NetBlock : public NetProc {
 public:
  ...
  bool synth_sync(Design* des, NetScope* scope, bool& ff_negedge,
                  NetNet* ff_clk, NetBus& ff_ce, NetBus& ff_aclr,
                  NetBus& ff_aset, vector<verinum>& ff_aset_value,
                  NexusSet& nex_map, NetBus& nex_out, vector<mask_t>& bitmasks,
                  const std::vector<NetEvProbe*>& events);
...
class NetCase : public NetProc {
 public:
  ...
  inline const NetProc* stat(unsigned idx) const {
    return items_[idx].statement;
  }
 ...
 private:
  ...
  bool synth_async_casez_(Design* des, NetScope* scope, NexusSet& nex_map,
                          NetBus& nex_out, NetBus& enables,
                          vector<mask_t>& bitmasks);
...
class NetCondit : public NetProc {
 public:
  ...
  bool is_condit() const override { return true; }
...
class NetDoWhile : public NetProc {
 public:
  NetDoWhile(NetExpr* c, NetProc* p) : cond_(c), proc_(p) {}
...
class NetEvWait : public NetProc {
 public:
  ...
  void set_statement(NetProc* statement) { statement_ = statement; }
  ...
  bool is_event_wait() const override { return true; }
...
class NetEvProbe : public NetNode {
 ...
 public:
  ...
  explicit NetEvProbe(NetScope* s, perm_string n, NetEvent* tgt, edge_t t,
                      unsigned p);
  ~NetEvProbe();
...
class NetFuncDef : public NetBaseDef {
 public:
  ...
  NetExpr* evaluate_function(const LineInfo& loc,
                             const std::vector<NetExpr*>& args) const;
...
class NetSTask : public NetProc {
 public:
  ...
  bool is_system_task() const override { return true; }
...
class NetWhile : public NetProc {
 public:
  NetWhile(NetExpr* c, NetProc* p) : cond_(c), proc_(p) {}
...
class NetProcTop : public LineInfo, public Attrib {
 ...
 private:
  bool tie_off_floating_inputs_(Design* des, NexusSet& nex_map, NetBus& nex_in,
...
class NetEBinary : public NetExpr {
 public:
  ...
  bool is_binary_expr() const override { return true; }
...
class NetEBComp : public NetEBinary {
 ...
 private:
  NetEConst* must_be_leeq_(const NetExpr* le, const verinum& rv,
                           bool eq_flag) const;

  NetEConst* eval_arguments_(const NetExpr* le, const NetExpr* re) const;
  NetEConst* eval_eqeq_(bool ne_flag, const NetExpr* le,
                        const NetExpr* re) const;
  NetEConst* eval_eqeq_real_(bool ne_flag, const NetExpr* le,
                             const NetExpr* re) const;
  NetEConst* eval_less_(const NetExpr* le, const NetExpr* re) const;
  NetEConst* eval_leeq_(const NetExpr* le, const NetExpr* re) const;
  NetEConst* eval_leeq_real_(const NetExpr* le, const NetExpr* ri,
                             bool eq_flag) const;
  NetEConst* eval_gt_(const NetExpr* le, const NetExpr* re) const;
  NetEConst* eval_gteq_(const NetExpr* le, const NetExpr* re) const;
  NetEConst* eval_eqeqeq_(bool ne_flag, const NetExpr* le,
                          const NetExpr* re) const;
  NetEConst* eval_weqeq_(bool ne_flag, const NetExpr* le,
                         const NetExpr* re) const;
};
...
class NetESFunc : public NetExpr {
 public:
  NetESFunc(const char* name, ivl_variable_type_t t, unsigned width,
            unsigned nprms, bool is_overridden = false);
 ...
 private:
  ...
  NetEConst* evaluate_array_funcs_(ID id, const NetExpr* arg0,
                                   const NetExpr* arg1) const;
...
class NetETernary : public NetExpr {
 public:
  NetETernary(NetExpr* c, NetExpr* t, NetExpr* f, unsigned wid,
              bool signed_flag);
  ~NetETernary();
  ...
  bool is_ternary_expr() const override { return true; }

 public:
  static bool test_operand_compat(ivl_variable_type_t tru,
                                  ivl_variable_type_t fal);
...
class NetEUnary : public NetExpr {
 public:
  ...
  bool is_unary_expr() const override { return true; }
...
class NetESignal : public NetExpr {
 public:
  ...
  bool is_signal_expr() const override { return true; }
...
struct elaborator_work_item_t {
  explicit elaborator_work_item_t(Design* d) : des(d) {}
...
class Design {
 public:
  ...
  void identify_clock(void);
  void do_always_clock_opt(void);
...
inline bool operator==(const Link& l, const Link& r) { return l.is_equal(r); }
...
inline bool connected(const Link& l, const Link& r) { return l.is_linked(r); }
...
extern bool nexus_overflow_flag;
