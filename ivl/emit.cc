...
bool NetTran::emit_node(struct target_t* tgt) const { return tgt->tran(this); }

bool NetBUFZ::emit_node(struct target_t* tgt) const { return tgt->bufz(this); }

bool NetProcTop::emit(struct target_t* tgt) const { return tgt->process(this); }
...
void NetForever::emit_recurse(struct target_t* tgt) const {
  if (statement_) statement_->emit_proc(tgt);
}
...
int Design::emit(struct target_t* tgt) const {
  ...
  if (tgt->start_design(this) == false) return -2;
  ...
  if (nodes_rc == false) tgt->errors += 1;
  if (tasks_rc == false) tgt->errors += 1;
  if (proc_rc == false) tgt->errors += 1;
  if (branches_rc == false) tgt->errors += 1;

  rc = tgt->end_design(this);

  if (nodes_rc == false) return -1;
  if (tasks_rc == false) return -2;
  if (proc_rc == false) return -3;
  if (branches_rc == false) return -4;
...
void NetENew::expr_scan(struct expr_scan_t* tgt) const { tgt->expr_new(this); }
