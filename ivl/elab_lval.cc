 ...
 * NetAssign_ object.
 */
NetAssign_* PEConcat::elaborate_lval(Design* des, NetScope* scope,
                                     bool is_cassign, bool is_force) const {
  if (repeat_) {
    cerr << get_fileline()
         << ": error: Repeat concatenations make "
            "no sense in l-value expressions. I refuse."
         << endl;
    des->errors += 1;
    return 0;
  }
  ...
  for (unsigned idx = 0; idx < parms_.size(); idx += 1) {
    ...
    NetAssign_* tmp =
        parms_[idx]->elaborate_lval(des, scope, is_cassign, is_force);
 ...
 * of that ident.
 */
NetAssign_* PEIdent::elaborate_lval(Design* des, NetScope* scope,
                                    bool is_cassign, bool is_force) const {
  NetNet* reg = 0;
  const NetExpr* par = 0;
  NetEvent* eve = 0;
  ...
  if (reg == 0) {
    if (use_scope->type() == NetScope::FUNC &&
      ...
      cerr << get_fileline() << ": error: "
           << "Cannot assign to " << path_ << " because function "
           << scope_path(use_scope) << " is void." << endl;
    } else {
      cerr << get_fileline() << ": error: Could not find variable ``" << path_
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PEIdent::elaborate_lval: "
         << "Found l-value path_=" << path_ << " as reg=" << reg->name()
         << " base_path=" << base_path << ", member_path=" << member_path
  ...
  if (reg->unpacked_dimensions() > name_tail.index.size()) {
    cerr << get_fileline() << ": error: Cannot assign to array " << path_
  ...
  if ((reg->type() != NetNet::REG) &&
      (reg->type() != NetNet::UNRESOLVED_WIRE) && !is_force) {
  ...
  if (reg->class_type() && member_path.size() > 0 && gn_system_verilog()) {
    NetAssign_* lv =
        elaborate_lval_net_class_member_(des, use_scope, reg, member_path);
    return lv;
  }
  ...
  bool need_const_idx =
      is_cassign || is_force || (reg->type() == NetNet::UNRESOLVED_WIRE);

  if (reg->unpacked_dimensions() > 0)
    return elaborate_lval_net_word_(des, scope, reg, need_const_idx);

  // This must be after the array word elaboration above!
  if (reg->get_scalar() && use_sel != index_component_t::SEL_NONE) {
  ...
  if (reg->type() == NetNet::UNRESOLVED_WIRE && !is_force) {
    cerr << get_fileline() << ": error: " << path_
         << " Unable to assign to unresolved wires." << endl;
    des->errors += 1;
    return 0;
  }
...
NetAssign_* PEIdent::elaborate_lval_method_class_member_(
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PEIdent::elaborate_lval_method_class_member_: "
         << "Ident " << member_name << " is a property of class "
  ...
  if (!name_comp.index.empty()) {
    ...
    if (const netsarray_t* stype =
            dynamic_cast<const netsarray_t*>(property_type)) {
      canon_index =
          make_canonical_index(des, scope, this, name_comp.index, stype, false);
...
NetAssign_* PEIdent::elaborate_lval_net_word_(Design* des, NetScope* scope,
                                              NetNet* reg,
  ...
  if (name_tail.index.size() < reg->unpacked_dimensions()) {
    cerr << get_fileline() << ": error: Array " << reg->name() << " needs "
         << reg->unpacked_dimensions() << " indices,"
  ...
  indices_to_expressions(des, scope, this, name_tail.index,
                         reg->unpacked_dimensions(), false, flags,
  ...
  if (flags.invalid) {
    ...
    cerr << get_fileline() << ": warning: "
         << "ignoring undefined l-value array access " << reg->name()
    ...
    ivl_assert(*this,
               unpacked_indices_const.size() == reg->unpacked_dimensions());
  ...
  if (canon_index == 0) canon_index = new NetEConst(verinum(verinum::Vx));
  ...
  if (debug_elaborate)
    cerr << get_fileline() << ": debug: Set array word=" << *canon_index
         << endl;
  ...
  if (reg->get_scalar() && use_sel != index_component_t::SEL_NONE) {
    ...
    if (reg->data_type() == IVL_VT_REAL)
      ...
      cerr << "scalar";
    cerr << " array word: " << reg->name() << as_indices(unpacked_indices)
         << endl;
    des->errors += 1;
    return 0;
  }
...
bool PEIdent::elaborate_lval_net_bit_(Design* des, NetScope* scope,
                                      NetAssign_* lv,
                                      bool need_const_idx) const {
  ...
  if (NetEConst* index_con = dynamic_cast<NetEConst*>(mux)) {
    ...
    if (index_con->value().is_defined()) {
      ...
      ivl_assert(*this, rcl);
      if (warn_ob_select) {
        cerr << get_fileline() << ": warning: L-value packed array select of "
             << reg->name();
  ...
  if (debug_elaborate && (reg->type() == NetNet::UNRESOLVED_WIRE)) {
    cerr << get_fileline() << ": PEIdent::elaborate_lval_net_bit_: "
         << "Try to assign bits of unresolved wire." << endl;
  }
  ...
  if (prefix_indices.size() + 2 <= reg->packed_dims().size()) {
    ...
    if (mux == 0) {
      ...
      unsigned long lwid;
      mux = normalize_variable_slice_base(prefix_indices, mux, reg, lwid);
    ...
    if (need_const_idx) {
      cerr << get_fileline() << ": error: '" << reg->name()
           << "' bit select must be a constant in this context." << endl;
      des->errors += 1;
      return false;
    }
  ...
  } else if (reg->vector_width() == 1 &&
             reg->sb_is_valid(prefix_indices, lsb)) {
    ...
    if (warn_ob_select && (loff < 0 || loff >= (long)reg->vector_width())) {
      cerr << get_fileline() << ": warning: bit select " << reg->name() << "["
           << lsb << "]"
...
bool PEIdent::elaborate_lval_darray_bit_(Design* des, NetScope* scope,
                                         NetAssign_* lv) const {
  const name_component_t& name_tail = path_.back();
  ivl_assert(*this, !name_tail.index.empty());
  ...
  if (lv->sig()->type() == NetNet::UNRESOLVED_WIRE) {
    cerr << get_fileline() << ": error: " << path_
         << " Unable to darray word select unresolved wires." << endl;
    des->errors += 1;
    return false;
  }
...
bool PEIdent::elaborate_lval_net_part_(Design* des, NetScope* scope,
                                       NetAssign_* lv) const {
  list<long> prefix_indices;
  bool rc = calculate_packed_indices_(des, scope, lv->sig(), prefix_indices);
  ivl_assert(*this, rc);
  ...
  if (reg->type() == NetNet::UNRESOLVED_WIRE) {
    ...
    if (rct) {
      cerr << get_fileline() << ": error: " << path_
           << "Part select is double-driving unresolved wire." << endl;
      des->errors += 1;
      return false;
    }
  }
  ...
  if (prefix_indices.size() + 1 < packed.size()) {
    ...
    if (moff < loff) {
      cerr << get_fileline() << ": error: part select " << reg->name() << "["
           << msb << ":" << lsb << "]"
           << " is reversed." << endl;
      des->errors += 1;
      return false;
  ...
  if (warn_ob_select && (loff < 0 || moff >= (long)reg->vector_width())) {
    cerr << get_fileline() << ": warning: Part select " << reg->name() << "["
         << msb << ":" << lsb << "]"
         << " is out of range." << endl;
  }
...
bool PEIdent::elaborate_lval_net_idx_(Design* des, NetScope* scope,
                                      NetAssign_* lv,
  ...
  if (NetEConst* base_c = dynamic_cast<NetEConst*>(base)) {
    ...
    if (need_const_idx) {
      cerr << get_fileline() << ": error: '" << reg->name()
           << "' base index must be a constant in this context." << endl;
      des->errors += 1;
      return false;
    }
    ...
    if (use_sel == index_component_t::SEL_IDX_UP) {
      base = normalize_variable_part_base(prefix_indices, base, reg, wid, true);
      sel_type = IVL_SEL_IDX_UP;
    } else {
      // This is assumed to be a SEL_IDX_DO.
      base =
          normalize_variable_part_base(prefix_indices, base, reg, wid, false);
      sel_type = IVL_SEL_IDX_DOWN;
    }
  }

  if (debug_elaborate)
    cerr << get_fileline() << ": debug: Set part select width=" << wid
         << ", base=" << *base << endl;
 ...
 */
NetAssign_* PEIdent::elaborate_lval_net_class_member_(
    Design* des, NetScope* scope, NetNet* sig, pform_name_t member_path) const {
  if (debug_elaborate) {
    cerr << get_fileline() << ": PEIdent::elaborate_lval_net_class_member_: "
         << "l-value is property " << member_path << " of " << sig->name()
         << "." << endl;
  }
  ...
  do {
    ...
    if (debug_elaborate) {
      cerr << get_fileline() << ": PEIdent::elaborate_lval_net_class_member_: "
           << "Processing member_cur=" << member_cur << endl;
    }
    ...
    if (mtype) {
      if (!member_cur.index.empty()) {
        cerr << get_fileline() << ": sorry: "
             << "Array index of array properties not supported." << endl;
        des->errors += 1;
      }
    }
 ...
 */
bool PEIdent::elaborate_lval_net_packed_member_(
    Design* des, NetScope* scope, NetAssign_* lv,
    pform_name_t member_path) const {
  if (debug_elaborate) {
    cerr << get_fileline() << ": PEIdent::elaborate_lval_net_packed_member_: "
         << "path_=" << path_ << " member_path=" << member_path << endl;
  }
  ...
  for (size_t idx = 1; idx < member_path.size(); idx += 1) ++name_idx;
  if (name_idx->name != peek_head_name(member_path)) {
    cerr << get_fileline() << ": internal error: "
         << "name_idx=" << name_idx->name
         << ", expecting member_name=" << peek_head_name(member_path) << endl;
    des->errors += 1;
    return false;
  }
  ...
  do {
    ...
    if (member == 0) {
      cerr << get_fileline() << ": error: Member " << member_name
           << " is not a member of struct type of " << reg->name() << "."
           << completed_path << endl;
      des->errors += 1;
      return false;
    }
    if (debug_elaborate) {
      cerr << get_fileline() << ": PEIdent::elaborate_lval_net_packed_member_: "
           << "Member type: " << *(member->net_type) << endl;
    }
    ...
    index_component_t::ctype_t use_sel = index_component_t::SEL_NONE;
    if (!member_comp.index.empty()) use_sel = member_comp.index.back().sel;

    if (use_sel != index_component_t::SEL_NONE &&
        use_sel != index_component_t::SEL_BIT &&
        use_sel != index_component_t::SEL_PART) {
      cerr << get_fileline()
           << ": sorry: Assignments to part selects of "
              "a struct member are not yet supported."
           << endl;
      des->errors += 1;
      return false;
    }

    if (const netvector_t* mem_vec =
      ...
      if (member_comp.index.size() > 0) {
        ...
        if (debug_elaborate) {
          cerr << get_fileline()
               ...
               << " (netvector_t context)" << endl;
        }

        long tail_off = 0;
        unsigned long tail_wid = 0;
        rc = calculate_part(this, des, scope, member_comp.index.back(),
                            tail_off, tail_wid);
        ivl_assert(*this, rc);
    ...
    } else if (const netstruct_t* tmp_struct =
  ...
  if (reg->type() == NetNet::UNRESOLVED_WIRE) {
    cerr << get_fileline() << ": error: " << path_
         << " Unable to member-select unresolved wires." << endl;
    des->errors += 1;
    return false;
  }
  ...
  cerr << get_fileline() << ": internal error: "
       << "I don't know how to handle this index expression? " << *packed_base
       << endl;
  ivl_assert(*this, 0);
  return false;
}
