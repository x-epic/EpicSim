...
NetNet* PExpr::elaborate_lnet(Design*, NetScope*) const {
  cerr << get_fileline() << ": error: "
       << "expression not valid in assign l-value: " << *this << endl;
  return 0;
}
...
PEAssignPattern::PEAssignPattern(const list<PExpr*>& p) : parms_(p.size()) {
  size_t idx = 0;
  for (list<PExpr*>::const_iterator cur = p.begin(); cur != p.end(); ++cur) {
    parms_[idx] = *cur;
    idx += 1;
  }
}
...
PECastSize::PECastSize(PExpr* si, PExpr* b) : size_(si), base_(b) {}

PECastSize::~PECastSize() {}

PECastType::PECastType(data_type_t* t, PExpr* b) : target_(t), base_(b) {}
...
PECallFunction::PECallFunction(PPackage* pkg, const pform_name_t& n,
                               const vector<PExpr*>& parms)
...
PECallFunction::PECallFunction(PPackage* pkg, perm_string n,
  ...
  for (list<PExpr*>::const_iterator idx = parms.begin(); idx != parms.end();
       ++idx)
    parms_[tmp_idx++] = *idx;
}
...
void PECallFunction::declare_implicit_nets(LexicalScope* scope,
                                           NetNet::Type type) {
...
PEConcat::PEConcat(const list<PExpr*>& p, PExpr* r)
  ...
  for (list<PExpr*>::const_iterator idx = p.begin(); idx != p.end(); ++idx)
    parms_[tmp_idx++] = *idx;
...
bool PEConcat::has_aa_term(Design* des, NetScope* scope) const {
  ...
  if (repeat_) flag = repeat_->has_aa_term(des, scope) || flag;
...
void PEIdent::declare_implicit_nets(LexicalScope* scope, NetNet::Type type) {
  ...
  if (no_implicit_sig_) return;
  if ((path_.size() == 1) && (path_.front().index.size() == 0)) {
    perm_string name = path_.front().name;
    LexicalScope* ss = scope;
    while (ss) {
      if (ss->wires.find(name) != ss->wires.end()) return;
      if (ss->localparams.find(name) != ss->localparams.end()) return;
      if (ss->parameters.find(name) != ss->parameters.end()) return;
      if (ss->genvars.find(name) != ss->genvars.end()) return;
      if (ss->events.find(name) != ss->events.end()) return;
...
PENewArray::~PENewArray() { delete size_; }

PENewClass::PENewClass(void) {}

PENewClass::PENewClass(const list<PExpr*>& p) : parms_(p.size()) {
  size_t tmp_idx = 0;
  for (list<PExpr*>::const_iterator cur = p.begin(); cur != p.end(); ++cur) {
    parms_[tmp_idx++] = *cur;
  }
...
PENewCopy::PENewCopy(PExpr* src) : src_(src) {}
...
PETypename::PETypename(data_type_t* dt) : data_type_(dt) {}
