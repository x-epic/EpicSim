...
void PGate::elaborate(Design*, NetScope*) const {
  cerr << "internal error: what kind of gate? " << typeid(*this).name() << endl;
}
...
void PGAssign::elaborate(Design* des, NetScope* scope) const {
  ...
  NetExpr* rval_expr =
      elaborate_rval_expr(des, scope, lval->net_type(), lval->data_type(),
                          lval->vector_width(), pin(1));

  if (rval_expr == 0) {
    cerr << get_fileline()
         << ": error: Unable to elaborate r-value: " << *pin(1) << endl;
    des->errors += 1;
    return;
  }
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": debug: PGAssign: elaborated r-value"
         << " width=" << rval->vector_width() << ", type=" << rval->data_type()
         << ", expr=" << *rval_expr << endl;
  }
  ...
  if (dynamic_cast<NetESignal*>(rval_expr)) need_driver_flag = true;
  ...
  if (lval->vector_width() < rval->vector_width()) {
    NetPartSelect* tmp =
        new NetPartSelect(rval, 0, lval->vector_width(), NetPartSelect::VP);
    des->add_node(tmp);
    tmp->set_line(*this);
    netvector_t* osig_vec =
        new netvector_t(rval->data_type(), lval->vector_width() - 1, 0);
    NetNet* osig =
        new NetNet(scope, scope->local_symbol(), NetNet::TRI, osig_vec);
    osig->set_line(*this);
  ...
  if (need_driver_flag) {
    NetBUFZ* driver =
        new NetBUFZ(scope, scope->local_symbol(), rval->vector_width(), false);
    ...
    netvector_t* tmp_vec =
        new netvector_t(rval->data_type(), rval->vector_width() - 1, 0);
    NetNet* tmp =
        new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp_vec);
    tmp->set_line(*this);
    tmp->local_flag(true);
  ...
  if (lval->local_flag()) delete lval;
}

void PGAssign::elaborate_unpacked_array_(Design* des, NetScope* scope,
                                         NetNet* lval) const {
  ...
  switch (type()) {
    case AND:
      if (pin_count() < 2) {
        cerr << get_fileline()
             << ": error: the AND "
                "primitive must have an input."
             << endl;
        des->errors += 1;
      } else {
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::AND,
                            instance_width);
      }
      break;
    ...
    case BUF:
      if (pin_count() < 2) {
        ...
        gate = new NetLogic(scope, inst_name, 2, NetLogic::BUF, instance_width);
      }
      break;
    ...
    case BUFIF0:
      if (pin_count() != 3) {
        cerr << get_fileline()
             << ": error: the BUFIF0 "
                "primitive must have three arguments."
             << endl;
        des->errors += 1;
      } else {
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::BUFIF0,
                            instance_width);
      }
      break;
    ...
    case BUFIF1:
      if (pin_count() != 3) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::BUFIF1,
                            instance_width);
      }
      break;
    ...
    case CMOS:
      if (pin_count() != 4) {
        cerr << get_fileline()
             << ": error: the CMOS "
                "primitive must have four arguments."
             << endl;
        des->errors += 1;
      } else {
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::CMOS,
                            instance_width);
      }
      break;
    ...
    case NAND:
      if (pin_count() < 2) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::NAND,
                            instance_width);
      }
      break;
    ...
    case NMOS:
      if (pin_count() != 3) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::NMOS,
                            instance_width);
      }
      break;
    ...
    case NOR:
      if (pin_count() < 2) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::NOR,
                            instance_width);
      }
      break;
    ...
    case NOT:
      if (pin_count() < 2) {
        ...
        gate = new NetLogic(scope, inst_name, 2, NetLogic::NOT, instance_width);
      }
      break;
    ...
    case NOTIF0:
      if (pin_count() != 3) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::NOTIF0,
                            instance_width);
      }
      break;
    ...
    case NOTIF1:
      if (pin_count() != 3) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::NOTIF1,
                            instance_width);
      }
      break;
    ...
    case OR:
      if (pin_count() < 2) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::OR,
                            instance_width);
      }
      break;
    ...
    case RCMOS:
      if (pin_count() != 4) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::RCMOS,
                            instance_width);
      }
      break;
    ...
    case RNMOS:
      if (pin_count() != 3) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::RNMOS,
                            instance_width);
      }
      break;
    ...
    case RPMOS:
      if (pin_count() != 3) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::RPMOS,
                            instance_width);
      }
      break;
    ...
    case PMOS:
      if (pin_count() != 3) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::PMOS,
                            instance_width);
      }
      break;
    ...
    case PULLDOWN:
      gate =
          new NetLogic(scope, inst_name, 1, NetLogic::PULLDOWN, instance_width);
      break;

    case PULLUP:
      gate =
          new NetLogic(scope, inst_name, 1, NetLogic::PULLUP, instance_width);
      break;
    ...
    case XNOR:
      if (pin_count() < 2) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::XNOR,
                            instance_width);
      }
      break;
    ...
    case XOR:
      if (pin_count() < 2) {
        ...
        gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::XOR,
                            instance_width);
      }
      break;
    ...
    case TRAN:
      if (pin_count() != 2) {
        ...
        gate = new NetTran(scope, inst_name, IVL_SW_TRAN, instance_width);
      }
      break;
    ...
    case RTRAN:
      if (pin_count() != 2) {
        ...
        gate = new NetTran(scope, inst_name, IVL_SW_RTRAN, instance_width);
      }
      break;
    ...
    case TRANIF0:
      if (pin_count() != 3) {
        ...
        gate = new NetTran(scope, inst_name, IVL_SW_TRANIF0, instance_width);
      }
      break;
    ...
    case RTRANIF0:
      if (pin_count() != 3) {
        ...
        gate = new NetTran(scope, inst_name, IVL_SW_RTRANIF0, instance_width);
      }
      break;
    ...
    case TRANIF1:
      if (pin_count() != 3) {
        ...
        gate = new NetTran(scope, inst_name, IVL_SW_TRANIF1, instance_width);
      }
      break;
    ...
    case RTRANIF1:
      if (pin_count() != 3) {
        ...
        gate = new NetTran(scope, inst_name, IVL_SW_RTRANIF1, instance_width);
      }
      break;
...
void PGBuiltin::elaborate(Design* des, NetScope* scope) const {
  ...
  for (unsigned idx = 0; idx < lval_count; idx += 1) {
    if (pin(idx) == 0) {
      cerr << get_fileline()
           << ": error: Logic gate port "
              "expressions are not optional."
           << endl;
      des->errors += 1;
      return;
    }
    ...
    ivl_assert(*this, idx == 0 || lval_sigs[idx]->vector_width() ==
                                      lval_sigs[0]->vector_width());
  }
  ...
  for (unsigned idx = 0; idx < array_count * gate_count; idx += 1) {
    ...
    for (unsigned adx = 0; adx < attrib_list_n; adx += 1)
      cur[idx]->attribute(attrib_list[adx].key, attrib_list[adx].val);
  ...
  for (unsigned idx = 0; idx < pin_count(); idx += 1) {
    ...
    if (idx < lval_count) {
      ...
      if (msb_ == 0 && tmp->expr_width() != 1)
        tmp = new NetESelect(tmp, make_const_0(1), 1, IVL_SEL_IDX_UP);
    ...
    if (array_count == 1) {
      ...
      if (1 == sig->vector_width() && instance_width != 1) {
        ...
        NetReplicate* rep = new NetReplicate(scope, scope->local_symbol(),
        ...
        netvector_t* osig_vec =
            new netvector_t(IVL_VT_LOGIC, instance_width - 1, 0);
        sig = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, osig_vec);
        sig->set_line(*this);
        sig->local_flag(true);
      ...
      if (instance_width != sig->vector_width()) {
        cerr << get_fileline() << ": error: "
             ...
             << " of logic gate array port " << idx + 1 << "." << endl;
        des->errors += 1;
      }
      ...
      if (idx < gate_count) {
        ...
        for (unsigned gdx = 0; gdx < array_count; gdx += 1) {
          ...
          NetNet* tmp2 =
              new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp2_vec);
          tmp2->set_line(*this);
        ...
        for (unsigned gdx = 0; gdx < array_count; gdx += 1) {
          ...
          NetPartSelect* tmp1 =
              new NetPartSelect(sig, gdx, 1, NetPartSelect::VP);
          tmp1->set_line(*this);
          des->add_node(tmp1);
      ...
      cerr << get_fileline() << ": error: Gate count of " << array_count
           << " does not match net width of " << sig->vector_width()
           << " at pin " << idx << "." << endl;
      des->errors += 1;
    }
  }
...
NetNet* PGModule::resize_net_to_port_(Design* des, NetScope* scope, NetNet* sig,
  ...
  NetNet* tmp =
      new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp_type);
  tmp->local_flag(true);
  tmp->set_line(*this);
  ...
  if (dir == NetNet::PINOUT) {
    ...
    NetTran* node = new NetTran(scope, scope->local_symbol(),
                                part_b ? wida : widb, part_b ? widb : wida, 0);
    if (part_b) {
  ...
  switch (dir) {
    case NetNet::POUTPUT:
      if (pwidth > swidth) {
        NetPartSelect* node =
            new NetPartSelect(tmp, 0, swidth, NetPartSelect::VP);
        connect(node->pin(0), sig->pin(0));
        des->add_node(node);
      } else {
        NetNet* osig;
    ...
    case NetNet::PINPUT:
      if (pwidth > swidth) {
        ...
        NetPartSelect* node =
            new NetPartSelect(sig, 0, pwidth, NetPartSelect::VP);
        connect(node->pin(0), tmp->pin(0));
        des->add_node(node);
      }
      break;
 ...
 */
static void convert_net(Design* des, const LineInfo* line, NetNet* net,
                        NetNet::Type type) {
  ...
  cerr << line->get_fileline()
       << ": sorry: Can not pull floating "
          "input type '"
       << net->type() << "'." << endl;
  des->errors += 1;
}
 ...
 */
void PGModule::elaborate_mod_(Design* des, Module* rmod,
                              NetScope* scope) const {
  assert(scope);

  if (debug_elaborate) {
    cerr << get_fileline() << ": debug: Instantiate module " << rmod->mod_name()
         << " with instance name " << get_name() << " in scope "
  ...
  if (pins_) {
    ...
    for (unsigned idx = 0; idx < npins_; idx += 1) {
      ...
      if (pins_[idx].name[0] == '*') {
        for (unsigned j = 0; j < nexp; j += 1) {
          if (!pins[j]) {
            ...
            symbol_search(this, des, scope, path_, net, par, eve);
            if (net != 0) {
      ...
      // as an error.
      if (pidx == nexp) {
        cerr << get_fileline() << ": error: port ``" << pins_[idx].name
             << "'' is not a port of " << get_name() << "." << endl;
        des->errors += 1;
        continue;
      }
    ...
    for (unsigned idx = 0; idx < rmod->port_count(); idx += 1) pins[idx] = 0;
    ...
    if (pin_count() != rmod->port_count()) {
      cerr << get_fileline()
           << ": error: Wrong number "
              "of ports. Expecting "
           << rmod->port_count() << ", got " << pin_count() << "." << endl;
      des->errors += 1;
      return;
    }
  ...
  if (debug_elaborate)
    cerr << get_fileline()
         << ": debug: start "
            "recursive elaboration of "
         << instance.size() << " instance(s) of " << get_name() << "..."
         << endl;
  ...
  for (unsigned idx = 0; idx < pins.size(); idx += 1) {
    ...
    if (pins[idx] == 0) {
      if (pins_fromwc[idx]) {
        cerr << get_fileline()
             << ": error: Wildcard named "
                "port connection (.*) did not find a matching "
                "identifier for port "
             << (idx + 1) << " (" << port_name << ")." << endl;
        des->errors += 1;
        return;
      }
      ...
      if (tmp->port_type() == NetNet::PINPUT) {
        ...
        if (warn_portbinding) {
          cerr << get_fileline() << ": warning: "
               << "Instantiating module " << rmod->mod_name()
               << " with dangling input port " << (idx + 1) << " ("
               << port_name;
          switch (rmod->uc_drive) {
            case Module::UCD_PULL0:
    ...
    if (debug_elaborate) {
      cerr << get_fileline() << ": debug: " << get_name() << ": Port "
           << (idx + 1) << " (" << port_name << ") has " << prts.size()
           << " sub-ports." << endl;
    }
    ...
    if (prts.empty() || (ptype == NetNet::PINPUT)) {
      ...
      if (tmp_expr == 0) {
        cerr << pins[idx]->get_fileline()
             << ": error: Failed to elaborate port expression." << endl;
        des->errors += 1;
        continue;
      }
      ...
      if (need_bufz_for_input_port(prts) || has_delay) {
        ...
        netvector_t* tmp2_vec =
            new netvector_t(sig->data_type(), sig->vector_width() - 1, 0);
        NetNet* tmp2 =
            new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp2_vec);
        tmp2->local_flag(true);
        tmp2->set_line(*this);
      ...
      if ((sig->data_type() == IVL_VT_REAL) && !prts.empty() &&
          (prts[0]->data_type() != IVL_VT_REAL)) {
        sig =
            cast_to_int4(des, scope, sig, prts_vector_width / instance.size());
      }
      ...
      if ((sig->data_type() == IVL_VT_LOGIC) && !prts.empty() &&
          (prts[0]->data_type() == IVL_VT_BOOL)) {
        sig = cast_to_int2(des, scope, sig, sig->vector_width());
      }
      ...
      if (sig == 0) {
        ...
        cerr << pins[idx]->get_fileline() << ":      : Port " << (idx + 1)
             << " (" << port_name << ") of " << rmod->mod_name()
             << " is connected to " << *pins[idx] << endl;
        des->errors += 1;
        continue;
      }
      ...
      if ((sig->data_type() == IVL_VT_REAL) && !prts.empty() &&
        ...
        cerr << pins[idx]->get_fileline() << ": error: "
             ...
             << rmod->mod_name() << " to real signal " << sig->name() << "."
             << endl;
        des->errors += 1;
        continue;
      }
      ...
      if ((sig->data_type() == IVL_VT_BOOL) && !prts.empty() &&
          (prts[0]->data_type() == IVL_VT_LOGIC)) {
        for (unsigned pidx = 0; pidx < prts.size(); pidx += 1) {
          prts[pidx] =
              cast_to_int2(des, scope, prts[pidx], prts[pidx]->vector_width());
        }
      }
      ...
      if ((sig->data_type() == IVL_VT_REAL) && !prts.empty() &&
          (prts[0]->data_type() == IVL_VT_REAL) && instance.size() != 1) {
        cerr << pins[idx]->get_fileline() << ": error: "
    ...
    if ((instance.size() != 1) && (sig->vector_width() != prts_vector_width) &&
        (sig->vector_width() != prts_vector_width / instance.size())) {
      cerr << pins[idx]->get_fileline() << ": error: "
           << "Port expression width " << sig->vector_width()
           << " does not match expected width " << prts_vector_width << " or "
           << (prts_vector_width / instance.size()) << "." << endl;
      des->errors += 1;
      continue;
    }
    ...
    if (debug_elaborate) {
      cerr << get_fileline() << ": debug: " << get_name() << ": Port "
           << (idx + 1) << " (" << port_name << ") has vector width of "
    ...
    if ((instance.size() == 1) && (prts_vector_width != sig->vector_width())) {
      ...
      cerr << get_fileline() << ": warning: Port " << (idx + 1) << " ("
           << port_name << ") of " << type_ << " expects " << prts_vector_width
      ...
      if (ptype == NetNet::PINOUT) {
        if (prts_vector_width > sig->vector_width()) {
          cerr << get_fileline() << ":        : Leaving "
               << (prts_vector_width - sig->vector_width())
               << " high bits of the port unconnected." << endl;
        } else {
          cerr << get_fileline() << ":        : Leaving "
               << (sig->vector_width() - prts_vector_width)
               << " high bits of the expression dangling." << endl;
        }
        ...
        cerr << (sig->vector_width() - prts_vector_width)
             << " high bits of the expression." << endl;
      }

      sig = resize_net_to_port_(des, scope, sig, prts_vector_width, ptype,
                                as_signed);
    }
    ...
    if (prts.size() == 1) {
      ...
      if (debug_elaborate) {
        cerr << get_fileline() << ": debug: " << get_name() << ": Replicating "
             << prts_vector_width << " bits across all "
             << prts_vector_width / instance.size() << " sub-ports." << endl;
      }
      ...
      switch (ptype) {
        case NetNet::POUTPUT:
          ctmp = new NetConcat(scope, scope->local_symbol(), prts_vector_width,
                               prts.size());
          ctmp->set_line(*this);
          des->add_node(ctmp);
          connect(ctmp->pin(0), sig->pin(0));
          for (unsigned ldx = 0; ldx < prts.size(); ldx += 1) {
            connect(ctmp->pin(ldx + 1), prts[prts.size() - ldx - 1]->pin(0));
          }
          break;

        case NetNet::PINPUT:
          if (debug_elaborate) {
            cerr << get_fileline() << ": debug: " << get_name() << ": Dividing "
                 << prts_vector_width << " bits across all "
                 << prts_vector_width / instance.size()
                 << " input sub-ports of port " << (idx + 1) << "." << endl;
          }

          for (unsigned ldx = 0, spin = 0; ldx < prts.size(); ldx += 1) {
            NetNet* sp = prts[prts.size() - ldx - 1];
            NetPartSelect* ptmp = new NetPartSelect(
                sig, spin, sp->vector_width(), NetPartSelect::VP);
            ptmp->set_line(*this);
            des->add_node(ptmp);
        ...
        case NetNet::PINOUT:
          for (unsigned ldx = 0, spin = 0; ldx < prts.size(); ldx += 1) {
            NetNet* sp = prts[prts.size() - ldx - 1];
            NetTran* ttmp =
                new NetTran(scope, scope->local_symbol(), sig->vector_width(),
                            sp->vector_width(), spin);
            ttmp->set_line(*this);
            des->add_node(ttmp);
...
void PGModule::elaborate_udp_(Design* des, PUdp* udp, NetScope* scope) const {
  ...
  if (my_name == 0) my_name = scope->local_symbol();
  ...
  if (overrides_) {
    if (overrides_->size() > 2) {
      ...
      tmp_del.eval_delays(des, scope, rise_expr, fall_expr, decay_expr, true);
    }
  }

  long low = 0, high = 0;
  unsigned inst_count =
      calculate_instance_count_(des, scope, high, low, my_name);
  if (inst_count == 0) return;

  if (inst_count != 1) {
    cerr << get_fileline() << ": sorry: UDPs with a range (" << my_name << " ["
         << high << ":" << low << "]) are "
         << "not supported." << endl;
    des->errors += 1;
    return;
  ...
  if (pins_) {
    ...
    if (pin_count() != udp->ports.count()) {
      cerr << get_fileline()
           << ": error: Wrong number "
              "of ports. Expecting "
           << udp->ports.count() << ", got " << pin_count() << "." << endl;
      des->errors += 1;
      return;
    }
  ...
  /* Handle the output port of the primitive special. It is an
     ...
     l-value net. */
  if (pins[0] == 0) {
    cerr << get_fileline() << ": warning: output port unconnected." << endl;
    ...
    if (sig == 0) {
      ...
      cerr << get_fileline() << ":      : Output "
           << "port of " << udp->name_ << " is " << udp->ports[0] << "."
           << endl;
      des->errors += 1;
    } else {
    ...
    if (sig->vector_width() != 1) {
      cerr << get_fileline() << ": error: "
           << "Output port expression " << *pins[0] << " is too wide ("
           << sig->vector_width() << ") expected 1." << endl;
      des->errors += 1;
    }
  }
  ...
  for (unsigned idx = 1; idx < net->pin_count(); idx += 1) {
    ...
    if (sig->vector_width() != 1) {
      cerr << get_fileline() << ": error: "
           << "Input port expression " << *pins[idx] << " is too wide ("
           << sig->vector_width() << ") expected 1." << endl;
      des->errors += 1;
    }
  }
...
void PGModule::elaborate(Design* des, NetScope* scope) const {
  ...
  if (!ignore_missing_modules) {
    cerr << get_fileline() << ": internal error: Unknown module type: " << type_
         << endl;
  }
}
...
void PGModule::elaborate_scope(Design* des, NetScope* sc) const {
  ...
  // Try a primitive type
  map<perm_string, PUdp*>::const_iterator udp = pform_primitives.find(type_);
  if (udp != pform_primitives.end()) return;
...
NetAssign_* PAssign_::elaborate_lval(Design* des, NetScope* scope) const {
  ...
  if (!lval_) {
    ...
    NetNet* tmp;
    netvector_t* tmp_vec = new netvector_t(
        rval_->expr_type(), rval_->expr_width() - 1, 0, rval_->has_sign());
...
NetExpr* PAssign_::elaborate_rval_(Design* des, NetScope* scope,
  ...
  cerr << get_fileline()
       << ": error: "
          "The RHS expression must be constant."
       << endl;
  cerr << get_fileline()
       << "       : "
 ...
 * scaled.
 */
static NetExpr* elaborate_delay_expr(PExpr* expr, Design* des,
                                     NetScope* scope) {
  ...
  if (dex == 0) {
    cerr << expr->get_fileline() << ": error: "
         << "Unable to elaborate (or evaluate) delay expression." << endl;
    des->errors += 1;
    return 0;
  }
...
NetProc* PAssign::elaborate(Design* des, NetScope* scope) const {
  ...
  if (op_ != 0) return elaborate_compressed_(des, scope);
  ...
  NetExpr* delay = 0;
  if (delay_ != 0) delay = elaborate_delay_expr(delay_, des, scope);
  ...
  if (dynamic_cast<const netclass_t*>(lv_net_type)) {
  ...
  } else if (const netdarray_t* dtype =
    ...
    if (lv->word()) use_lv_type = dtype->element_type();

    rv = elaborate_rval_(des, scope, use_lv_type);

  } else if (const netuarray_t* utype =
    ...
    rv = elaborate_rval_(des, scope, lv_net_type, lv->expr_type(),
                         count_lval_width(lv));
  }
  ...
  if (delay || event_) {
    ...
    NetNet* tmp =
        new NetNet(scope, scope->local_symbol(), NetNet::REG, tmp2_vec);
    tmp->local_flag(true);
    tmp->set_line(*this);
  ...
  if (lv->enumeration()) {
    if (!rv->enumeration()) {
      cerr << get_fileline()
           << ": error: "
              "This assignment requires an explicit cast."
           << endl;
      des->errors += 1;
    } else if (!lv->enumeration()->matches(rv->enumeration())) {
      cerr << get_fileline()
           << ": error: "
              "Enumeration type mismatch in assignment."
           << endl;
      des->errors += 1;
    }
  }
...
NetProc* PAssignNB::elaborate(Design* des, NetScope* scope) const {
  ...
  if (scope->in_func()) {
    cerr << get_fileline()
         << ": error: functions cannot have non "
  ...
  if (scope->in_final()) {
    cerr << get_fileline()
         << ": error: final procedures cannot have "
  ...
  NetExpr* rv =
      elaborate_rval_(des, scope, 0, lv->expr_type(), count_lval_width(lv));
  if (rv == 0) return 0;
...
NetProc* PBlock::elaborate(Design* des, NetScope* scope) const {
  ...
  if (pscope_name() != 0) {
    ...
    if (nscope == 0) {
      cerr << get_fileline()
           << ": internal error: "
              "unable to find block scope "
           << scope_path(scope) << "." << pscope_name() << endl;
      des->errors += 1;
      return 0;
    }
    assert(nscope);
  ...
  if ((type == NetBlock::SEQU) && (list_.size() == 1) && (pscope_name() == 0)) {
    assert(list_[0]);
  ...
  if (!nscope->is_const_func()) scope->is_const_func(false);
  if (nscope->calls_sys_task()) scope->calls_sys_task(true);
...
static int test_case_width(Design* des, NetScope* scope, PExpr* pe,
  ...
  if (debug_elaborate) {
    cerr << pe->get_fileline() << ": debug: test_width "
         << "of case expression " << *pe << endl;
    cerr << pe->get_fileline() << ":        "
         << "returns type=" << pe->expr_type() << ", width=" << expr_width
         << ", signed=" << pe->has_sign()
...
NetProc* PCase::elaborate(Design* des, NetScope* scope) const {
  ...
  for (unsigned idx = 0; idx < items_->count(); idx += 1) {
    ...
    for (list<PExpr*>::iterator idx_expr = cur->expr.begin();
      ...
      unsigned cur_width = test_case_width(des, scope, cur_expr, cur_mode);
      if (cur_mode > context_mode) context_mode = cur_mode;
      if (cur_width > context_width) context_width = cur_width;
      if (cur_expr->expr_type() == IVL_VT_REAL) context_is_real = true;
      if (!cur_expr->has_sign()) context_unsigned = true;
    }
  }
  ...
  if (context_is_real) {
    ...
    for (unsigned idx = 0; idx < items_->count(); idx += 1) {
      ...
      for (list<PExpr*>::iterator idx_expr = cur->expr.begin();
        ...
        unsigned cur_width =
            test_case_width(des, scope, cur_expr, context_mode);
        if (cur_width > context_width) context_width = cur_width;
      }
    }

    if (context_width < integer_width) context_width += 1;
  }
  ...
  NetExpr* expr = elab_and_eval_case(des, scope, expr_, context_is_real,
                                     context_unsigned, context_width);
  if (expr == 0) {
    cerr << get_fileline()
         << ": error: Unable to elaborate this case"
            " expression."
         << endl;
    return 0;
  }
  ...
  for (unsigned idx = 0; idx < items_->count(); idx += 1) {
    ...
    if (cur->expr.empty()) {
      /* If there are no expressions, then this is the
         default case. */
      NetProc* st = 0;
      if (cur->stat) st = cur->stat->elaborate(des, scope);
      ...
      for (list<PExpr*>::iterator idx_expr = cur->expr.begin();
        ...
        PExpr* cur_expr = *idx_expr;
        ivl_assert(*this, cur_expr);
        NetExpr* gu = elab_and_eval_case(des, scope, cur_expr, context_is_real,
                                         context_unsigned, context_width);
...
NetProc* PChainConstructor::elaborate(Design* des, NetScope* scope) const {
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PChainConstructor::elaborate: "
         << "Elaborate constructor chain in scope=" << scope_path(scope)
         << endl;
  }
  ...
  if (NetScope* new_scope =
          class_super->method_from_name(perm_string::literal("new@"))) {
  ...
  if (NetScope* new_scope =
    ...
    if (missing_parms) {
      cerr << get_fileline() << ": error: "
           << "Missing " << missing_parms << " arguments to constructor "
           << scope_path(new_scope) << "." << endl;
      des->errors += 1;
    }
...
NetProc* PCondit::elaborate(Design* des, NetScope* scope) const {
  ...
  if (expr->expr_width() < 1) {
    cerr << get_fileline()
         << ": internal error: "
            "incomprehensible expression width (0)."
         << endl;
    return 0;
  }
...
NetProc* PCallTask::elaborate_sys(Design* des, NetScope* scope) const {
  ...
  if ((parm_count == 1) && (parms_[0] == 0)) parm_count = 0;
 ...
 */
NetProc* PCallTask::elaborate_sys_task_method_(
    Design* des, NetScope* scope, NetNet* net, perm_string method_name,
  ...
  for (unsigned idx = 0; idx < nparms; idx += 1) {
    PExpr* ex = parms_[idx];
    if (ex != 0) {
      argv[idx + 1] = elab_sys_task_arg(des, scope, method_name, idx, ex);
    } else {
      argv[idx + 1] = 0;
    }
  }
...
NetProc* PCallTask::elaborate_method_(Design* des, NetScope* scope,
  ...
  if (const netclass_t* class_type = net->class_type()) {
    ...
    if (task == 0) {
      cerr << get_fileline() << ": internal error: "
           << "Can't find task " << method_name << " in class "
    ...
    if (debug_elaborate) {
      cerr << get_fileline() << ": PCallTask::elaborate_method_: "
           << "Elaborate " << class_type->get_name() << " method "
           << task->basename() << endl;
    }
...
bool PCallTask::test_task_calls_ok_(Design* des, NetScope* scope) const {
  if (scope->in_func()) {
    cerr << get_fileline()
         << ": error: Functions cannot enable/call "
            "tasks."
         << endl;
    des->errors += 1;
    return false;
...
NetProc* PCallTask::elaborate_void_function_(Design* des, NetScope* scope,
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PCallTask::elaborate_void_function_: "
         << "function void " << scope_path(dscope) << endl;
  }
...
NetProc* PCallTask::elaborate_build_call_(Design* des, NetScope* scope,
  ...
  if (parms_.size() > parm_count) {
    cerr << get_fileline() << ": error: "
         << "Too many arguments (" << parms_.size() << ", expecting "
         << parm_count << ")"
  ...
  if (const NetBlock* tp = dynamic_cast<const NetBlock*>(def->proc())) {
    if (tp->proc_first() == 0) return block;
  }
  ...
  for (unsigned idx = use_this ? 1 : 0; idx < parm_count; idx += 1) {
    ...
    if (port->port_type() == NetNet::POUTPUT) continue;
    ...
    if (parms_idx < parms_.size() && parms_[parms_idx]) {
      rv = elaborate_rval_expr(des, scope, port->net_type(), lv_type, wid,
                               parms_[parms_idx]);
      ...
      cerr << get_fileline() << ": error: "
           << "Missing argument " << (idx + 1) << " of call to task." << endl;
      des->errors += 1;
      continue;
    }
  ...
  for (unsigned idx = use_this ? 1 : 0; idx < parm_count; idx += 1) {
    ...
    /* Skip input ports. */
    assert(port->port_type() != NetNet::NOT_A_PORT);
    if (port->port_type() == NetNet::PINPUT) continue;
...
NetCAssign* PCAssign::elaborate(Design* des, NetScope* scope) const {
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": debug: Elaborate cassign,"
         << " lval width=" << lwid << " rval width=" << rexp->expr_width()
         << " rval=" << *rexp << endl;
  }
...
NetProc* PDisable::elaborate(Design* des, NetScope* scope) const {
  ...
  if (scope_.empty()) {
    if (gn_system_verilog()) {
      ...
      cerr << get_fileline()
           << ": error: 'disable fork' requires SystemVerilog." << endl;
      des->errors += 1;
      return 0;
    }
  }
  ...
  if (target == 0) {
    cerr << get_fileline() << ": error: Cannot find scope " << scope_ << " in "
         << scope_path(scope) << endl;
    des->errors += 1;
    return 0;
  }
...
NetProc* PEventStatement::elaborate_st(Design* des, NetScope* scope,
  ...
  if (expr_.count() == 0) {
    ...
    if (nset == 0) {
      cerr << get_fileline() << ": error: Unable to elaborate:" << endl;
      enet->dump(cerr, 6);
      des->errors += 1;
      return enet;
    }
    ...
    NetEvProbe* pr = new NetEvProbe(scope, scope->local_symbol(), ev,
                                    NetEvProbe::ANYEDGE, nset->size());
...
// FIXME: create a part select to get the correct bits to connect.
    ...
    for (unsigned idx = 0; idx < expr_.count(); idx += 1) {
      ...
      if (PEIdent* id = dynamic_cast<PEIdent*>(expr_[idx]->expr())) {
        ...
        NetScope* found_in =
            symbol_search(this, des, use_scope, id->path(), sig, par, eve);
        ...
        if (found_in && eve) {
          ...
          if (expr_[idx]->type() != PEEvent::ANYEDGE) {
            ...
            cerr << " can not be used with a named event (" << eve->name()
                 << ")." << endl;
            des->errors += 1;
          }
          continue;
      ...
      unsigned pins =
          (expr_[idx]->type() == PEEvent::ANYEDGE) ? expr->pin_count() : 1;
...
NetProc* PEventStatement::elaborate_wait(Design* des, NetScope* scope,
  ...
  NetExpr* expr =
      pe->elaborate_expr(des, scope, pe->expr_width(), PExpr::NO_FLAGS);
  if (expr == 0) {
    cerr << get_fileline()
         << ": error: Unable to elaborate"
  ...
  if (expr->expr_width() < 1) {
    cerr << get_fileline()
         << ": internal error: "
            "incomprehensible wait expression width (0)."
         << endl;
    return 0;
  }
 ...
 *     @(0) <noop>;
 */
NetProc* PEventStatement::elaborate_wait_fork(Design* des,
                                              NetScope* scope) const {
  assert(scope);
  assert(expr_.count() == 1);
  assert(expr_[0] == 0);
  ...
  if (gn_system_verilog()) {
    ...
    cerr << get_fileline() << ": error: 'wait fork' requires SystemVerilog."
         << endl;
    des->errors += 1;
    return 0;
  }
...
NetForce* PForce::elaborate(Design* des, NetScope* scope) const {
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": debug: Elaborate force,"
         ...
         << " rval width=" << rexp->expr_width() << " rval=" << *rexp << endl;
  }
...
static void find_property_in_class(const LineInfo& loc, const NetScope* scope,
                                   perm_string name,
                                   const netclass_t*& found_in, int& property) {
...
NetProc* PForeach::elaborate(Design* des, NetScope* scope) const {
  ...
  if (array_sig == 0)
    find_property_in_class(*this, scope, array_var_, class_scope,
                           class_property);

  if (debug_elaborate && array_sig) {
    cerr << get_fileline() << ": PForeach::elaborate: "
         << "Found array_sig in " << scope_path(array_sig->scope()) << "."
         << endl;
  }

  if (debug_elaborate && class_scope) {
    cerr << get_fileline() << ": PForeach::elaborate: "
         << "Found array_sig property (" << class_property << ") in class "
         << class_scope->get_name() << " as "
         << *class_scope->get_prop_type(class_property) << "." << endl;
  }
  ...
  if (class_scope != 0 && class_property >= 0) {
    ...
    if (dims.size() < index_vars_.size()) {
      cerr << get_fileline() << ": error: "
           << "class " << class_scope->get_name() << " property " << array_var_
  ...
  if (array_sig == 0) {
    cerr << get_fileline() << ": error:"
         << " Unable to find foreach array " << array_name << " in scope "
         << scope_path(scope) << "." << endl;
    des->errors += 1;
    return 0;
  }
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": PForeach::elaborate: "
         << "Scan array " << array_sig->name() << " of "
         << array_sig->data_type() << " with "
         << array_sig->unpacked_dimensions() << " unpacked"
         << " and " << array_sig->packed_dimensions() << " packed dimensions."
         << endl;
  }

  std::vector<netrange_t> dims = array_sig->unpacked_dims();
  if (array_sig->packed_dimensions() > 0) {
    dims.insert(dims.end(), array_sig->packed_dims().begin(),
                array_sig->packed_dims().end());
  }
 ...
 */
NetProc* PForeach::elaborate_static_array_(
    Design* des, NetScope* scope, const vector<netrange_t>& dims) const {
  if (debug_elaborate) {
...
NetProc* PForStatement::elaborate(Design* des, NetScope* scope) const {
  ...
  initial_expr =
      elaborate_rval_expr(des, scope, sig->net_type(), sig->data_type(),
                          sig->vector_width(), expr1_);

  if (debug_elaborate && initial_expr) {
    cerr << get_fileline() << ": debug: FOR initial assign: " << sig->name()
         << " = " << *initial_expr << endl;
  }
  ...
  if (dynamic_cast<NetEConst*>(ce)) {
    cerr << get_fileline()
         << ": warning: condition expression "
            "of for-loop is constant."
         << endl;
  }
...
void PFunction::elaborate(Design* des, NetScope* scope) const {
  ...
  if (def == 0) {
    cerr << get_fileline() << ": internal error: "
         << "No function definition for function " << scope_path(scope) << endl;
    des->errors += 1;
    return;
  }
  assert(def);
...
NetProc* PRepeat::elaborate(Design* des, NetScope* scope) const {
  ...
  if (expr->expr_type() == IVL_VT_REAL) expr = cast_to_int4(expr, 64);
...
NetProc* PReturn::elaborate(Design* des, NetScope* scope) const {
  ...
  for (;;) {
    ...
    if (target->type() == NetScope::FUNC) break;
  ...
  NetExpr* val =
      elaborate_rval_expr(des, scope, res->net_type(), lv_type, wid, expr_);
  ...
  if (statement_ == 0) {
    ...
    if (st == 0) {
      cerr << statement_->get_fileline()
           << ": Unable to elaborate "
              "statement in task "
           << scope_path(task) << " at " << get_fileline() << "." << endl;
      return;
    }
  }
...
NetProc* PTrigger::elaborate(Design* des, NetScope* scope) const {
  ...
  NetScope* found_in =
      symbol_search(this, des, use_scope, event_, sig, par, eve);
...
bool PProcess::elaborate(Design* des, NetScope* scope) const {
  ...
  code is executed. */
  do {
    if ((top->type() != IVL_PR_ALWAYS) && (top->type() != IVL_PR_ALWAYS_COMB) &&
    ...
    if (st->nevents() != 1) break;

    NetEvent* ev = st->event(0);

    if (ev->nprobe() == 0) break;

    bool anyedge_test = true;
    for (unsigned idx = 0; anyedge_test && (idx < ev->nprobe()); idx += 1) {
      const NetEvProbe* pr = ev->probe(idx);
      if (pr->edge() != NetEvProbe::ANYEDGE) anyedge_test = false;
    }

    if (!anyedge_test) break;

    top->attribute(perm_string::literal("_ivl_schedule_push"), verinum(1));
  } while (0);
...
void PSpecPath::elaborate(Design* des, NetScope* scope) const {
  ...
  for (unsigned idx = 0; idx < ndelays; idx += 1) {
    ...
    if (NetEConst* con = dynamic_cast<NetEConst*>(cur)) {
      ...
      delay_value[idx] = des->scale_to_precision(fn.as_ulong64(), scope);

    } else if (NetECReal* rcon = dynamic_cast<NetECReal*>(cur)) {
      delay_value[idx] = get_scaled_time_from_real(des, scope, rcon);
  ...
  if (!full_flag_ && ((src.size() != 1) || (dst.size() != 1))) {
    ...
    for (str_vec_iter cur = src.begin(); (cur != src.end() && all_single);
         ++cur) {
    ...
    for (str_vec_iter cur = dst.begin(); (cur != dst.end() && all_single);
         ++cur) {
      NetNet* psig = scope->find_signal(*cur);
  ...
  for (str_vector_iter cur = dst.begin(); cur != dst.end(); ++cur) {
    if (debug_elaborate) {
    ...
    if (dst_sig == 0) {
      cerr << get_fileline() << ": error: No wire '" << *cur
           << "' in this module." << endl;
      des->errors += 1;
      continue;
    }
    ...
    switch (ndelays) {
      case 12:
        path->set_delays(delay_value[0], delay_value[1], delay_value[2],
                         delay_value[3], delay_value[4], delay_value[5],
                         delay_value[6], delay_value[7], delay_value[8],
                         delay_value[9], delay_value[10], delay_value[11]);
        break;
      case 6:
        path->set_delays(delay_value[0], delay_value[1], delay_value[2],
                         delay_value[3], delay_value[4], delay_value[5]);
    ...
    unsigned idx = 0;
    for (str_vector_iter cur_src = src.begin(); cur_src != src.end();
         ++cur_src) {
      NetNet* src_sig = scope->find_signal(*cur_src);
      if (src_sig == 0) {
        cerr << get_fileline() << ": error: No wire '" << *cur_src
             << "' in this module." << endl;
        des->errors += 1;
        continue;
      }
      ...
      if ((src_sig->port_type() != NetNet::PINPUT) &&
        ...
        cerr << get_fileline() << ": error: Path source " << *cur_src
             << " must be an input or inout port." << endl;
        des->errors += 1;
      }
    ...
    if (condit_sig) connect(condit_sig->pin(0), path->pin(idx));
...
static void elaborate_functions(Design* des, NetScope* scope,
  ...
  for (mfunc_it_t cur = funcs.begin(); cur != funcs.end(); ++cur) {
...
static void elaborate_tasks(Design* des, NetScope* scope,
  ...
  for (mtask_it_t cur = tasks.begin(); cur != tasks.end(); ++cur) {
    hname_t use_name((*cur).first);
...
void NetScope::elaborate_delays(Design* des) {
  ...
  for (signals_map_iter_t cur = signals_map_.begin(); cur != signals_map_.end();
       ++cur) {
  ...
  for (std::list<NetNet*>::iterator cur = sigs.begin(); cur != sigs.end();
       ++cur) {
    NetNet* sig = *cur;
    NetBUFZ* tmp = new NetBUFZ(this, local_symbol(), sig->vector_width(), true);
    tmp->set_line(*sig);
    des->add_node(tmp);
    netvector_t* tmp2_vec =
        new netvector_t(sig->data_type(), sig->vector_width() - 1, 0);
    NetNet* tmp2 = new NetNet(this, local_symbol(), NetNet::WIRE, tmp2_vec);
    tmp2->local_flag(true);
    tmp2->set_line(*sig);
...
bool Module::elaborate(Design* des, NetScope* scope) const {
  ...
  for (list<PGate*>::const_iterator gt = gl.begin(); gt != gl.end(); ++gt) {
...
void netclass_t::elaborate(Design* des, PClass* pclass) {
  ...
  for (map<perm_string, PTask*>::iterator cur = pclass->tasks.begin();
    ...
    if (debug_elaborate) {
      cerr << cur->second->get_fileline() << ": netclass_t::elaborate: "
           << "Elaborate class " << scope_path(class_scope_) << " task method "
           << cur->first << endl;
    }
...
bool PGenerate::elaborate(Design* des, NetScope* container) const {
  if (direct_nested_) return elaborate_direct_(des, container);
  ...
  if (debug_elaborate) {
    cerr << get_fileline()
         << ": PGenerate::elaborate: "
            "generate "
         << scheme_type << " elaborating in scope " << scope_path(container)
         << "." << endl;
    cerr << get_fileline()
         << ": PGenerate::elaborate: "
            "generate scope_name="
         << scope_name << ", id_number=" << id_number << endl;
  }
  ...
  for (scope_list_it_t cur = scope_list_.begin(); cur != scope_list_.end();
       ++cur) {
    NetScope* scope = *cur;
    // Check that this scope is one that is contained in the
    // container that the caller passed in.
    if (scope->parent() != container) continue;
...
bool PGenerate::elaborate_direct_(Design* des, NetScope* container) const {
  ...
  if (debug_elaborate) {
    cerr << get_fileline() << ": debug: "
         << "Direct nesting elaborate in scope " << scope_path(container)
...
bool LexicalScope::elaborate_var_inits_(Design* des, NetScope* scope) const {
  if (var_inits.size() == 0) return true;
  ...
  if (gn_system_verilog()) {
    top->attribute(perm_string::literal("_ivl_schedule_init"), verinum(1));
  }
  des->add_process(top);
...
class elaborate_package_t : public elaborator_work_item_t {
 public:
  ...
  virtual void elaborate_runrun() {
    if (!package_->elaborate_scope(des, scope_)) des->errors += 1;
  }
...
class elaborate_root_scope_t : public elaborator_work_item_t {
 public:
  ...
  virtual void elaborate_runrun() {
    ...
    for (list<Module::named_expr_t>::iterator cur =
             Module::user_defparms.begin();
         cur != Module::user_defparms.end(); ++cur) {
      pform_name_t tmp_name = cur->first;
      if (peek_head_name(tmp_name) != scope_->basename()) continue;

      tmp_name.pop_front();
      if (tmp_name.size() != 1) continue;

      root_repl[peek_head_name(tmp_name)] = cur->second;
    }

    if (!rmod_->elaborate_scope(des, scope_, root_repl)) des->errors += 1;
  }
...
class top_defparams : public elaborator_work_item_t {
 public:
  explicit top_defparams(Design* des__) : elaborator_work_item_t(des__) {}
...
class later_defparams : public elaborator_work_item_t {
 public:
  explicit later_defparams(Design* des__) : elaborator_work_item_t(des__) {}
...
bool Design::check_proc_delay() const {
  ...
  for (const NetProcTop* pr = procs_; pr; pr = pr->next_) {
    ...
    if (pr->type() == IVL_PR_ALWAYS) {
      ...
      if (dly_type == NO_DELAY || dly_type == ZERO_DELAY) {
        cerr << pr->get_fileline()
             << ": error: always "
                "process does not have any delay."
             << endl;
        cerr << pr->get_fileline()
             << ":      : A runtime "
        ...
        cerr << pr->get_fileline()
             << ": warning: always "
                "process may not have any delay."
             << endl;
    ...
    if ((pr->type() == IVL_PR_ALWAYS_COMB) ||
      ...
      if ((pr->type() != IVL_PR_ALWAYS_FF) && (wait->nevents() == 0)) {
...
static void print_nexus_name(const Nexus* nex) {
  for (const Link* cur = nex->first_nlink(); cur; cur = cur->next_nlink()) {
    ...
    } else if (const NetPartSelect* ps =
    ...
    } else if (const NetUReduce* reduce =
                   dynamic_cast<const NetUReduce*>(obj)) {
...
static void check_event_probe_width(const LineInfo* info,
                                    const NetEvProbe* prb) {
  assert(prb->pin_count() == 1);
  assert(prb->pin(0).get_dir() == Link::INPUT);
  assert(prb->pin(0).is_linked());
  if (prb->edge() == NetEvProbe::ANYEDGE) return;
  if (prb->pin(0).nexus()->vector_width() > 1) {
    cerr << info->get_fileline()
         << " Warning: Synthesis wants "
...
bool Design::check_proc_synth() const {
  ...
  for (const NetProcTop* pr = procs_; pr; pr = pr->next_) {
    if ((pr->type() == IVL_PR_ALWAYS_COMB) ||
        (pr->type() == IVL_PR_ALWAYS_FF) ||
        (pr->type() == IVL_PR_ALWAYS_LATCH)) {
      result |= pr->statement()->check_synth(pr->type(), pr->scope());
      if (pr->type() == IVL_PR_ALWAYS_FF) {
...
static void check_timescales() {
  ...
  for (mod = pform_modules.begin(); mod != pform_modules.end(); ++mod) {
    ...
    if (some_explicit && some_implicit) break;
  }
  ...
  if (gn_system_verilog() && !(some_explicit && some_implicit)) {
    for (unsigned idx = 0; idx < pform_units.size(); idx += 1) {
      ...
      if (pp->parameters.empty() && pp->localparams.empty() &&
          pp->wires.empty() && pp->tasks.empty() && pp->funcs.empty() &&
          pp->classes.empty())
        continue;
  ...
  if (!(some_explicit && some_implicit)) return;

  if (gn_system_verilog()) {
    cerr << "warning: "
         << "Some design elements have no explicit time unit and/or" << endl;
    cerr << "       : "
         << "time precision. This may cause confusing timing results." << endl;
    cerr << "       : "
         << "Affected design elements are:" << endl;
  } else {
    cerr << "warning: "
         << "Some modules have no timescale. This may cause" << endl;
    cerr << "       : "
         << "confusing timing results.	Affected modules are:" << endl;
  }

  for (mod = pform_modules.begin(); mod != pform_modules.end(); ++mod) {
    Module* mp = (*mod).second;
    if (mp->has_explicit_timescale()) continue;
    cerr << "       :   -- module " << (*mod).first
         << " declared here: " << mp->get_fileline() << endl;
  }

  if (!gn_system_verilog()) return;

  for (pkg = pform_packages.begin(); pkg != pform_packages.end(); ++pkg) {
    PPackage* pp = (*pkg).second;
    if (pp->has_explicit_timescale()) continue;
...
Design* elaborate(list<perm_string> roots) {
  ...
  if (gn_system_verilog()) npackages += pform_units.size();
  ...
  for (map<perm_string, PPackage*>::iterator pac = pform_packages.begin();
    ...
    elaborator_work_item_t* es =
        new elaborate_package_t(des, scope, pac->second);
    des->elaboration_work_list.push_back(es);
  ...
  for (list<perm_string>::const_iterator root = roots.begin();
    ...
    if (mod == pform_modules.end()) {
      cerr << "error: Unable to find the root module \"" << (*root)
           << "\" in the Verilog source." << endl;
      cerr << "     : Perhaps ``-s " << (*root) << "'' is incorrect?" << endl;
      des->errors++;
      continue;
    }
    ...
    NetScope* scope = des->make_root_scope(
        *root, unit_scope, rmod->program_block, rmod->is_interface);
  ...
  if (debug_elaborate) {
    cerr << "<toplevel>: elaborate: "
         << "elaboration work list done. Start processing residual defparams."
         << endl;
  }
  ...
  for (i = 0; i < root_elems.size(); i++) {
    ...
    if (debug_elaborate) {
      cerr << "<toplevel>"
           << ": debug: " << rmod->mod_name() << ": port elaboration root "
    ...
    if (!rmod->elaborate_sig(des, scope)) {
      if (debug_elaborate) {
        cerr << "<toplevel>"
             << ": debug: " << rmod->mod_name() << ": elaborate_sig failed!!!"
             << endl;
      }
      delete des;
      return 0;
    ...
    for (unsigned idx = 0; idx < rmod->port_count(); idx += 1) {
      ...
      if (debug_elaborate) {
        cerr << "<toplevel>"
             << ": debug: " << rmod->mod_name() << ": adding module port "
             << rmod->get_port_name(idx) << endl;
      }
      scope->add_module_port_info(idx, rmod->get_port_name(idx), ptype,
                                  prt_vector_width);
    }
  }
  ...
  if (debug_elaborate) {
    cerr << "<toplevel>"
         << ": debug: "
         << " finishing with " << des->find_root_scopes().size()
         << " root scopes " << endl;
  }
