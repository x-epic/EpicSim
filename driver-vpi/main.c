...
{
	fprintf(stderr, "usage: iverilog-vpi" EPICSIM_SUFFIX " [options] [src and obj files]...\n");
...
static int GetRegistryKey(char *key, char **value)
{
	...
	lrv = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\EpicSim", 0, KEY_QUERY_VALUE, &hkKey);
...
//	printf("MinGW root directory:  %s.\n", gstr.pMINGW);
	...
	compile(gstr.pCCSRC, gstr.pCCFLAGS, &gstr.pOBJ, &compile_errors, EPICSIM_VPI_CC );
	  /* compile the C++ source files (*.cc, *.cpp) */
	compile(gstr.pCXSRC, gstr.pCXFLAGS, &gstr.pOBJ, &compile_errors, EPICSIM_VPI_CXX);
