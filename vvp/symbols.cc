...
static inline char* node_last_key(struct tree_node_* node) {
  while (node->leaf_flag == false) node = node->child[node->count - 1];
...
static struct tree_node_* split_leaf_(struct tree_node_* cur) {
  ...
  if (cur->parent->count == node_width) split_node_(cur->parent);
  ...
  if (cur->leaf_flag) {
    ...
    for (;;) {
      ...
      if (idx == cur->count) {
        ...
        cur->count += 1;
        if (cur->count == leaf_width) split_leaf_(cur);
      ...
      if (rc == 0) {
        if (force_flag) cur->leaf[idx].val = val;
    ...
    for (;;) {
      ...
      if (rc == 0) {
        return find_value_(cur->child[idx], key, val, force_flag);
      }
      ...
      if (rc > 0) {
        ...
        if (min == cur->count)
          return find_value_(cur->child[idx], key, val, force_flag);
        if (min == max)
          return find_value_(cur->child[max], key, val, force_flag);
