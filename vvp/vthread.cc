...
struct vthread_s {
 ...
 public:
  ...
  inline void push_real(double val) { stack_real_.push_back(val); }
 ...
 public:
  ...
  inline void push_str(const string& val) { stack_str_.push_back(val); }
 ...
 public:
  ...
  inline void pop_object(unsigned cnt, unsigned skip = 0) {
    ...
    for (size_t idx = stack_obj_size_ - skip - cnt;
         idx < stack_obj_size_ - skip; idx += 1)
...
inline vthread_s::vthread_s() { stack_obj_size_ = 0; }
...
void vthread_s::debug_dump(ostream& fd, const char* label) {
  ...
  fd << "**** Flags: ";
  for (int idx = 0; idx < FLAGS_COUNT; idx += 1) fd << flags[idx];
  fd << endl;
  fd << "**** vec4 stack..." << endl;
  for (size_t idx = stack_vec4_.size(); idx > 0; idx -= 1)
    fd << "    " << (stack_vec4_.size() - idx) << ": " << stack_vec4_[idx - 1]
       << endl;
...
__vpiScope* vthread_scope(struct vthread_s* thr) { return thr->parent_scope; }
...
const vvp_vector4_t& vthread_get_vec4_stack(struct vthread_s* thr,
                                            unsigned depth) {
...
static vvp_queue* get_queue_object(vthread_t thr, vvp_net_t* net) {
...
template <class T>
T coerce_to_width(const T& that, unsigned width) {
  if (that.size() == width) return that;
...
static void multiply_array_imm(unsigned long* res, unsigned long* val,
  ...
  for (unsigned idx = 0; idx < words; idx += 1) res[idx] = 0;
...
vthread_t vthread_new(vvp_code_t pc, __vpiScope* scope) {
  ...
  thr->flags[2] = BIT4_X;
  thr->flags[3] = BIT4_Z;
  for (int idx = 4; idx < 8; idx += 1) thr->flags[idx] = BIT4_X;
...
void vthread_delay_delete() {
  if (running_thread) running_thread->delay_delete = 1;
}
...
bool of_ASSIGN_VEC4_A_D(vthread_t thr, vvp_code_t cp) {
  ...
  if (thr->flags[4] == BIT4_1) return true;

  if (off >= (long)array_wid) return true;
  if (off < 0) {
    if ((unsigned)-off >= array_wid) return true;
...
bool of_BLEND(vthread_t thr, vvp_code_t) {
  ...
  for (unsigned idx = 0; idx < vala.size(); idx += 1) {
    v = vala.value(idx);
    if (v == valb.value(idx) && v != BIT4_Z) continue;
...
bool of_BREAKPOINT(vthread_t, vvp_code_t) { return true; }
...
bool of_CASSIGN_LINK(vthread_t, vvp_code_t cp) {
  ...
  vvp_fun_signal_base* sig = dynamic_cast<vvp_fun_signal_base*>(dst->fun);
  assert(sig);
...
bool of_CASSIGN_VEC4_OFF(vthread_t thr, vvp_code_t cp) {
  ...
  if (base < 0 && (wid <= (unsigned)-base)) return true;

  if (base >= (long)sig->value_size()) return true;
...
bool do_cast_vec_dar(vthread_t thr, vvp_code_t cp, bool as_vec4) {
  ...
  if (vec.size() != wid) {
    cerr << "VVP error: size mismatch when casting dynamic array to vector."
         << endl;
...
bool of_CAST_VEC4_STR(vthread_t thr, vvp_code_t cp) {
  ...
  while (vdx > 0) {
    ...
    for (unsigned bdx = 0; bdx < 8; bdx += 1) {
      if (ch & 1) vec.set_bit(vdx + bdx, BIT4_1);
      ch >>= 1;
    }
  }
...
static void do_CMPE(vthread_t thr, const vvp_vector4_t& lval,
  ...
  if (lval.has_xz() || rval.has_xz()) {
    ...
    for (unsigned idx = 0; idx < wid; idx += 1) {
      ...
      if (eq == BIT4_1 && (bit4_is_xz(lv) || bit4_is_xz(rv))) eq = BIT4_X;
      if ((lv == BIT4_0) && (rv == BIT4_1)) eq = BIT4_0;
      if ((lv == BIT4_1) && (rv == BIT4_0)) eq = BIT4_0;
...
static void do_CMPS(vthread_t thr, const vvp_vector4_t& lval,
                    const vvp_vector4_t& rval) {
  assert(rval.size() == lval.size());
...
static void do_CMPU(vthread_t thr, const vvp_vector4_t& lval,
                    const vvp_vector4_t& rval) {
  vvp_bit4_t eq = BIT4_1;
  ...
  for (unsigned wdx = 0; wdx < words; wdx += 1) {
    if (larray[wdx] == rarray[wdx]) continue;
...
static void do_CMPWE(vthread_t thr, const vvp_vector4_t& lval,
                     const vvp_vector4_t& rval) {
  assert(rval.size() == lval.size());
  ...
  if (lval.has_xz() || rval.has_xz()) {
    ...
    for (unsigned idx = 0; idx < wid; idx += 1) {
      ...
      if (bit4_is_xz(rv)) continue;
      if ((eq == BIT4_1) && bit4_is_xz(lv)) eq = BIT4_X;
      if ((lv == BIT4_0) && (rv == BIT4_1)) eq = BIT4_0;
      if ((lv == BIT4_1) && (rv == BIT4_0)) eq = BIT4_0;
...
bool of_DEASSIGN(vthread_t, vvp_code_t cp) {
  ...
  if (vvp_net_t* src = sig->cassign_link) {
    if (!full_sig) {
      fprintf(stderr,
              "Sorry: when a signal is assigning a "
...
static bool do_disable(vthread_t thr, vthread_t match) {
  ...
  while (!thr->children.empty()) {
    ...
    thr->i_am_joining = 0;
    if (do_disable(tmp, match)) flag = true;
  ...
  if (parent && parent->i_am_joining && test_joinable(parent, thr)) {
    ...
    if (!parent->i_have_ended) schedule_vthread(parent, 0, true);
...
bool of_DISABLE(vthread_t thr, vvp_code_t cp) {
  ...
  while (!scope->threads.empty()) {
    ...
    if (do_disable(*cur, thr)) disabled_myself_flag = true;
  }
...
static unsigned long* divide_bits(unsigned long* ap, unsigned long* bp,
                                  unsigned wid) {
  ...
  while (btop > 0 && bp[btop] == 0) btop -= 1;

  // Detect divide by 0, and exit.
  if (btop == 0 && bp[0] == 0) return 0;
  ...
  for (unsigned idx = 0; idx < words; idx += 1) result[idx] = 0;
  ...
  for (unsigned cur = words - btop; cur > 0; cur -= 1) {
    ...
    if (ap[cur_ptr + btop] >= bp[btop]) {
      ...
      if (cur_ptr + btop + 1 < words) high = ap[cur_ptr + btop + 1];
      ...
      cur_res =
          divide2words(ap[cur_ptr + btop], bp[btop], ap[cur_ptr + btop + 1]);
    }
...
bool of_DIV_S(vthread_t thr, vvp_code_t) {
  ...
  if (unsigned long sign_bits = (words * CPU_WORD_BITS) - wid) {
    ...
    if (ap[words - 1] & (sign_mask >> 1)) ap[words - 1] |= sign_mask;
    if (bp[words - 1] & (sign_mask >> 1)) bp[words - 1] |= sign_mask;
  }
...
bool of_END(vthread_t thr, vvp_code_t) {
  ...
  if (!thr->i_am_detached && thr->parent && thr->parent->i_am_joining) {
    ...
    if (!test_joinable(tmp, thr)) return false;
...
bool of_FORCE_VEC4(vthread_t thr, vvp_code_t cp) {
  ...
  net->force_vec4(value,
                  vvp_vector2_t(vvp_vector2_t::FILL1, net->fil->filter_size()));
...
bool of_FORCE_VEC4_OFF(vthread_t thr, vvp_code_t cp) {
  ...
  if (base >= (long)use_size) return true;
  if (base < -(long)use_size) return true;

  if ((base + wid) > use_size) wid = use_size - base;
...
bool of_IX_ADD(vthread_t thr, vvp_code_t cp) {
  thr->words[cp->number].w_int += get_as_64_bit(cp->bit_idx[0], cp->bit_idx[1]);
  return true;
}
...
bool of_IX_GETV_S(vthread_t thr, vvp_code_t cp) {
  ...
  if (sig == 0) {
    ...
    cerr << "%%ix/getv/s error: Net arg not a vector signal? "
         ...
         << ", fil=" << (net->fil ? typeid(*net->fil).name() : "<>") << endl;
  }
  assert(sig);
...
bool of_JMP0(vthread_t thr, vvp_code_t cp) {
  if (thr->flags[cp->bit_idx[0]] == BIT4_0) thr->pc = cp->cptr;
...
bool of_JMP0XZ(vthread_t thr, vvp_code_t cp) {
  if (thr->flags[cp->bit_idx[0]] != BIT4_1) thr->pc = cp->cptr;
...
static bool do_join_opcode(vthread_t thr) {
  ...
  for (set<vthread_t>::iterator cur = thr->children.begin();
       cur != thr->children.end(); ++cur) {
    vthread_t curp = *cur;
    if (!curp->i_have_ended) continue;

    if (!test_joinable(thr, curp)) continue;
...
bool of_JOIN(vthread_t thr, vvp_code_t) { return do_join_opcode(thr); }
...
bool of_LOAD_VEC4(vthread_t thr, vvp_code_t cp) {
  ...
  if (sig == 0) {
    cerr << "%load/v error: Net arg not a signal? "
         << (net->fil ? typeid(*net->fil).name() : typeid(*net->fun).name())
         << endl;
    assert(sig);
  }
...
bool of_MOD_S(vthread_t thr, vvp_code_t) {
  ...
  if (wid <= 8 * sizeof(long long)) {
    ...
    if ((lv == LLONG_MIN) && (rv == -1)) goto zero_out;

    /* Sign extend the signed operands when needed. */
    if (wid < 8 * sizeof(long long)) {
      if (lv & (1LL << (wid - 1))) lv |= -1ULL << wid;
      if (rv & (1LL << (wid - 1))) rv |= -1ULL << wid;
    }
...
bool of_NEW_DARRAY(vthread_t thr, vvp_code_t cp) {
  ...
  if (strcmp(text, "b8") == 0) {
    ...
    cerr << "Internal error: Unsupported dynamic array type: " << text << "."
         << endl;
    assert(0);
  }
...
bool of_NOOP(vthread_t, vvp_code_t) { return true; }
...
bool of_POW(vthread_t thr, vvp_code_t) { return of_POW_base(thr, false); }

bool of_POW_S(vthread_t thr, vvp_code_t) { return of_POW_base(thr, true); }
...
bool of_PUSHV_STR(vthread_t thr, vvp_code_t) {
  ...
  for (size_t idx = 0; idx < vec.size(); idx += 8) {
    char tmp = 0;
    size_t trans = 8;
    if (idx + trans > vec.size()) trans = vec.size() - idx;

    for (size_t bdx = 0; bdx < trans; bdx += 1) {
      if (vec.value(idx + bdx) == BIT4_1) tmp |= 1 << bdx;
    }

    if (tmp != 0) buf.push_back(tmp);
  }

  string val;
  for (vector<char>::reverse_iterator cur = buf.rbegin(); cur != buf.rend();
       ++cur) {
    val.push_back(*cur);
  }
...
bool of_STORE_VEC4A(vthread_t thr, vvp_code_t cp) {
  ...
  if (off < 0) {
    unsigned shift = -off;
    unsigned wid = value.size();
    value = value.subvalue(shift, wid - shift);
    off = 0;
  }
  unsigned size = cp->array->get_word_size();
  if (off + value.size() > size) {
    value = value.subvalue(0, size - off);
  }
...
bool of_SUBSTR_VEC4(vthread_t thr, vvp_code_t cp) {
  ...
  for (int idx = 0; idx < 8; idx += 1) {
    if (tmp & (1 << idx)) res.set_bit(idx, BIT4_1);
  }
...
bool of_FILE_LINE(vthread_t, vvp_code_t cp) {
  if (show_file_line) {
    ...
    cerr << vpi_get_str(vpiFile, handle) << ":" << vpi_get(vpiLineNo, handle)
         << ": ";
...
bool of_VPI_CALL(vthread_t thr, vvp_code_t cp) {
  ...
  if (schedule_stopped()) {
    if (!schedule_finished()) schedule_vthread(thr, 0, false);
