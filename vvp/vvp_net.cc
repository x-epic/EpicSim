...
void* vvp_net_t::operator new(size_t size) {
  ...
  if (vvp_net_alloc_remaining == 0) {
    ...
    vvp_net_pool = (vvp_net_t**)realloc(
        vvp_net_pool, vvp_net_pool_count * sizeof(vvp_net_t**));
...
#ifdef CHECK_WITH_VALGRIND
  VALGRIND_MEMPOOL_ALLOC(vvp_net_pool[vvp_net_pool_count - 1], return_this,
                         size);
...
void pool_local_net(vvp_net_t* net) {
  ...
  local_net_pool = (vvp_net_t**)realloc(
      local_net_pool, local_net_pool_count * sizeof(vvp_net_t**));
...
void vvp_net_delete(vvp_net_t* item) {
  ...
  if (vvp_fun_signal_object_sa* tmp =
          dynamic_cast<vvp_fun_signal_object_sa*>(item->fun)) {
    delete tmp;
  }
  /* Only delete static string variables. */
  if (vvp_fun_signal_string_sa* tmp =
          dynamic_cast<vvp_fun_signal_string_sa*>(item->fun)) {
    delete tmp;
  }
}
...
void vvp_net_pool_delete() {
  ...
  if (RUNNING_ON_VALGRIND && (vvp_nets_del != count_vvp_nets)) {
    ...
    VALGRIND_PRINTF("Error: vvp missed deleting %ld of %lu net(s).",
                    (long)count_vvp_nets - vvp_nets_del, count_vvp_nets);
  }
...
void vvp_net_t::operator delete(void*) { assert(0); }

vvp_net_t::vvp_net_t() : out_(vvp_net_ptr_t(0, 0)) {
  fun = 0;
  fil = 0;
}
#ifdef HAVE_READABLE_INFO
void vvp_net_t::add_readable_info(char* label, char* name) {
  if (name) strncpy(szName, name, DEBUG_INFO_LEN);

  if (label) strncpy(szLabel, label, DEBUG_INFO_LEN);
}

void vvp_net_t::add_readable_info(char* pInfo) {
  if (pInfo) strncpy(szInfo, pInfo, DEBUG_INFO_LEN);
}
...
void vvp_net_t::count_drivers(unsigned idx, unsigned counts[4]) {
  ...
  if (wire->is_forced(idx)) counts[3] = 1;
...
vvp_net_fil_t::~vvp_net_fil_t() { assert(force_link_ == 0); }

vvp_net_fil_t::prop_t vvp_net_fil_t::filter_vec4(const vvp_vector4_t&,
                                                 vvp_vector4_t&, unsigned,
                                                 unsigned) {
  return PROP;
}

vvp_net_fil_t::prop_t vvp_net_fil_t::filter_vec8(const vvp_vector8_t&,
                                                 vvp_vector8_t&, unsigned,
                                                 unsigned) {
  return PROP;
}

vvp_net_fil_t::prop_t vvp_net_fil_t::filter_real(double&) { return PROP; }

vvp_net_fil_t::prop_t vvp_net_fil_t::filter_long(long&) { return PROP; }
...
void vvp_net_fil_t::release_mask(const vvp_vector2_t& mask) {
  if (force_mask_.size() == 0) return;

  assert(force_mask_.size() == mask.size());
  for (unsigned idx = 0; idx < mask.size(); idx += 1) {
    if (mask.value(idx)) force_mask_.set_bit(idx, 0);
  }
...
void vvp_net_fil_t::operator delete(void*) { assert(0); }
...
vvp_bit4_t operator^(vvp_bit4_t a, vvp_bit4_t b) {
  if (bit4_is_xz(a)) return BIT4_X;
  if (bit4_is_xz(b)) return BIT4_X;
  if (a == BIT4_0) return b;
  if (b == BIT4_0) return a;
  return BIT4_0;
}
...
const edge_t vvp_edge_posedge =
    VVP_EDGE(BIT4_0, BIT4_1) | VVP_EDGE(BIT4_0, BIT4_X) |
    VVP_EDGE(BIT4_0, BIT4_Z) | VVP_EDGE(BIT4_X, BIT4_1) |
    VVP_EDGE(BIT4_Z, BIT4_1);

const edge_t vvp_edge_negedge =
    VVP_EDGE(BIT4_1, BIT4_0) | VVP_EDGE(BIT4_1, BIT4_X) |
    VVP_EDGE(BIT4_1, BIT4_Z) | VVP_EDGE(BIT4_X, BIT4_0) |
    VVP_EDGE(BIT4_Z, BIT4_0);

int edge(vvp_bit4_t from, vvp_bit4_t to) {
  edge_t mask = VVP_EDGE(from, to);
  if (mask & vvp_edge_posedge) return 1;
  if (mask & vvp_edge_negedge) return -1;
  return 0;
}
...
void vvp_send_vec8(vvp_net_ptr_t ptr, const vvp_vector8_t& val) {
  while (vvp_net_t* cur = ptr.ptr()) {
    ...
    if (cur->fun) cur->fun->recv_vec8(ptr, val);
...
void vvp_send_real(vvp_net_ptr_t ptr, double val, vvp_context_t context) {
  while (vvp_net_t* cur = ptr.ptr()) {
    ...
    if (cur->fun) cur->fun->recv_real(ptr, val, context);
...
void vvp_send_long(vvp_net_ptr_t ptr, long val) {
  while (vvp_net_t* cur = ptr.ptr()) {
    ...
    if (cur->fun) cur->fun->recv_long(ptr, val);

    ptr = next;
  }
}

void vvp_send_long_pv(vvp_net_ptr_t ptr, long val, unsigned base,
                      unsigned wid) {
  while (vvp_net_t* cur = ptr.ptr()) {
    vvp_net_ptr_t next = cur->port[ptr.port()];

    if (cur->fun) cur->fun->recv_long_pv(ptr, val, base, wid);
...
void vvp_vector4_t::copy_inverted_from_(const vvp_vector4_t& that) {
  ...
  if (size_ > BITS_PER_WORD) {
    ...
    for (idx = 0; idx < words; idx += 1) bbits_ptr_[idx] = that.bbits_ptr_[idx];
...
void vvp_vector4_t::allocate_words_(unsigned long inita, unsigned long initb) {
  if (size_ > BITS_PER_WORD) {
    ...
    for (unsigned idx = 0; idx < cnt; idx += 1) abits_ptr_[idx] = inita;
    for (unsigned idx = 0; idx < cnt; idx += 1) bbits_ptr_[idx] = initb;
...
vvp_vector4_t::vvp_vector4_t(unsigned size__, double val) : size_(size__) {
  bool is_neg = false;
  double fraction;
  int exponent;
...
vvp_vector4_t::vvp_vector4_t(const vvp_vector4_t& that, unsigned adr,
                             unsigned wid) {
...
unsigned long* vvp_vector4_t::subarray(unsigned adr, unsigned wid,
                                       bool xz_to_0) const {
  ...
  for (unsigned idx = 0; idx < awid; idx += 1) val[idx] = 0;
  ...
  if (size_ <= BITS_PER_WORD) {
    ...
    while (wid > 0) {
      ...
      if (trans > (BIT2_PER_WORD - val_off)) trans = BIT2_PER_WORD - val_off;
      if (wid < trans) trans = wid;
...
void vvp_vector4_t::setarray(unsigned adr, unsigned wid,
  ...
  if (size_ <= BITS_PER_WORD) {
    ...
    unsigned long hmask =
        ((adr + wid) < BITS_PER_WORD) ? -1UL << (adr + wid) : 0;
    ...
    while (wid > 0) {
      unsigned trans = wid;
      if (trans > (BIT2_PER_WORD - val_off)) trans = BIT2_PER_WORD - val_off;
      if (trans > (BITS_PER_WORD - off)) trans = BITS_PER_WORD - off;

      unsigned long lmask = (1UL << off) - 1UL;
      unsigned long hmask =
          ((off + trans) < BITS_PER_WORD) ? -1UL << (off + trans) : 0;
      unsigned long mask = ~(hmask | lmask);
...
void vvp_vector4_t::add(const vvp_vector4_t& that) {
  ...
  for (int idx = 0; idx < cnt; idx += 1) {
    if (bbits_ptr_[idx] | that.bbits_ptr_[idx]) goto x_out;

    abits_ptr_[idx] =
        add_with_carry(abits_ptr_[idx], that.abits_ptr_[idx], carry);
  }

  if (unsigned tail = size_ % BITS_PER_WORD) {
    unsigned long mask = ~(-1UL << tail);
    if ((bbits_ptr_[cnt] | that.bbits_ptr_[cnt]) & mask) goto x_out;

    abits_ptr_[cnt] =
        add_with_carry(abits_ptr_[cnt], that.abits_ptr_[cnt], carry);
    abits_ptr_[cnt] &= mask;
  }
...
void vvp_vector4_t::mov(unsigned dst, unsigned src, unsigned cnt) {
  ...
  if (size_ <= BITS_PER_WORD) {
    ...
    while (cnt > 0) {
      unsigned trans = cnt;
      if ((soff + trans) > BITS_PER_WORD) trans = BITS_PER_WORD - soff;

      if ((doff + trans) > BITS_PER_WORD) trans = BITS_PER_WORD - doff;
...
void vvp_vector4_t::mul(const vvp_vector4_t& that) {
  ...
  res[cnt - 1] &= mask;
  for (int idx = 0; idx < cnt; idx += 1) abits_ptr_[idx] = res[idx];
...
bool vvp_vector4_t::eeq(const vvp_vector4_t& that) const {
  ...
  for (unsigned idx = 0; idx < words; idx += 1) {
    if (abits_ptr_[idx] != that.abits_ptr_[idx]) return false;
    if (bbits_ptr_[idx] != that.bbits_ptr_[idx]) return false;
  }
...
bool vvp_vector4_t::eq_xz(const vvp_vector4_t& that) const {
  ...
  if (size_ < BITS_PER_WORD) {
    unsigned long mask = (1UL << size_) - 1;
    return ((abits_val_ | bbits_val_) & mask) ==
               ((that.abits_val_ | that.bbits_val_) & mask) &&
           (bbits_val_ & mask) == (that.bbits_val_ & mask);
  }
  ...
  unsigned words = size_ / BITS_PER_WORD;
  for (unsigned idx = 0; idx < words; idx += 1) {
    if ((abits_ptr_[idx] | bbits_ptr_[idx]) !=
  ...
  if (mask > 0) {
    mask = (1UL << mask) - 1;
    return ((abits_ptr_[words] | bbits_ptr_[words]) & mask) ==
               ((that.abits_ptr_[words] | that.bbits_ptr_[words]) & mask) &&
           (bbits_ptr_[words] & mask) == (that.bbits_ptr_[words] & mask);
  }
...
vvp_vector4_t& vvp_vector4_t::operator&=(const vvp_vector4_t& that) {
  ...
  if (size_ <= BITS_PER_WORD) {
    ...
    for (unsigned idx = 0; idx < words; idx += 1) {
      ...
      bbits_ptr_[idx] =
          (tmp1 & that.bbits_ptr_[idx]) | (tmp2 & bbits_ptr_[idx]);
    }
  }
...
vvp_vector4_t& vvp_vector4_t::operator|=(const vvp_vector4_t& that) {
  ...
  if (size_ <= BITS_PER_WORD) {
    unsigned long tmp =
        abits_val_ | bbits_val_ | that.abits_val_ | that.bbits_val_;
    ...
    for (unsigned idx = 0; idx < words; idx += 1) {
      unsigned long tmp = abits_ptr_[idx] | bbits_ptr_[idx] |
                          that.abits_ptr_[idx] | that.bbits_ptr_[idx];
      bbits_ptr_[idx] =
          ((~abits_ptr_[idx] | bbits_ptr_[idx]) & that.bbits_ptr_[idx]) |
          ((~that.abits_ptr_[idx] | that.bbits_ptr_[idx]) & bbits_ptr_[idx]);
      abits_ptr_[idx] = tmp;
    }
  }
...
vvp_vector4_t& vvp_vector4_t::operator+=(int64_t that) {
  ...
  for (idx = 0; idx < size(); idx += 1) {
    if (that == 0 && carry == BIT4_0) break;

    vvp_bit4_t that_bit = (that & 1) ? BIT4_1 : BIT4_0;
    that >>= 1;

    if (that_bit == BIT4_0 && carry == BIT4_0) continue;
...
template <class INT>
bool vector4_to_value(const vvp_vector4_t& vec, INT& val, bool is_signed,
                      bool is_arithmetic) {
  INT res = 0;
  INT msk = 1;
  bool rc_flag = true;
...
template <class T>
bool vector4_to_value(const vvp_vector4_t& vec, bool& overflow_flag, T& val) {
  T res = 0;
  T msk = 1;
...
void vvp_vector4array_t::set_word_(v4cell* cell, const vvp_vector4_t& that) {
  ...
  unsigned cnt = (width_ + vvp_vector4_t::BITS_PER_WORD - 1) /
                 vvp_vector4_t::BITS_PER_WORD;
...
vvp_vector4_t vvp_vector4array_t::get_word_(v4cell* cell) const {
  ...
  if (cell->abits_ptr_ == 0) return res;
...
vvp_vector4array_sa::~vvp_vector4array_sa() {
  if (array_) {
    if (width_ > vvp_vector4_t::BITS_PER_WORD) {
      for (unsigned idx = 0; idx < words_; idx += 1)
        if (array_[idx].abits_ptr_) delete[] array_[idx].abits_ptr_;
    }
    delete[] array_;
  }
}
...
vvp_vector4_t vvp_vector4array_sa::get_word(unsigned index) const {
  if (index >= words_) return vvp_vector4_t(width_, BIT4_X);
...
void vvp_vector4array_aa::reset_instance(vvp_context_t context) {
  v4cell* cell =
      static_cast<v4cell*>(vvp_get_context_item(context, context_idx_));
...
void vvp_vector4array_aa::set_word(unsigned index, const vvp_vector4_t& that) {
  ...
  v4cell* cell =
      static_cast<v4cell*>(vthread_get_wt_context_item(context_idx_)) + index;
...
vvp_vector4_t vvp_vector4array_aa::get_word(unsigned index) const {
  ...
  v4cell* cell =
      static_cast<v4cell*>(vthread_get_rd_context_item(context_idx_)) + index;
...
vvp_vector2_t::vvp_vector2_t(unsigned long v, unsigned wid) {
  ...
  vec_[0] = v;
  for (unsigned idx = 1; idx < words; idx += 1) vec_[idx] = 0;
}
...
vvp_vector2_t::vvp_vector2_t(vvp_vector2_t::fill_t fill, unsigned wid) {
  ...
  vec_ = new unsigned long[words];
  for (unsigned idx = 0; idx < words; idx += 1) vec_[idx] = fill ? -1 : 0;
}

vvp_vector2_t::vvp_vector2_t(const vvp_vector2_t& that, unsigned base,
                             unsigned wid) {
  wid_ = wid;
  const unsigned words = (wid_ + BITS_PER_WORD - 1) / BITS_PER_WORD;
...
void vvp_vector2_t::copy_from_that_(const vvp_vector2_t& that) {
  ...
  vec_ = new unsigned long[words];
  for (unsigned idx = 0; idx < words; idx += 1) vec_[idx] = that.vec_[idx];
}
...
vvp_vector2_t& vvp_vector2_t::operator<<=(unsigned int shift) {
  ...
  if (wshift >= words) {
    for (unsigned idx = 0; idx < words; idx += 1) vec_[idx] = 0;
  ...
  if (wshift > 0) {
    ...
    for (unsigned idx = 0; idx < wshift; idx += 1) vec_[idx] = 0;
  }
...
vvp_vector2_t& vvp_vector2_t::operator>>=(unsigned shift) {
  ...
  if (wshift > 0) {
    ...
    for (unsigned idx = words - wshift; idx < words; idx += 1) vec_[idx] = 0;
  }
...
static unsigned long add_carry(unsigned long a, unsigned long b,
  ...
  if ((ULONG_MAX - out) < a) carry += 1;
  out += a;

  if ((ULONG_MAX - out) < b) carry += 1;
  out += b;
...
bool vvp_vector2_t::is_NaN() const { return wid_ == 0; }
...
bool vvp_vector2_t::is_one() const {
  if (vec_[0] != 1) return false;
  const unsigned words = (wid_ + BITS_PER_WORD - 1) / BITS_PER_WORD;
  for (unsigned idx = 1; idx < words; idx += 1) {
    if (vec_[idx] != 0) return false;
  }

  return true;
}

bool vvp_vector2_t::is_all_one() const {
...
static void multiply_long(unsigned long a, unsigned long b, unsigned long& low,
                          unsigned long& high) {
...
vvp_vector2_t operator*(const vvp_vector2_t& a, const vvp_vector2_t& b) {
  ...
  for (unsigned bdx = 0; bdx < words; bdx += 1) {
    ...
    for (unsigned adx = 0; adx < words; adx += 1) {
      ...
      unsigned long carry = 0;
      for (unsigned sdx = 0; (adx + bdx + sdx) < words; sdx += 1) {
        r.vec_[adx + bdx + sdx] =
            add_carry(r.vec_[adx + bdx + sdx], low, carry);
        low = hig;
        hig = 0;
      }
    }
...
vvp_vector2_t operator-(const vvp_vector2_t& that) {
  ...
  const unsigned words = (neg.wid_ + neg.BITS_PER_WORD - 1) / neg.BITS_PER_WORD;
  for (unsigned idx = 0; idx < words; idx += 1) {
...
bool operator>(const vvp_vector2_t& a, const vvp_vector2_t& b) {
  const unsigned awords = (a.wid_ + vvp_vector2_t::BITS_PER_WORD - 1) /
                          vvp_vector2_t::BITS_PER_WORD;
  const unsigned bwords = (b.wid_ + vvp_vector2_t::BITS_PER_WORD - 1) /
                          vvp_vector2_t::BITS_PER_WORD;
...
bool operator<=(const vvp_vector2_t& a, const vvp_vector2_t& b) {
  ...
  for (unsigned idx = awords; idx > 0; idx -= 1) {
    if (a.vec_[idx - 1] < b.vec_[idx - 1]) return true;
    if (a.vec_[idx - 1] > b.vec_[idx - 1]) return false;
  }
...
bool c4string_test(const char* str) {
  if (strncmp(str, "C4<", 3) != 0) return false;
  size_t value_size = strspn(str + 3, "01xz");
  if (str[3 + value_size] != '>') return false;
  if (str[3 + value_size + 1] != 0) return false;
...
vvp_vector8_t::vvp_vector8_t(const vvp_vector4_t& that, unsigned str0,
                             unsigned str1)
    : size_(that.size()) {
  if (size_ == 0) return;
...
vvp_vector8_t::vvp_vector8_t(const vvp_vector2_t& that, unsigned str0,
                             unsigned str1)
    : size_(that.size()) {
  if (size_ == 0) return;
  ...
  if (size_ <= sizeof(val_)) {
    ...
    for (unsigned idx = 0; idx < size_; idx += 1)
      val_[idx] =
          vvp_scalar_t(that.value(idx) ? BIT4_1 : BIT4_0, str0, str1).raw();
  } else {
    ptr_ = new unsigned char[size_];
    for (unsigned idx = 0; idx < size_; idx += 1)
      ptr_[idx] =
...
vvp_vector8_t& vvp_vector8_t::operator=(const vvp_vector8_t& that) {
  ...
  if (size_ != that.size_) {
    if (size_ > sizeof(val_)) delete[] ptr_;
    size_ = 0;
  }
...
vvp_vector8_t part_expand(const vvp_vector8_t& that, unsigned wid,
                          unsigned off) {
  assert(off < wid);
  vvp_vector8_t tmp(wid);

  unsigned char* tmp_ptr = tmp.size_ <= sizeof(tmp.val_) ? tmp.val_ : tmp.ptr_;
  const unsigned char* that_ptr =
      that.size_ <= sizeof(that.val_) ? that.val_ : that.ptr_;
...
vvp_net_fun_t::vvp_net_fun_t() { count_functors += 1; }
...
void vvp_net_fun_t::recv_vec4_pv(vvp_net_ptr_t, const vvp_vector4_t& bit,
  ...
  cerr << "internal error: " << typeid(*this).name() << ": "
       << "recv_vec4_pv(" << bit << ", " << base << ", " << wid << ", " << vwid
...
void vvp_net_fun_t::recv_string(vvp_net_ptr_t, const std::string& bit,
                                vvp_context_t) {
...
void vvp_wide_fun_core::propagate_real(double bit, vvp_time64_t delay) {
  if (delay) {
...
unsigned vvp_wide_fun_core::port_count() const { return nports_; }
...
void vvp_wide_fun_core::recv_real_from_inputs(unsigned) { assert(0); }
...
void vvp_wide_fun_core::dispatch_real_from_input_(unsigned port, double bit) {
  assert(port < nports_);
...
void vvp_wide_fun_t::recv_real(vvp_net_ptr_t port, double bit, vvp_context_t) {
  unsigned pidx = port_base_ + port.port();
...
unsigned vvp_scalar_t::strength0() const { return STREN0(value_); }

unsigned vvp_scalar_t::strength1() const { return STREN1(value_); }
...
vvp_scalar_t fully_featured_resolv_(vvp_scalar_t a, vvp_scalar_t b) {
  ...
  if (sv0a > sv1) sv1 = sv0a;
  if (sv1b > sv1) sv1 = sv1b;
  if (sv0b > sv1) sv1 = sv0b;

  if (sv1a < sv0) sv0 = sv1a;
  if (sv1b < sv0) sv0 = sv1b;
  if (sv0b < sv0) sv0 = sv0b;
  ...
  if ((res.value_ & 0x77) == 0) res.value_ = 0;
...
vvp_bit4_t compare_gtge(const vvp_vector4_t& lef, const vvp_vector4_t& rig,
  ...
  if (lef.has_xz()) return BIT4_X;

  // Check for X/Z in the right operand
  if (rig.has_xz()) return BIT4_X;

  for (unsigned idx = lef.size(); idx > rig.size(); idx -= 1) {
    if (lef.value(idx - 1) == BIT4_1) return BIT4_1;
  }

  for (unsigned idx = rig.size(); idx > lef.size(); idx -= 1) {
    if (rig.value(idx - 1) == BIT4_1) return BIT4_0;
  }
...
vvp_bit4_t compare_gtge_signed(const vvp_vector4_t& a, const vvp_vector4_t& b,
                               vvp_bit4_t out_if_equal) {
  assert(a.size() == b.size());
  ...
  if (bit4_is_xz(a_sign)) return BIT4_X;
  if (bit4_is_xz(b_sign)) return BIT4_X;

  if (a_sign == b_sign) return compare_gtge(a, b, out_if_equal);

  if (a.has_xz()) return BIT4_X;

  if (b.has_xz()) return BIT4_X;
