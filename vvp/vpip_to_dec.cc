 ...
 */
static inline void shift_in(unsigned long *valv, unsigned int vlen,
                            unsigned long val) {
  unsigned int i;
  /* printf("shift in %u\n",val); */
  for (i = 0; i < vlen; i++) {
 ...
 */
static inline int write_digits(unsigned long v, char **buf, unsigned int *nbuf,
                               int zero_suppress) {
...
unsigned vpip_vec4_to_dec_str(const vvp_vector4_t &vec4, char *buf,
...
#define ALLOC_MARGIN 4
  ...
  if (count_x == vec4.size()) {
    ...
    for (i = vlen - 1; i >= 0; i--) {
      zero_suppress = write_digits(valv[i], &buf, &nbuf, zero_suppress);
      /* printf(",%.4u",valv[i]); */
    }
...
void vpip_dec_str_to_vec4(vvp_vector4_t &vec, const char *buf) {
  ...
  for (unsigned idx = 0; idx < slen; idx += 1) {
    ...
    if (isdigit(buf[slen - idx - 1]))
      ...
      fprintf(stderr,
              "Warning: Invalid decimal digit %c(%d) in "
              "\"%s.\"\n",
