...
void vvp_delay_t::calculate_min_delay_() {
  min_delay_ = rise_;
  if (fall_ < min_delay_) min_delay_ = fall_;
  if (ignore_decay_)
    decay_ = min_delay_;
...
vvp_time64_t vvp_delay_t::get_min_delay() const { return min_delay_; }
...
vvp_fun_delay::~vvp_fun_delay() {
  while (struct event_ *cur = dequeue_()) delete cur;
}
...
bool vvp_fun_delay::clean_pulse_events_(vvp_time64_t use_delay, double bit) {
  if (list_ == 0) return false;
...
void vvp_fun_delay::clean_pulse_events_(vvp_time64_t use_delay) {
  ...
  do {
    ...
    if (cur->sim_time + use_delay <= use_delay + schedule_simtime()) break;
...
void vvp_fun_delay::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t &bit,
  ...
  if (initial_) {
    ...
    const vvp_vector4_t &use_vec4 =
        (list_ && list_->next->sim_time == schedule_simtime())
            ? list_->next->ptr_vec4
            : cur_vec4_;
...
void vvp_fun_delay::recv_vec8(vvp_net_ptr_t port, const vvp_vector8_t &bit) {
  ...
  if (initial_) {
    ...
    /* Scan the vectors looking for delays. Select the maximum
       delay encountered. */
    use_delay =
        delay_.get_delay(use_vec8.value(0).value(), bit.value(0).value());

    for (unsigned idx = 1; idx < use_wid; idx += 1) {
      vvp_time64_t tmp;
      tmp =
          delay_.get_delay(use_vec8.value(idx).value(), bit.value(idx).value());
      if (tmp > use_delay) use_delay = tmp;
    }
  }
...
void vvp_fun_delay::recv_real(vvp_net_ptr_t port, double bit, vvp_context_t) {
  if (port.port() > 0) {
...
void vvp_fun_delay::run_run() {
  ...
  if (list_ == 0 || list_->next->sim_time > sim_time) return;
...
void vvp_fun_modpath::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t &bit,
  ...
  for (vvp_fun_modpath_src *cur = src_list_; cur; cur = cur->next_) {
    ...
    if (cur->condition_flag_ == false) continue;
  ...
  for (++cur; cur != candidate_list.end(); ++cur) {
    ...
    for (unsigned idx = 0; idx < 12; idx += 1) {
      ...
      if (tmp <= now)
        ...
        tmp -= now;
      if (tmp < out_at[idx]) out_at[idx] = tmp;
    }
  }

  /* Given the scheduled output time, create an output event. */
  vvp_time64_t use_delay =
      delay_from_edge(cur_vec4_.value(0), bit.value(0), out_at);
  ...
  for (unsigned idx = 1; idx < bit.size(); idx += 1) {
    vvp_time64_t tmp =
        delay_from_edge(cur_vec4_.value(idx), bit.value(idx), out_at);
...
void vvp_fun_modpath::run_run() { net_->send_vec4(cur_vec4_, 0); }

vvp_fun_modpath_src::vvp_fun_modpath_src(vvp_time64_t del[12]) {
  for (unsigned idx = 0; idx < 12; idx += 1) delay_[idx] = del[idx];
...
void vvp_fun_modpath_src::get_delay12(vvp_time64_t val[12]) const {
  for (unsigned idx = 0; idx < 12; idx += 1) val[idx] = delay_[idx];
}

void vvp_fun_modpath_src::put_delay12(const vvp_time64_t val[12]) {
  for (unsigned idx = 0; idx < 12; idx += 1) delay_[idx] = val[idx];
}

void vvp_fun_modpath_src::recv_vec4(vvp_net_ptr_t port,
                                    const vvp_vector4_t &bit, vvp_context_t) {
  if (port.port() == 0) {
    // The modpath input...
    if (test_vec4(bit)) wake_time_ = schedule_simtime();
...
bool vvp_fun_modpath_src::test_vec4(const vvp_vector4_t &) { return true; }

vvp_fun_modpath_edge::vvp_fun_modpath_edge(vvp_time64_t del[12], bool pos,
                                           bool neg)
...
static vpiHandle modpath_src_get_handle(int code, vpiHandle ref) {
  ...
  switch (code) {
    ...
    case vpiModule: {
      ...
      while (scope && scope->get_type_code() != vpiModule) scope = scope->scope;
      assert(scope);
      return scope;
    }
...
static void modpath_src_get_delays(vpiHandle ref, p_vpi_delay delays) {
  ...
  if (delays->time_type == vpiSimTime) {
    ...
    for (idx = 0; idx < delays->no_of_delays; idx += 1) {
      delays->da[idx].real =
          vpip_time_to_scaled_real(tmp[idx], src->dest->scope);
    }
  }
}
...
int __vpiModPathSrc::get_type_code(void) const { return vpiModPath; }

int __vpiModPathSrc::vpi_get(int code) { return modpath_src_get(code, this); }
...
int __vpiModPathTerm::get_type_code(void) const { return vpiPathTerm; }

int __vpiModPathTerm::vpi_get(int code) { return pathterm_get(code, this); }
...
#ifdef CHECK_WITH_VALGRIND
  mp_count += 1;
  mp_list = (struct __vpiModPath **)realloc(
      mp_list, mp_count * sizeof(struct __vpiModPath **));
