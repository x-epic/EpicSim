...
static void function_argument_logic(ivl_signal_t port, ivl_expr_t expr) {
  ...
  draw_eval_vec4(expr);
  if (ewidth < pwidth) fprintf(vvp_out, "    %%pad/u %u;\n", pwidth);
}
...
static void draw_ufunc_preamble(ivl_expr_t expr) {
  ...
  switch (ivl_expr_value(expr)) {
    case IVL_VT_VOID:
      ...
      break;
    case IVL_VT_REAL:
      fprintf(vvp_out, "    %%callf/real TD_%s",
              vvp_mangle_id(ivl_scope_name(def)));
      fprintf(vvp_out, ", S_%p;\n", def);
      break;
    case IVL_VT_BOOL:
    case IVL_VT_LOGIC:
      fprintf(vvp_out, "    %%callf/vec4 TD_%s",
              vvp_mangle_id(ivl_scope_name(def)));
      fprintf(vvp_out, ", S_%p;\n", def);
      break;
    case IVL_VT_STRING:
      fprintf(vvp_out, "    %%callf/str TD_%s",
              vvp_mangle_id(ivl_scope_name(def)));
      fprintf(vvp_out, ", S_%p;\n", def);
      break;
    case IVL_VT_CLASS:
    case IVL_VT_DARRAY:
    case IVL_VT_QUEUE:
      fprintf(vvp_out, "    %%callf/obj TD_%s",
              vvp_mangle_id(ivl_scope_name(def)));
      fprintf(vvp_out, ", S_%p;\n", def);
      break;
    default:
