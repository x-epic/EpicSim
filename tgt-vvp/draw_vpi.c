...
static const char *magic_sfuncs[] = {"$time", "$stime", "$realtime", "$simtime",
                                     0};

static int is_magic_sfunc(const char *name) {
  int idx;
  for (idx = 0; magic_sfuncs[idx]; idx += 1)
    if (strcmp(magic_sfuncs[idx], name) == 0) return 1;
...
static int is_fixed_memory_word(ivl_expr_t net) {
  ...
  if (ivl_expr_type(net) != IVL_EX_SIGNAL) return 0;

  sig = ivl_expr_signal(net);

  if (ivl_signal_dimensions(sig) == 0) return 1;

  if (ivl_signal_type(sig) == IVL_SIT_REG) return 0;

  if (number_is_immediate(ivl_expr_oper1(net), IMM_WID, 0)) return 1;
...
static int get_vpi_taskfunc_signal_arg(struct args_info *result,
  ...
  switch (ivl_expr_type(expr)) {
    case IVL_EX_SIGNAL:
      ...
      if (ivl_expr_width(expr) != ivl_signal_width(ivl_expr_signal(expr))) {
        ...
        if (word_ex && (ivl_expr_type(word_ex) == IVL_EX_SIGNAL ||
          ...
          if (get_vpi_taskfunc_signal_arg(result->child, word_ex)) {
            snprintf(buffer, sizeof buffer, "&A<v%p, %s >", sig,
                     result->child->text);
    ...
    case IVL_EX_SELECT: {
      ...
      if (ivl_expr_type(vexpr) != IVL_EX_SIGNAL &&
          ivl_expr_type(vexpr) != IVL_EX_SELECT)
        return 0;
      ...
      if (ivl_expr_value(vexpr) == IVL_VT_STRING) return 0;

      /* If the sub-expression is a DARRAY, then this select
         is a dynamic-array word select. Handle that
         elsewhere. */
      if (ivl_expr_value(vexpr) == IVL_VT_DARRAY) return 0;
      ...
      if (wexpr && number_is_immediate(wexpr, 64, 1) &&
        ...
        snprintf(buffer, sizeof buffer, "&APV<v%p, %ld, %ld, %u>",
                 ivl_expr_signal(vexpr), get_number_immediate(wexpr),
                 get_number_immediate(bexpr), ivl_expr_width(expr));
        ...
        snprintf(buffer, sizeof buffer, "&PV<v%p_0, %ld, %u>",
                 ivl_expr_signal(vexpr), get_number_immediate(bexpr),
                 ivl_expr_width(expr));
        ...
        if (get_vpi_taskfunc_signal_arg(result->child, bexpr)) {
          snprintf(buffer, sizeof buffer, "&PV<v%p_0, %s, %u>",
                   ivl_expr_signal(vexpr), result->child->text,
                   ivl_expr_width(expr));
          free(result->child->text);
        } else {
          free(result->child);
...
static void draw_vpi_taskfunc_args(const char *call_string,
                                   ivl_statement_t tnet, ivl_expr_t fnet) {
  unsigned idx;
  unsigned parm_count = tnet ? ivl_stmt_parm_count(tnet) : ivl_expr_parms(fnet);
  ...
  for (idx = 0; idx < parm_count; idx += 1) {
    ivl_expr_t expr =
        tnet ? ivl_stmt_parm(tnet, idx) : ivl_expr_parm(fnet, idx);
    ...
    switch (ivl_expr_type(expr)) {
      ...
      case IVL_EX_ARRAY:
        snprintf(buffer, sizeof buffer, "v%p", ivl_expr_signal(expr));
      ...
      case IVL_EX_NUMBER: {
        if ((par = ivl_expr_parameter(expr))) {
          ...
          snprintf(buffer, sizeof buffer, "%u'%sb", wid,
                   ivl_expr_signed(expr) ? "s" : "");
          dp = buffer + strlen(buffer);
          for (bit = wid; bit > 0; bit -= 1) *dp++ = bits[bit - 1];
          *dp++ = 0;
          assert(dp >= buffer);
      ...
      case IVL_EX_STRING:
        if ((par = ivl_expr_parameter(expr))) {
          ...
          snprintf(args[idx].text, needed_len, "\"%s\"", ivl_expr_string(expr));
        }
        continue;
  ...
  fprintf(vvp_out, " {%u %u %u}", vec4_stack_need, real_stack_need,
          str_stack_need);
  fprintf(vvp_out, ";\n");
}
...
void draw_vpi_task_call(ivl_statement_t tnet) {
  ...
  if (parm_count == 0) {
    fprintf(vvp_out, "    %s %u %u \"%s\" {0 0 0};\n", command,
            ivl_file_table_index(ivl_stmt_file(tnet)), ivl_stmt_lineno(tnet),
            ivl_stmt_name(tnet));
  } else {
...
void draw_vpi_func_call(ivl_expr_t fnet) {
  ...
  sprintf(call_string, "    %%vpi_func %u %u \"%s\" %u",
          ivl_file_table_index(ivl_expr_file(fnet)), ivl_expr_lineno(fnet),
          ivl_expr_name(fnet), ivl_expr_width(fnet));
