...
unsigned width_of_nexus(ivl_nexus_t nex) {
  ...
  for (idx = 0; idx < ivl_nexus_ptrs(nex); idx += 1) {
    ivl_nexus_ptr_t ptr = ivl_nexus_ptr(nex, idx);
    ivl_signal_t sig = ivl_nexus_ptr_sig(ptr);
    if (sig != 0) return ivl_signal_width(sig);
  }
...
ivl_variable_type_t data_type_of_nexus(ivl_nexus_t nex) {
  ...
  for (idx = 0; idx < ivl_nexus_ptrs(nex); idx += 1) {
    ivl_nexus_ptr_t ptr = ivl_nexus_ptr(nex, idx);
    ivl_signal_t sig = ivl_nexus_ptr_sig(ptr);
    if (sig != 0) return ivl_signal_data_type(sig);
  }
...
static ivl_nexus_ptr_t ivl_logic_pin_ptr(ivl_net_logic_t net, unsigned pin) {
  ...
  for (idx = 0; idx < ivl_nexus_ptrs(nex); idx += 1) {
    ...
    if (tmp == 0) continue;
    if (tmp != net) continue;
    if (ivl_nexus_ptr_pin(ptr) != pin) continue;
...
int signal_is_return_value(ivl_signal_t sig) {
  ...
  if (ivl_scope_type(sig_scope) != IVL_SCT_FUNCTION) return 0;
...
int can_elide_bufz(ivl_net_logic_t net, ivl_nexus_ptr_t nptr) {
  ...
  for (idx = 0; idx < ivl_nexus_ptrs(in_n); idx += 1) {
    ...
    if (ivl_nexus_ptr_drive0(in_np) != dr0) return 0;
  ...
  for (idx = 0; idx < ivl_nexus_ptrs(out_n); idx += 1) {
    ...
    if (out_sig && ivl_signal_forced_net(out_sig)) return 0;
  }
...
const char *draw_input_from_net(ivl_nexus_t nex) {
  ...
  if (sig == 0) return draw_net_input(nex);
...
static void draw_reg_in_scope(ivl_signal_t sig) {
  ...
  const char *datatype_flag =
      ivl_signal_integer(sig) ? "/i" : ivl_signal_signed(sig) ? "/s" : "";
  ...
  if (ivl_signal_dimensions(sig) > 0 && vector_dims == 0) {
    ...
    fprintf(vvp_out, "v%p .array%s \"%s\", %d %d;\n", sig, datatype_flag,
            vvp_mangle_name(ivl_signal_basename(sig)), swapped ? first : last,
            swapped ? last : first);
    ...
    fprintf(vvp_out, "v%p_0 .var%s %s\"%s\", %d %d;%s\n", sig, datatype_flag,
            local_flag, vvp_mangle_name(ivl_signal_basename(sig)), msb, lsb,
            ivl_signal_local(sig) ? " Local signal" : "");
  }
}
...
static void draw_net_in_scope(ivl_signal_t sig) {
  ...
  for (iword = 0; iword < ivl_signal_array_count(sig); iword += 1) {
    ...
    if (nex_data->net == 0) {
      ...
      const char *vec8 = "";
      if (nex_data->flags & VVP_NEXUS_DATA_STR) strength_aware_flag = 1;
      if (nex_data->drivers_count > 1) vec8 = "8";
      if (strength_aware_flag) vec8 = "8";
      ...
      if (dimensions > 0) {
        ...
        fprintf(vvp_out,
                ...
                vvp_mangle_name(ivl_signal_basename(sig)), msb, lsb, driver,
                nex_data->drivers_count,
                strength_aware_flag ? ", strength-aware" : "");
      }
      nex_data->net = sig;
      ...
      if (word_count == ivl_signal_array_count(nex_data->net)) {
        if (iword == 0) {
          fprintf(vvp_out, "v%p .array \"%s\", v%p; Alias to %s \n", sig,
                  vvp_mangle_name(ivl_signal_basename(sig)), nex_data->net,
        ...
        fprintf(vvp_out,
                "v%p_%u .net%s v%p %u, %d %d, "
                "v%p_%u; Alias to %s\n",
                sig, iword, datatype_flag, sig, iword, msb, lsb, nex_data->net,
                nex_data->net_word, ivl_signal_basename(nex_data->net));
      }
    } else {
...
static void draw_udp_def(ivl_udp_t udp) {
  ...
  if (ivl_udp_sequ(udp))
    fprintf(vvp_out, "UDP_%s .udp/sequ \"%s\", %u, %u",
            vvp_mangle_id(ivl_udp_name(udp)),
            vvp_mangle_name(ivl_udp_name(udp)), ivl_udp_nin(udp), init);
  else
    fprintf(vvp_out, "UDP_%s .udp/comb \"%s\", %u",
            vvp_mangle_id(ivl_udp_name(udp)),
            vvp_mangle_name(ivl_udp_name(udp)), ivl_udp_nin(udp));
...
static int udp_has_modpath_output(ivl_net_logic_t lptr) {
  ...
  for (idx = 0; idx < ivl_nexus_ptrs(nex); idx += 1) {
    ...
    if ((ivl_signal_scope(sig) == scope) &&
        (ivl_signal_port(sig) == IVL_SIP_OUTPUT) && (ivl_signal_npath(sig)))
      return 1;
  }
...
static void draw_logic_in_scope(ivl_net_logic_t lptr) {
  ...
  switch (ivl_logic_type(lptr)) {
    ...
    case IVL_LO_BUFZ: {
      ...
      if (can_elide_bufz(lptr, nptr)) return;
  ...
  while (ninp) {
    ...
    for (inst = 0; inst < (unsigned)ninp; inst += 4) {
      if (ninp > 4)
        fprintf(vvp_out, "L_%p/%d/%u .functor %s %u", lptr, level, inst, lcasc,
                vector_width);
      else {
        fprintf(vvp_out, "L_%p%s .functor %s %u", lptr,
                need_delay_flag ? "/d" : "", ltype, vector_width);
      ...
      for (pdx = inst; pdx < (unsigned)ninp && pdx < inst + 4; pdx += 1) {
        if (level) {
          fprintf(vvp_out, ", L_%p/%d/%u", lptr, level - 1, pdx * 4);
        } else {
          fprintf(vvp_out, ", %s", input_strings[pdx]);
        }
      }
...
static void draw_event_in_scope(ivl_event_t obj) {
  ...
  if (nany > 0) cnt += (nany + ntmp - 1) / ntmp;

  if (nneg > 0) cnt += (nneg + ntmp - 1) / ntmp;

  if (npos > 0) cnt += (npos + ntmp - 1) / ntmp;
  ...
  if (cnt == 0) {
    ...
    for (idx = 1; idx < cnt; idx += 1) fprintf(vvp_out, ", E_%p/%u", obj, idx);
...
static void draw_lpm_add(ivl_lpm_t net) {
  ...
  if (dta == IVL_VT_REAL || dtb == IVL_VT_REAL) dto = IVL_VT_REAL;
...
static void draw_lpm_cmp(ivl_lpm_t net) {
  ...
  if (dta == IVL_VT_REAL || dtb == IVL_VT_REAL) dtc = IVL_VT_REAL;
...
static unsigned lpm_concat_inputs(ivl_lpm_t net, unsigned start, unsigned cnt,
  ...
  for (; idx < 4; idx += 1) fprintf(vvp_out, " 0");
...
static void draw_lpm_concat(ivl_lpm_t net) {
  ...
  if (icnt <= 4) {
    ...
    for (idx = 0; idx < icnt; idx += 4) {
      unsigned wid = 0;
      unsigned trans = 4;
      if ((idx + trans) > icnt) trans = icnt - idx;
    ...
    while (icnt > 4) {
      for (idx = 0; idx < icnt; idx += 4) {
        ...
        fprintf(vvp_out, "LS_%p_%u_%u .concat%s [", net, depth, idx, z);
        ...
        for (; tdx < 4; tdx += 1) fprintf(vvp_out, " 0");

        fprintf(vvp_out, "]");

        for (tdx = 0; tdx < trans; tdx += 1) {
          fprintf(vvp_out, ", LS_%p_%u_%u", net, depth - 1,
                  tree[idx + tdx].base);
        }
    ...
    fprintf(vvp_out, "L_%p%s .concat%s [", net, dly, z);
    for (idx = 0; idx < icnt; idx += 1) fprintf(vvp_out, " %u", tree[idx].wid);
    for (; idx < 4; idx += 1) fprintf(vvp_out, " 0");
    fprintf(vvp_out, "]");

    for (idx = 0; idx < icnt; idx += 1)
      fprintf(vvp_out, ", LS_%p_%u_%u", net, depth - 1, tree[idx].base);
...
static void draw_lpm_ff(ivl_lpm_t net) {
  ...
  if (ivl_lpm_async_clr(net)) {
    ...
    if (ivl_lpm_async_set(net)) {
      fprintf(stderr,
              "%s:%u:vvp.tgt: sorry: No support for a DFF "
...
static void draw_lpm_sfunc(ivl_lpm_t net) {
  ...
  if (ivl_lpm_trigger(net))
    fprintf(vvp_out, "L_%p%s .sfunc/e %u %u \"%s\", E_%p", net, dly,
            ...
            ivl_lpm_string(net), ivl_lpm_trigger(net));
  else
...
static void draw_lpm_ufunc(ivl_lpm_t net) {
  ...
  if (ivl_lpm_trigger(net))
    fprintf(vvp_out, "L_%p%s .ufunc/e TD_%s, %u, E_%p", net, dly,
            vvp_mangle_id(ivl_scope_name(def)), ivl_lpm_width(net),
            ivl_lpm_trigger(net));
  else
...
static void draw_lpm_part(ivl_lpm_t net) {
  ...
  if (sel == 0) {
    fprintf(vvp_out, "L_%p%s .part %s", net, dly,
...
static void draw_lpm_part_pv(ivl_lpm_t net) {
  ...
  fprintf(vvp_out, "L_%p .part/pv %s", net,
          draw_net_input(ivl_lpm_data(net, 0)));
...
static void draw_lpm_repeat(ivl_lpm_t net) {
  ...
  fprintf(vvp_out, "L_%p%s .repeat %u, %u, %s;\n", net, dly, ivl_lpm_width(net),
          ivl_lpm_size(net), draw_net_input(ivl_lpm_data(net, 0)));
}
...
int draw_scope(ivl_scope_t net, ivl_scope_t parent) {
  ...
  fprintf(vvp_out, "S_%p .scope %s%s%s, \"%s\" \"%s\" %u %u", net, prefix, type,
          suffix, vvp_mangle_name(ivl_scope_basename(net)),
          vvp_mangle_name(ivl_scope_tname(net)),
          ivl_file_table_index(ivl_scope_file(net)), ivl_scope_lineno(net));
  ...
  if (ivl_scope_type(net) == IVL_SCT_MODULE) {
    ...
    for (idx = 0; idx < ivl_scope_mod_module_ports(net); ++idx) {
      ...
      fprintf(vvp_out, "    .port_info %u %s %u \"%s\";\n", idx,
              vvp_port_info_type_str(ptype), width, vvp_mangle_name(name));
    }
  }
    ...
    switch (ivl_expr_type(pex)) {
      case IVL_EX_STRING:
        fprintf(vvp_out, "P_%p .param/str \"%s\" %d %u %u, \"%s\";\n", par,
                ...
                ivl_parameter_lineno(par), ivl_expr_string(pex));
        break;
      case IVL_EX_NUMBER:
        fprintf(vvp_out, "P_%p .param/l \"%s\" %d %u %u, %sC4<", par,
                vvp_mangle_name(ivl_parameter_basename(par)),
                ivl_parameter_local(par),
                ivl_file_table_index(ivl_parameter_file(par)),
                ivl_parameter_lineno(par), ivl_expr_signed(pex) ? "+" : "");
        {
      ...
      case IVL_EX_REALNUM: {
        ...
        fprintf(vvp_out,
                "P_%p .param/real \"%s\" %d %u %u, %s; "
                "value=%#g\n",
                par, vvp_mangle_name(ivl_parameter_basename(par)),
                ivl_parameter_local(par),
                ivl_file_table_index(ivl_parameter_file(par)),
                ivl_parameter_lineno(par), res, ivl_expr_dvalue(pex));
        free(res);
      } break;
      default:
  ...
  if (ivl_scope_type(net) == IVL_SCT_TASK) draw_task_definition(net);
