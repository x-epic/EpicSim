...
void resize_vec4_wid(ivl_expr_t expr, unsigned wid) {
  if (ivl_expr_width(expr) == wid) return;
...
int test_immediate_vec4_ok(ivl_expr_t re) {
  ...
  if (ivl_expr_type(re) != IVL_EX_NUMBER) return 0;

  if (ivl_expr_width(re) <= 32) return 1;

  bits = ivl_expr_bits(re);

  for (idx = 32; idx < ivl_expr_width(re); idx += 1) {
    if (bits[idx] != '0') return 0;
  }

  return 1;
}

static void make_immediate_vec4_words(ivl_expr_t re, unsigned long* val0p,
                                      unsigned long* valxp, unsigned* widp) {
...
static void draw_binary_vec4_arith(ivl_expr_t expr) {
  ...
  int signed_flag =
      (ivl_expr_signed(le) || is_power_op) && ivl_expr_signed(re) ? 1 : 0;
...
static void draw_binary_vec4_compare_class(ivl_expr_t expr) {
  ...
  if (ivl_expr_type(re) == IVL_EX_NULL &&
      ivl_expr_type(le) == IVL_EX_PROPERTY) {
    ivl_signal_t sig = ivl_expr_signal(le);
...
static void draw_binary_vec4_land(ivl_expr_t expr) {
  ...
  /* Push the left expression. Reduce it to a single bit if
     necessary. */
  draw_eval_vec4(le);
  if (ivl_expr_width(le) > 1) fprintf(vvp_out, "    %%or/r;\n");

  /* Now push the right expression. Again, reduce to a single
     bit if necessary. */
  draw_eval_vec4(re);
  if (ivl_expr_width(re) > 1) fprintf(vvp_out, "    %%or/r;\n");
...
static void draw_select_pad_vec4(ivl_expr_t expr) {
  ...
  if (wid == ivl_expr_width(subexpr)) return;
...
static void draw_sfunc_vec4(ivl_expr_t expr) {
  ...
  if (parm_count == 0) {
    ...
    fprintf(vvp_out, "    %%vpi_func %u %u \"%s\" %u {0 0 0};\n",
            ivl_file_table_index(ivl_expr_file(expr)), ivl_expr_lineno(expr),
            ivl_expr_name(expr), ivl_expr_width(expr));
    return;
  }
...
static void draw_unary_vec4(ivl_expr_t expr) {
  ...
  if (debug_draw) {
    fprintf(vvp_out, " ; %s:%u:draw_unary_vec4: opcode=%c\n",
            ivl_expr_file(expr), ivl_expr_lineno(expr), ivl_expr_opcode(expr));
  }
...
void draw_eval_vec4(ivl_expr_t expr) {
  if (debug_draw) {
    fprintf(vvp_out, " ; %s:%u:draw_eval_vec4: expr_type=%d\n",
            ivl_expr_file(expr), ivl_expr_lineno(expr), ivl_expr_type(expr));
  }
