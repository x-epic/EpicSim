...
static void get_vec_from_lval_slice(ivl_lval_t lval,
                                    struct vec_slice_info* slice,
                                    unsigned wid) {
  ...
  if (ivl_signal_dimensions(sig) == 0 && part_off_ex == 0 && word_ix == 0 &&
  ...
  } else if (ivl_signal_dimensions(sig) == 0 && part_off_ex == 0 &&
             word_ix == 0) {
    assert(use_word == 0);
    ...
    draw_eval_expr_into_integer(word_ix,
 ...
 * results back.
 */
static void get_vec_from_lval(ivl_statement_t net,
  ...
  switch (slice->type) {
    ...
    case SLICE_MEMORY_WORD_STATIC:
      if (slice->u_.memory_word_static.use_word < ivl_signal_array_count(sig)) {
        ...
        fprintf(vvp_out, "    %%flag_set/imm 4, 0;\n");
        fprintf(vvp_out, "    %%ix/load %d, %lu, 0;\n", word_idx,
                slice->u_.memory_word_static.use_word);
...
static void put_vec_to_lval(ivl_statement_t net,
                            struct vec_slice_info* slices) {
  unsigned lidx;
  unsigned cur_bit;
...
static ivl_type_t draw_lval_expr(ivl_lval_t lval) {
  ...
  if (ivl_lval_idx(lval)) {
    fprintf(vvp_out,
            " ; XXXX Don't know how to handle ivl_lval_idx values here.\n");
  }
...
static void store_vec4_to_lval(ivl_statement_t net) {
  for (unsigned lidx = 0; lidx < ivl_stmt_lvals(net); lidx += 1) {
    ...
    if (word_ex) {
      ...
      clr_word(word_index);
      if (part_index) clr_word(part_index);
      ...
      if (signal_is_return_value(lsig)) {
        fprintf(vvp_out,
                "    %%ret/vec4 0, %d, %u; Assign to %s (store_vec4_to_lval)\n",
      ...
      if (signal_is_return_value(lsig)) {
        fprintf(vvp_out,
                "    %%ret/vec4 0, 0, %u;  Assign to %s (store_vec4_to_lval)\n",
...
static int show_stmt_assign_vector(ivl_statement_t net) {
  ...
  if (ivl_stmt_opcode(net) != 0) {
    fprintf(vvp_out,
            "    ; show_stmt_assign_vector: Get l-value for compressed %c= "
            "operand\n",
            ivl_stmt_opcode(net));
...
static int show_stmt_assign_sig_real(ivl_statement_t net) {
  ...
  if (ivl_stmt_opcode(net) != 0) {
    fprintf(
        vvp_out,
        "    ; show_stmt_assign_real: Get l-value for compressed %c= operand\n",
        ivl_stmt_opcode(net));
...
static int show_stmt_assign_darray_pattern(ivl_statement_t net) {
  ...
  for (idx = 0; idx < ivl_expr_parms(rval); idx += 1) {
    switch (ivl_type_base(element_type)) {
      ...
      default:
        fprintf(vvp_out,
                "; ERROR: show_stmt_assign_darray_pattern: type_base=%d not "
                "implemented\n",
    ...
    if (ivl_type_base(prop_type) == IVL_VT_BOOL) {
      assert(ivl_type_packed_dimensions(prop_type) == 0 ||
             (ivl_type_packed_dimensions(prop_type) == 1 &&
              ivl_type_packed_msb(prop_type, 0) >=
                  ivl_type_packed_lsb(prop_type, 0)));
      ...
      fprintf(vvp_out, "    %%load/obj v%p_0;\n", sig);
      fprintf(vvp_out,
              "    %%store/prop/v %d, %u; Store in logic property %s\n",
              prop_idx, lwid, ivl_type_prop_name(sig_type, prop_idx));
      fprintf(vvp_out, "    %%pop/obj 1, 0;\n");
