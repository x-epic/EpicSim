...
int Package::emit_package(ostream& fd) const {
  ...
  for (map<perm_string, const VType*>::const_iterator cur = cur_types_.begin();
    ...
    // dynamic_cast<const VTypeDef*>(cur->second) ? empty_perm_string :
    // cur->first);
    // fd << " ;" << endl;
  }

  // for (map<perm_string,struct const_t*>::const_iterator cur =
  // use_constants_.begin() ; cur != use_constants_.end() ; ++cur) {
  // fd << "localparam \\" << cur->first << " = ";
  // errors += cur->second->val->emit_package(fd);
  // fd << ";" << endl;
  //}
  // for (map<perm_string,struct const_t*>::const_iterator cur =
  // cur_constants_.begin() ; cur != cur_constants_.end() ; ++cur) {
  // fd << "localparam " << cur->first << " = ";
  // errors += cur->second->val->emit_package(fd);
  // fd << ";" << endl;
  //}
  ...
  for (map<perm_string, SubHeaderList>::const_iterator cur =
    ...
    for (SubHeaderList::const_iterator it = subp_list.begin();
      ...
      // Do not emit unbounded functions, we will just need fixed instances
      // later
      if (!header->unbounded())
